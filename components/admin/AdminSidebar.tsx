...
const SIDEBAR_BG = "linear-gradient(180deg, #0c1a3a 0%, #0f2050 35%, #1a3580 100%)";
  ...
  const UserFooterDark = () => (
    <div className="border-t border-white/10 px-3 py-3 space-y-0.5 shrink-0">
      <div className="flex items-center gap-2.5 px-2 py-2 rounded-lg">
        ...
        <button onClick={handleLogout} title="Đăng xuất" className="text-white/60 hover:text-white transition-colors shrink-0 cursor-pointer">
          <LogOut size={15} />
        </button>
      </div>
      {storeHref && (
        <Link href={storeHref} className="flex items-center gap-2.5 px-2 py-2 text-[13px] text-white/70 hover:text-white rounded-lg hover:bg-white/10 transition-all duration-150">
  ...
  return (
    <>
      ...
      >
        ...
        <NavContent navGroups={navGroups} allHrefs={allHrefs} pathname={pathname} openGroups={openGroups} toggleGroup={toggleGroup} dark={false} />
      ...
      <div className={`hidden md:flex ${collapsed ? "w-14" : "w-64"} h-full flex-col transition-all duration-300 relative z-40 border-r border-white/10`} style={{ background: SIDEBAR_BG }}>
        ...
        {collapsed ? (
          ...
          <NavContent navGroups={navGroups} allHrefs={allHrefs} pathname={pathname} openGroups={openGroups} toggleGroup={toggleGroup} dark={true} />
        )}

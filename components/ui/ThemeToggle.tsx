...
export default function ToggleSwitch({ className = "" }: ToggleSwitchProps) {
  ...
  return (
    ...
    >
      <div className={`absolute top-3 left-0 w-full flex justify-center transition-opacity duration-300 ${isDark ? "opacity-100" : "opacity-30"}`}>
        <Moon className="w-4 h-4 text-neutral-light" />
      </div>

      {/* Toggle button */}
      <div className={`absolute w-8 h-12 bg-neutral-light rounded-full left-1 shadow-md flex items-center justify-center transition-all duration-300 ${isDark ? "top-1" : "top-15"}`}>
        {isDark ? <Moon className="w-4 h-4 text-primary" /> : <Sun className="w-4 h-4 text-primary" />}
      </div>

      {/* Icon Sun (dưới) */}
      <div className={`absolute bottom-3 left-0 w-full flex justify-center transition-opacity duration-300 ${isDark ? "opacity-30" : "opacity-100"}`}>

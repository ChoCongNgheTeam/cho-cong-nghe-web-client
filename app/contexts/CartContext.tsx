...
function transformApiItem(raw: any): CartItemWithDetails {
   ...
   const storageLabel =
      raw.storageLabel ?? raw.storage ?? extractStorage(raw.variantCode ?? "");
   ...
   const updateQuantity = useCallback(
      async (cartItemId: string, delta: number): Promise<boolean> => {
         ...
         if (
            ...
            item.quantity + delta > item.availableQuantity
         ) {
            return false;
         }
         ...
         const newQty =
            item.availableQuantity > 0
               ...
               : Math.max(1, item.quantity + delta); // guest: không giới hạn trên

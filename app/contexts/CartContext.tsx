...
import {
   ...
   SyncCartPayload,
   writeLocalCart,
} from "@/(client)/cart/components/LocalCart";
...
export interface CartContextValue {
   ...
   updateItem: (
      cartItemId: string,
      patch: Partial<CartItemWithDetails>,
   ) => void;
...
function transformApiItem(raw: any): CartItemWithDetails {
   const now = Date.now();
   // Ưu tiên colorLabel nếu backend trả về, fallback về color
   const colorLabel = raw.colorLabel ?? raw.color ?? "";
   const storageLabel =
      raw.storageLabel ?? extractStorage(raw.variantCode ?? "");
   ...
   return {
      ...
      originalPrice: raw.originalPrice ?? raw.unitPrice ?? 0,
...
export function CartProvider({ children }: { children: ReactNode }) {
               ...
               if (res.success && Array.isArray(res.data?.items)) {
                  setItems((prev) =>
                     res.data.items.map((raw: CartItemWithDetails) => ({
                        ...
                        selected:
                           prev.find((p) => p.id === raw.id)?.selected ?? true,
                     })),
                  );
               } else {
                  setItems([]);
      ...
      async (
         ...
         if (existing) {
            ...
            local.push({
               ...
               variantCode:
                  (meta?.variantName ??
                     [storageLabel, colorLabel].filter(Boolean).join(" / ")) ||
                  "Mặc định",
               colorLabel,
               storageLabel,
               color: colorLabel,
   ...
   const changeVariant = useCallback(
      async (cartItemId: string, newVariant: NewVariantData): Promise<void> => {
         const newVariantCode = [newVariant.storageLabel, newVariant.colorLabel]
            .filter(Boolean)
            .join(" / ");
         ...
         setItems((prev) => {
            ...
            const duplicate = prev.find(
               (i) =>
                  i.productVariantId === newVariant.id && i.id !== cartItemId,
            );
         ...
         if (isAuthenticated) {
            try {
               ...
               const res = (await apiRequest.put(
                  `/cart/${cartItemId}/change-variant`,
                  {
   ...
   const totalDiscount = selectedItems.reduce(
      (s, i) =>
         s + Math.max(0, (i.originalPrice ?? 0) - i.unitPrice) * i.quantity,
      0,
   );

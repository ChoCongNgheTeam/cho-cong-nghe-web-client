...
function MediaForm({ initial, onSubmit, loading }: { initial?: Partial<FormState & { imageUrl?: string }>; onSubmit: (fd: FormData) => Promise<void>; loading: boolean }) {
  ...
  const labelCls = "mb-1.5 block text-xs font-semibold text-primary uppercase tracking-widest";
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <select className={selectCls} value={form.type} onChange={(e) => set("type", e.target.value as MediaType)}>
            {MEDIA_TYPES.map((t) => (
        ...
        <div>
          ...
          <select className={selectCls} value={form.position} onChange={(e) => set("position", e.target.value as MediaPosition)}>
      ...
      <div>
        ...
        <input className={inputCls} placeholder="Nhập tiêu đề..." value={form.title} onChange={(e) => set("title", e.target.value)} />
      </div>

      {/* Sub Title */}
      <div>
        <label className={labelCls}>Sub Title</label>
        <input className={inputCls} placeholder="Nhập mô tả ngắn..." value={form.subTitle} onChange={(e) => set("subTitle", e.target.value)} />
      </div>

      {/* Link URL */}
      <div>
        <label className={labelCls}>Link URL</label>
        <input className={inputCls} placeholder="https://..." value={form.linkUrl} onChange={(e) => set("linkUrl", e.target.value)} />
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <input className={inputCls} type="number" min={0} placeholder="Tự động" value={form.order} onChange={(e) => set("order", e.target.value)} />
        </div>
        <div>
          ...
          <button
            ...
            className={`flex h-[42px] w-full items-center justify-center gap-2 rounded-lg border text-sm font-medium transition ${
              form.isActive ? "border-green-300 bg-green-50 text-green-700 dark:border-green-700/40 dark:bg-green-900/20 dark:text-green-400" : "border-neutral bg-neutral-light text-neutral-dark"
            }`}
          >
            <span className={`h-2 w-2 rounded-full ${form.isActive ? "bg-green-500" : "bg-neutral-dark"}`} />
          ...
          {form.imagePreview ? (
            ...
            <div className="flex flex-col items-center gap-2 py-8 text-neutral-dark">
              <svg width="28" height="28" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 16.5V9.75m0 0 3 3m-3-3-3 3M6.75 19.5a4.5 4.5 0 0 1-1.332-8.82 3.997 3.997 0 0 1 6.614-3.654A4.5 4.5 0 0 1 17.25 19.5H6.75Z" />
              </svg>
...
function DeleteConfirmContent({ item, onConfirm, onCancel }: { item: Media; onConfirm: () => void; onCancel: () => void }) {
  return (
    <div className="space-y-4">
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="mt-0.5 text-sm text-primary">
            Bạn có chắc muốn xoá <span className="font-medium text-primary">&ldquo;{item.title ?? "media này"}&rdquo;</span>? Hành động này không thể hoàn tác.
          </p>
        </div>
      </div>
      ...
      {item.imageUrl && (
        // eslint-disable-next-line @next/next/no-img-element
        <img src={item.imageUrl} alt="" className="h-28 w-full rounded-lg object-cover opacity-80" />
      )}

      <div className="flex gap-3 pt-1">
        <button onClick={onCancel} className="flex-1 rounded-lg border border-neutral py-2.5 text-sm font-medium text-primary transition hover:bg-neutral-light-active">
          Huỷ
        </button>
        <button onClick={onConfirm} className="flex-1 rounded-lg bg-promotion py-2.5 text-sm font-semibold text-white transition hover:bg-promotion-hover active:bg-promotion-active">
          Xoá
        </button>
      </div>
    </div>
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-primary">
        Đặt vị trí mới cho <span className="font-semibold text-primary">&ldquo;{item.title ?? "media này"}&rdquo;</span> trong{" "}
        <span className="font-medium text-accent">{positionLabel[item.position]}</span>.
      </p>
      <div>
        <label className="mb-1.5 block text-xs font-semibold uppercase tracking-widest text-primary">Thứ tự mới</label>
        <input
          type="number"
          min={0}
          value={val}
  ...
  return (
    <div className="min-h-screen bg-neutral-light">
      <div className="px-6 py-10">
        ...
        <div className="mb-8 flex items-start justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-primary">
              {mediaList.length} mục · {mediaList.filter((m) => m.isActive).length} đang hiện
            </p>
          </div>
          <button
        ...
        <div className="mb-5 flex flex-wrap items-center gap-3">
          ...
          <div className="relative">
            <svg className="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-dark" width="15" height="15" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
          ...
          <select
            ...
            onChange={(e) => setFilterPosition(e.target.value as MediaPosition | "ALL")}
          >
          ...
          {/* Clear */}
          {(filterType !== "ALL" || filterPosition !== "ALL" || searchQuery) && (
            <button
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-neutral bg-neutral-light-active">
                  {["Hình ảnh", "Tiêu đề / Mô tả", "Type", "Position", "Thứ tự", "Trạng thái", "Đã xoá", "Thao tác"].map((h) => (
                    <th key={h} className="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-primary">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody className="divide-y divide-neutral">
                {loading ? (
                  <tr>
                    <td colSpan={8} className="py-20 text-center text-neutral-dark">
                  ...
                  filtered.map((item) => (
                    <tr key={item.id} className="transition hover:bg-neutral-light-hover">
                      {/* Image */}
                      <td className="px-5 py-3.5">
                        {item.imageUrl ? (
                          // eslint-disable-next-line @next/next/no-img-element
                          <img src={item.imageUrl} alt={item.title ?? ""} className="h-12 w-20 rounded-lg object-cover border border-neutral" />
                        ) : (
                      ...
                      {/* Title / Sub / Link */}
                      <td className="px-5 py-3.5">
                        <p className="font-medium text-primary">{item.title ?? <span className="italic text-neutral-dark">—</span>}</p>
                        {item.subTitle && <p className="mt-0.5 text-xs text-primary line-clamp-1">{item.subTitle}</p>}
                        {item.linkUrl && <p className="mt-0.5 max-w-[200px] truncate text-xs text-neutral-dark">{item.linkUrl}</p>}
                      </td>

                      {/* Type */}
                      <td className="px-5 py-3.5">
                        <span className={`rounded-md px-2.5 py-1 text-xs font-semibold ${typeColor[item.type]}`}>{item.type}</span>
                      </td>

                      {/* Position */}
                      <td className="px-5 py-3.5 text-xs text-primary">{positionLabel[item.position]}</td>

                      {/* Order */}
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-1.5">
                          <span className="font-mono text-sm text-primary">{item.order}</span>
                          <button onClick={() => openReorder(item)} title="Đổi thứ tự" className="rounded px-1.5 py-0.5 text-xs text-neutral-dark transition hover:bg-neutral hover:text-primary">
                            ↕
                          </button>
                        </div>
                      </td>
                      ...
                      <td className="px-5 py-3.5">
                        <button
                          ...
                          title={item.isActive ? "Click để ẩn" : "Click để hiện"}
                          className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${item.isActive ? "bg-green-500" : "bg-neutral"}`}
                        >
                          <span className={`absolute h-3.5 w-3.5 rounded-full bg-white shadow transition-transform ${item.isActive ? "translate-x-4" : "translate-x-0.5"}`} />
                        </button>
                      </td>
                      ...
                      <td className="px-5 py-3.5">
                        {item.deletedAt ? (
                          <div>
                            ...
                            <p className="mt-1 text-[11px] text-neutral-dark">{new Date(item.deletedAt).toLocaleDateString("vi-VN")}</p>
                            {item.deletedBy && <p className="text-[11px] text-neutral-dark truncate max-w-[120px]">bởi {item.deletedBy}</p>}
                          </div>
                        ) : (
                          <span className="text-xs text-neutral-dark italic">—</span>
                        )}
                      </td>
                      ...
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-1">
                          <button onClick={() => openEdit(item)} title="Chỉnh sửa" className="rounded-lg p-1.5 text-primary transition hover:bg-accent-light hover:text-accent">
                          ...
                          <button onClick={() => openDelete(item)} title="Xoá" className="rounded-lg p-1.5 text-primary transition hover:bg-promotion-light hover:text-promotion">
                            <svg width="15" height="15" fill="none" stroke="currentColor" strokeWidth={1.8} viewBox="0 0 24 24">
                              <path
                                strokeLinecap="round"
                                strokeLinejoin="round"
        ...
        {!loading && (
          <div className="mt-3 flex flex-wrap gap-4 text-xs text-primary">
            <span>
              Hiển thị <span className="font-medium text-primary">{filtered.length}</span> / <span className="font-medium text-primary">{mediaList.length}</span> mục
            </span>
            {MEDIA_POSITIONS.map((p) => {
              const count = mediaList.filter((m) => m.position === p).length;
              return count > 0 ? (
                <span key={p}>
                  {positionLabel[p]}: <span className="font-medium text-primary">{count}</span>
                </span>
              ) : null;
            })}
          </div>
      ...
      <Popzy
        ...
        content={
          <div>
            <h2 className="mb-5 text-base font-semibold text-primary">Tạo Media mới</h2>
      ...
      <Popzy
        ...
        content={
          editTarget ? (
            <div>
              <h2 className="mb-5 text-base font-semibold text-primary">Chỉnh sửa Media</h2>
              <MediaForm
                initial={{
      ...
      <Popzy
        ...
        content={
          reorderTarget ? (
            <div>
              <h2 className="mb-4 text-base font-semibold text-primary">Sắp xếp thứ tự</h2>
              <ReorderContent
                item={reorderTarget}

  ...
  return (
    <div className="flex flex-col items-center justify-center py-6 gap-2">
      <svg width="56" height="44" viewBox="0 0 56 44" fill="none">
        <rect x="4" y="22" width="9" height="18" rx="2" fill="#e2e8f0" />
        <rect x="18" y="14" width="9" height="26" rx="2" fill="#e2e8f0" />
        <rect x="32" y="8" width="9" height="32" rx="2" fill="#e2e8f0" />
        <rect x="46" y="16" width="9" height="24" rx="2" fill="#e2e8f0" />
        <path d="M2 42 L54 42" stroke="#cbd5e1" strokeWidth="1" strokeLinecap="round" />
      </svg>
      <p className="text-[11px] text-slate-400">Không có dữ liệu</p>
    </div>
  );
}

function DonutChart({ data, total }: { data: (OrderStatusBreakdown & { pct: number })[]; total: number }) {
  const SIZE = 120,
    STROKE = 20,
    GAP = 2;
  ...
  const arcs = data.reduce<Array<(typeof data)[number] & { startAngle: number; angle: number }>>((acc, d) => {
    ...
    const angle = Math.max((d.count / total) * (360 - data.length * GAP), 0.1);
    ...
    <div className="relative flex items-center justify-center">
      <svg width={SIZE} height={SIZE}>
        ...
        {arcs.map((arc) => {
          ...
          return <path key={arc.status} d={`M ${start.x} ${start.y} A ${R} ${R} 0 ${largeArc} 1 ${end.x} ${end.y}`} fill="none" stroke={cfg.color} strokeWidth={STROKE} strokeLinecap="round" />;
        })}
      </svg>
...
export function OrderStatusChart({ data }: { data: OrderStatusBreakdown[] }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-100 px-4 py-3 shadow-sm h-full flex flex-col">
      <h3 className="text-xs font-semibold text-slate-600 mb-2">Trạng thái đơn hàng</h3>
      <DonutChart data={normalized} total={total} />
      <div className="mt-2 space-y-1.5 flex-1">
        {normalized.length === 0 ? (
          <p className="text-[11px] text-slate-400 text-center">Chưa có dữ liệu</p>
        ) : (
          normalized.map((item) => {
            ...
            return (
              <div key={item.status} className="flex items-center gap-1.5">
                ...
                <div className="w-12 h-1 bg-slate-100 rounded-full overflow-hidden">
                  <div className="h-full rounded-full" style={{ width: `${item.pct}%`, backgroundColor: cfg.color }} />
                </div>
                <span className="text-[11px] font-semibold text-slate-700 w-4 text-right">{item.count}</span>

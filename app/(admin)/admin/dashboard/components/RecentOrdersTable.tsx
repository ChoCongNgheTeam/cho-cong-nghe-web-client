   ...
   return (
      <div className="bg-neutral-light rounded-xl border border-neutral shadow-sm overflow-hidden">
         ...
         <div className="flex items-center justify-between px-4 py-2.5 border-b border-neutral">
            ...
            <Link
               href="/admin/orders"
               className="text-[13px] text-accent hover:text-accent-hover font-medium flex items-center gap-0.5"
            >
         ...
         {orders.length === 0 ? (
            <div className="py-10 text-center text-neutral-dark text-xs">
               {emptyMessage}
            </div>
         ) : (
               ...
               <table className="w-full text-xs">
                  <thead>
                     <tr className="bg-neutral border-b border-neutral">
                        <th className="text-left text-[13px] font-medium text-neutral-dark px-4 py-2">
                           Mã đơn
                        </th>
                        <th className="text-left text-[13px] font-medium text-neutral-dark px-3 py-2">
                           Khách hàng
                        </th>
                        <th className="text-left text-[13px] font-medium text-neutral-dark px-3 py-2">
                           Sản phẩm
                        ...
                        <th className="text-right text-[13px] font-medium text-neutral-dark px-3 py-2">
                           TT
                        </th>
                        <th className="text-right text-[13px] font-medium text-neutral-dark px-3 py-2">
                           Tổng tiền
                        </th>
                        <th className="text-right text-[13px] font-medium text-neutral-dark px-4 py-2">
                           Thời gian
                        </th>
                     </tr>
                  </thead>
                  <tbody className="divide-y divide-neutral">
                     {orders.map((order) => {
                        ...
                        return (
                           <tr
                              key={order.id}
                              className="hover:bg-neutral-light-hover transition-colors group"
                           >
                              {/* Order code */}
                              <td className="px-4 py-2.5">
                                 <div className="flex items-center gap-1.5">
                                    {order.isChatbotRequest && (
                                       <Bot
                                          className="w-3 h-3 text-accent-active shrink-0"
                                          aria-label="Đơn chatbot"
                                       />
                                    )}
                                    <Link
                                       href={`/admin/orders/${order.id}`}
                                       className="font-mono text-[13px] text-accent hover:text-accent-hover font-semibold group-hover:underline"
                                    >
                                       #{order.orderCode}
                                    </Link>
                                 </div>
                              ...
                              <td className="px-3 py-2.5 max-w-[160px]">
                                 <p className="font-medium text-primary text-[13px] leading-tight truncate">
                                    {order.customerName}
                                 </p>
                                 {order.customerPhone && (
                                    ...
                                    <div className="space-y-0.5">
                                       {order.items
                                          ...
                                          .map((item, i) => (
                                             ...
                                             >
                                                <span className="text-[13px] text-primary font-medium truncate max-w-[130px]">
                                                   {item.productName}
                                                </span>
                                                <span className="text-[13px] text-neutral-dark shrink-0">
                                                   ×{item.quantity}
                                                </span>
                                             </div>
                                          ))}
                              ...
                              <td className="px-3 py-2.5">
                                 <span
                                    className={`inline-flex items-center px-1.5 py-0.5 rounded text-[13px] font-medium border ${statusCfg.cls}`}
                                 >
                                    {statusCfg.label}
                                 </span>
                              </td>
                              ...
                              <td className="px-3 py-2.5 text-right">
                                 <span className={`text-[13px] ${payment.cls}`}>
                                    {payment.label}
                                 </span>
                              </td>
                              ...
                              {/* Date */}
                              <td className="px-4 py-2.5 text-right text-[13px] text-neutral-dark whitespace-nowrap">

...
export const formatVNDFull = (value: number) =>
...
function StatCard({ label, value, subLabel, change, icon, iconCls, alert, primary, sparkline }: StatCardProps) {
  ...
  return (
    <div className="bg-neutral-light border border-neutral rounded-xl px-3.5 py-3 flex flex-col justify-between shadow-sm relative overflow-hidden min-h-[80px]">
      {sparkline && sparkline.length >= 2 && (
        <div className="absolute bottom-0 right-1 opacity-25 pointer-events-none">
          <Sparkline data={sparkline} color="var(--color-accent)" opacity={0.9} />
        </div>
      )}
      <div className="flex items-center justify-between">
        ...
        {change !== undefined && (
          <span
            className={`inline-flex items-center gap-0.5 text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${isPositive ? "bg-accent-light text-accent-dark" : "bg-promotion-light text-promotion"}`}
          >
            <TrendIcon className="w-2.5 h-2.5" />
            {Math.abs(change)}%
          </span>
      ...
      <div className="mt-1 relative">
        <p className="text-base font-bold text-primary leading-tight tracking-tight">{value}</p>
        <p className="text-[13px] text-neutral-dark mt-0.5">{label}</p>
        {subLabel && <p className="text-[10px] text-neutral-dark-hover">{subLabel}</p>}
      </div>
      {alert && alert.count > 0 && (
        <div className="flex items-center gap-1 bg-accent-light border border-accent-light-active rounded px-1.5 py-0.5 mt-1">
          <Bot className="w-2.5 h-2.5 text-accent shrink-0" />
          <span className="text-[12px] text-accent-dark font-medium truncate">
            {alert.count} {alert.label}
          </span>
        </div>
      )}

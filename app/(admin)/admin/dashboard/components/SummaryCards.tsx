   ...
   return (
      <div className="bg-neutral-light border border-neutral rounded-xl px-3.5 py-3 flex flex-col justify-between shadow-sm relative overflow-hidden min-h-[80px]">
         ...
         <div className="flex items-center justify-between">
            <div
               className={`w-6 h-6 rounded-md flex items-center justify-center ${iconCls}`}
            >
               {icon}
            </div>
            {change !== undefined && (
         ...
         <div className="mt-1 relative">
            <p className="text-base font-bold text-primary leading-tight tracking-tight">
               {value}
            </p>
            <p className="text-[13px] text-neutral-dark mt-0.5">{label}</p>
            {subLabel && (
               <p className="text-[10px] text-neutral-dark-hover">{subLabel}</p>
            )}
         </div>
         {alert && alert.count > 0 && (
...
export function SummaryCards({ summary }: { summary: DashboardSummary }) {
   const cards: StatCardProps[] = [
      ...
      },
      {
         ...
         alert:
            summary.orders.pendingChatbot > 0
               ? { label: "chatbot chờ", count: summary.orders.pendingChatbot }
               : null,
         ...
         alert:
            summary.products.lowStock + summary.products.outOfStock > 0
               ? {
                    label: "sắp/hết hàng",
                    count:
                       summary.products.lowStock + summary.products.outOfStock,
                 }
               : null,
      },
   ];

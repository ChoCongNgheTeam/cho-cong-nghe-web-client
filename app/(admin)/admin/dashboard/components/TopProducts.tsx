...
function StockBadge({ stock, days }: { stock?: number; days?: number }) {
  ...
  return <span className="text-[9px] text-neutral-dark font-mono">{stock}</span>;
}
  ...
  return (
    <div className="bg-neutral-light rounded-xl border border-neutral px-4 py-3 shadow-sm h-full flex flex-col">
      <div className="flex items-center justify-between mb-2">
        ...
        <Link href={href(`/products`)} className="text-[10px] text-accent hover:text-accent-hover font-medium flex items-center gap-0.5">
          Xem tất cả <ArrowUpRight className="w-3 h-3" />
        </Link>
      </div>

      {products.length === 0 ? (
        <p className="text-[13px] text-neutral-dark text-center py-6">Chưa có dữ liệu</p>
      ) : (
          ...
          {products.map((product, index) => (
            <div key={product.productId} className="flex items-center gap-3">
              ...
              <div className="w-8 h-8 rounded bg-neutral overflow-hidden shrink-0 flex items-center justify-center">
                {product.imageUrl ? <Image src={product.imageUrl} alt={product.productName} width={32} height={32} className="object-cover" /> : <Package className="w-3.5 h-3.5 text-neutral-dark" />}
              </div>

              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5">
                  <Link href={`/admin/products?slug=${product.productSlug}`} className="text-[13px] font-semibold text-primary hover:text-accent truncate block">
                    {product.productName}
                  </Link>
              ...
              <div className="text-right shrink-0">
                <p className="text-[13px] font-bold text-primary">{product.totalSold} đã bán</p>
                <p className="text-[10px] text-neutral-dark">{fmtVND(product.totalRevenue)}</p>
              </div>
            </div>
          ))}
        </div>

...
const productTabs = ["Sắp hết hàng", "Hết hàng", "Sản phẩm ngừng kinh doanh", "Sản phẩm mới"];
...
function MiniLineChart({ color = "#F97316" }: { color?: string }) {
  return (
    <svg viewBox="0 0 300 80" className="w-full h-20" preserveAspectRatio="none">
      <polyline points="0,60 30,50 60,55 90,35 120,45 150,30 180,40 210,55 240,38 270,45 300,35" fill="none" stroke={color} strokeWidth="2.5" strokeLinejoin="round" strokeLinecap="round" />
    </svg>
  );
}
...
function TinyWaveLine({ color = "#10B981" }: { color?: string }) {
  return (
    <svg viewBox="0 0 120 30" className="w-24 h-6" preserveAspectRatio="none">
      <polyline points="0,15 20,8 40,18 60,5 80,20 100,10 120,15" fill="none" stroke={color} strokeWidth="2" strokeLinejoin="round" />
    </svg>
  );
}
...
function DonutChart({ segments, cx = 80, cy = 80, r = 60, innerR = 38 }: { segments: { value: number; color: string; label: string }[]; cx?: number; cy?: number; r?: number; innerR?: number }) {
      ...
      <div className="bg-white border-b border-gray-100 px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            <input
              type="text"
              placeholder="Tìm kiếm sản phẩm, đơn hàng, tổ cáo,..."
              className="w-72 pl-9 pr-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-gray-50"
            />
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 col-span-1">
            <h3 className="text-sm font-semibold text-gray-700 mb-3">Người dùng mới</h3>
            <MiniLineChart color="#F97316" />
            <div className="flex justify-between text-xs text-gray-400 mt-1 px-1">
              {["01 Jan", "02", "03", "04", "05", "06", "07", "08", "09"].map((d) => (
                <span key={d}>{d}</span>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
            <h3 className="text-sm font-semibold text-gray-700 mb-2">Lượt đánh giá trung bình</h3>
            <div className="text-5xl font-bold text-gray-900 mt-4 mb-2">4.5</div>
            ...
            <div className="flex items-center gap-2 mt-3">
              <span className="text-green-600 text-sm font-medium">↑ +35</span>
          ...
          {/* Best selling */}
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
            <h3 className="text-sm font-semibold text-gray-700 mb-3">Sản phẩm bán chạy</h3>
            <div className="flex items-center justify-center mb-3 relative">
              ...
              <div className="absolute text-center">
                ...
                <div className="text-xs font-bold text-gray-800">12,142,000đ</div>
              </div>
            </div>
            <div className="space-y-1">
              {Object.entries(bestSellerColors).map(([label, color]) => (
                <div key={label} className="flex items-center justify-between text-xs">
                  ...
                  <span className="text-gray-700 font-medium">12,000,000đ</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="grid grid-cols-3 gap-4">
              ...
              {productTabs.map((tab, i) => (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-2 text-sm rounded-t-lg font-medium transition-colors ${
                    i === activeTab ? "bg-blue-50 text-blue-600 border-b-2 border-blue-600" : "text-gray-500 hover:text-gray-700"
                  }`}
                >
                  {tab}
                  <span className="bg-orange-500 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center font-bold">{tabCounts[i]}</span>
                </button>
              ))}
            </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100">
                    ...
                    <th className="text-left px-3 py-3 text-gray-500 font-medium">Tên sản phẩm</th>
                    <th className="text-left px-3 py-3 text-gray-500 font-medium">Danh mục</th>
                    <th className="text-left px-3 py-3 text-gray-500 font-medium">Giá sản phẩm</th>
                    <th className="text-left px-3 py-3 text-gray-500 font-medium">Tồn Kho</th>
                    <th className="text-left px-3 py-3 text-gray-500 font-medium">Trạng thái</th>
                    <th className="text-left px-3 py-3 text-gray-500 font-medium">Ngày tạo</th>
                  </tr>
                </thead>
                <tbody>
                  {products.map((p) => (
                    <tr key={p.id} className="border-b border-gray-50 hover:bg-gray-50">
                      ...
                      <td className="px-3 py-2.5 text-gray-600 text-xs">{p.category}</td>
                      <td className="px-3 py-2.5 text-gray-600 text-xs">{p.price}</td>
                      <td className="px-3 py-2.5 text-gray-600 text-xs">{p.stock}</td>
                      <td className="px-3 py-2.5">
                        <span className={`px-2 py-1 rounded-md text-xs font-medium ${p.status === "Sắp hết hàng" ? "bg-orange-100 text-orange-600" : "bg-red-100 text-red-600"}`}>{p.status}</span>
                      </td>
          ...
          {/* Stock chart */}
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
            <h3 className="text-sm font-semibold text-gray-700 mb-3">Sản phẩm tồn kho</h3>
            <div className="flex justify-center mb-4">
              <DonutChart
                segments={[
                  { value: 10, color: "#F59E0B", label: "Sắp hết hàng" },
                  { value: 21, color: "#EF4444", label: "Hết hàng" },
                  { value: 32, color: "#6B7280", label: "Sản phẩm ngừng kinh doanh" },
                  { value: 14, color: "#3B82F6", label: "Sản phẩm mới" },
                ]}
              />
            </div>
              ...
              {Object.entries(stockColors).map(([label, color]) => {
                ...
                return (
                  <div key={label} className="flex items-center justify-between text-xs">
                    ...
                    <span className="text-gray-700 font-semibold">{counts[label]}</span>
                  </div>
                );
              })}
            </div>

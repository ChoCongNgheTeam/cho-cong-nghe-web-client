...
function Avatar({ user }: { user: Comment["user"] }) {
  if (user?.avatarImage) {
    return <img src={user.avatarImage} alt={user.fullName ?? user.email} className="w-7 h-7 rounded-full object-cover flex-shrink-0" />;
  }
...
export function ReplyCommentModal({ commentId, onClose, onReplied }: ReplyCommentModalProps) {
  ...
  const modalContent = (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2.5 pb-3 border-b border-neutral">
        ...
        <div>
          <p className="text-[14px] font-bold text-primary">Phản hồi bình luận</p>
          <p className="text-[11px] text-neutral-dark">{replies.length > 0 ? "Bình luận này đã được phản hồi" : "Phản hồi sẽ hiển thị công khai bên dưới bình luận"}</p>
        </div>
      </div>
          ...
          <div className="rounded-xl bg-white border border-neutral p-3.5 space-y-2">
            <div className="flex items-center gap-2">
              ...
              <div className="min-w-0">
                <p className="text-[12px] font-semibold text-primary truncate">{comment.user?.fullName ?? comment.user?.email ?? "Ẩn danh"}</p>
                <p className="text-[10px] text-neutral-dark">{formatDate(comment.createdAt)}</p>
              </div>
            </div>
            <p className="text-[13px] text-primary leading-relaxed pl-9">{comment.content}</p>
          </div>
          ...
          {replies.length > 0 && (
            <div className="space-y-2">
              ...
              <div className="space-y-2 pl-3 border-l-2 border-neutral max-h-48 overflow-y-auto pr-1">
                {replies.map((reply) => (
                  <div key={reply.id} className="rounded-lg bg-white border border-neutral p-3 space-y-1.5">
                    <div className="flex items-center gap-2">
                      <Avatar user={reply.user} />
                      <div className="min-w-0 flex-1">
                        <p className="text-[11px] font-semibold text-primary truncate">{reply.user?.fullName ?? reply.user?.email ?? "Ẩn danh"}</p>
                        <p className="text-[10px] text-neutral-dark">{formatDate(reply.createdAt)}</p>
                      </div>
                      {reply.isApproved ? (
                        <span className="text-[10px] font-medium text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded-md flex-shrink-0">Đã duyệt</span>
                      ) : (
                        <span className="text-[10px] font-medium text-orange-500 bg-orange-50 px-1.5 py-0.5 rounded-md flex-shrink-0">Chờ duyệt</span>
                      )}
                    </div>
                    <p className="text-[12px] text-primary leading-relaxed pl-9">{reply.content}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          {replies.length === 0 && (
            <div className="space-y-2">
              ...
              <div className="flex items-center justify-end gap-2">
                <button onClick={onClose} className="px-4 py-2 text-[13px] border border-neutral rounded-lg text-primary hover:bg-neutral-light-active transition-colors cursor-pointer">
                  Hủy
                </button>
                <button
  ...
  return <Popzy isOpen={!!commentId} onClose={onClose} content={modalContent} closeMethods={["button", "overlay", "escape"]} cssClass="!w-[min(560px,92%)]" />;
}

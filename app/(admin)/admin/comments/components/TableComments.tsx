...
export function getCommentColumns({ page, pageSize, selected, toggleOne, onViewClick, onApproveClick, onDeleteClick }: GetCommentColumnsParams): AdminColumn<Comment>[] {
  return [
    {
      key: "_select",
      label: "",
    ...
    },
    {
      ...
      render: (comment) => (
        <div className="space-y-0.5">
          ...
          {comment.parentId && <span className="text-[11px] text-neutral-dark italic">↩ Reply</span>}
        </div>
      ),
    },
    {
      ...
      label: "Loại",
      render: (comment) => (
        <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-medium ${TARGET_TYPE_COLORS[comment.targetType] ?? "text-neutral-dark bg-neutral-light-active"}`}>

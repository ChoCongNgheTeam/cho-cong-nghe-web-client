...
export const updateComment = async (
   id: string,
   payload: { content?: string; isApproved?: boolean },
): Promise<MutateCommentResponse> => {
   return apiRequest.patch<MutateCommentResponse>(
      `/comments/admin/${id}`,
      payload,
   );
};
...
export const bulkApproveComments = async (
   ...
   return apiRequest.patch<{ message: string }>(
      "/comments/admin/bulk/approve",
      { commentIds, isApproved },
   );
};
...
export const getCommentReplies = async (
   ...
   return apiRequest.get<{ data: Comment[]; message: string }>(
      `/comments/${id}/replies`,
   );
};
...
export const createCommentReply = async (
   parentId: string,
   payload: { content: string; targetId: string; targetType: string },
): Promise<MutateCommentResponse> => {
   return apiRequest.post<MutateCommentResponse>("/comments", {

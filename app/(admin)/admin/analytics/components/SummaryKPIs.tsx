...
function ChangePill({ change }: { change?: number }) {
  ...
  return (
    <span className={`inline-flex items-center gap-0.5 text-[9px] font-semibold px-1 py-0.5 rounded ${up ? "bg-accent-light text-accent-dark" : "bg-promotion-light text-promotion"}`}>
...
function KPICard({ label, value, sub, icon, iconBg, change, primary, danger }: KPICardProps) {
  ...
  return (
    <div className={`bg-neutral-light rounded-xl border px-3 py-2.5 flex items-center gap-2.5 shadow-sm ${danger ? "border-promotion-light-active" : "border-neutral"}`}>
      <div className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 ${iconBg}`}>{icon}</div>
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5">
          <p className={`text-sm font-bold truncate ${danger ? "text-promotion" : "text-primary"}`}>{value}</p>
          <ChangePill change={change} />
        </div>
        <p className="text-[10px] text-primary truncate">{label}</p>
        {sub && <p className="text-[9px] text-neutral-dark truncate">{sub}</p>}
      </div>
    </div>
  );
}
...
export function SummaryKPIs({ summary }: { summary: AnalyticsSummary }) {
  return (
    <div className="flex flex-col gap-2">
      ...
      />
      <KPICard label="Tổng đơn hàng" value={fmtNum(summary.totalOrders)} icon={<ShoppingCart className="w-3.5 h-3.5" />} iconBg="bg-accent-light text-accent" change={summary.ordersChange} />
      <KPICard label="Đã giao thành công" value={fmtNum(summary.totalDelivered)} icon={<CheckCircle className="w-3.5 h-3.5" />} iconBg="bg-accent-light text-accent-dark" />
      <KPICard label="Tỉ lệ giao hàng" value={fmtPct(summary.deliveryRate)} icon={<TrendingUp className="w-3.5 h-3.5" />} iconBg="bg-accent-light text-accent" />
      <KPICard
        label="Tỉ lệ huỷ đơn"
        value={fmtPct(summary.cancellationRate)}
        icon={<XCircle className="w-3.5 h-3.5" />}
        iconBg={summary.cancellationRate > 10 ? "bg-promotion-light text-promotion" : "bg-neutral text-neutral-dark"}
        danger={summary.cancellationRate > 10}
      />
      <KPICard label="Giá trị đơn TB" value={fmtVNDShort(summary.averageOrderValue)} icon={<BarChart3 className="w-3.5 h-3.5" />} iconBg="bg-accent-light text-accent-active" />
    </div>
  );
}

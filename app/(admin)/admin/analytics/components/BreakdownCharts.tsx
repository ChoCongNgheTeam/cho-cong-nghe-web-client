...
function PaymentTooltip({ active, payload }: TooltipProps<ValueType, NameType> & { payload?: any[] }) {
  ...
  return (
    <div className="bg-primary text-white rounded-lg px-2.5 py-1.5 shadow-xl text-xs">
      ...
      <p className="text-primary text-[10px]">{formatVND(d.revenue)}</p>
...
export function PaymentMethodChart({ data }: { data: RevenueByPaymentMethod[] }) {
  return (
    <div className="bg-neutral-light rounded-xl border border-neutral px-4 py-3 shadow-sm">
      <h3 className="font-semibold text-primary mb-2">Doanh thu theo PTTT</h3>

      {data.length === 0 ? (
        <p className="text-[11px] text-neutral-dark text-center py-6">Không có dữ liệu</p>
      ) : (
              ...
              <div key={d.methodCode}>
                <div className="flex items-center justify-between mb-0.5">
                  <div className="flex items-center gap-1 min-w-0">
                    ...
                    />
                    <span className="text-[10px] text-primary truncate">{d.method}</span>
                  </div>
                  <span className="text-[10px] font-bold text-primary ml-1 shrink-0">{d.percentage.toFixed(1)}%</span>
                </div>
...
export function CategoryChart({ data }: { data: RevenueByCategory[] }) {
  ...
  return (
    <div className="bg-neutral-light rounded-xl border border-neutral px-4 py-3 shadow-sm">
      <h3 className="text-xs font-semibold text-primary mb-2">Doanh thu theo danh mục</h3>
      ...
      {data.length === 0 ? (
        ...
        <ResponsiveContainer width="100%" height={chartH}>
          <BarChart layout="vertical" data={chartData} margin={{ top: 0, right: 48, left: 0, bottom: 0 }} barCategoryGap="30%">
            ...
            <CartesianGrid strokeDasharray="3 3" stroke="var(--color-neutral)" horizontal={false} />
            <XAxis type="number" tickFormatter={fmtShort} tick={{ fontSize: 8, fill: "var(--color-neutral-dark)" }} axisLine={false} tickLine={false} />
            <YAxis type="category" dataKey="categoryName" tick={{ fontSize: 9, fill: "var(--color-primary)" }} axisLine={false} tickLine={false} width={96} />
            <Tooltip content={<CategoryTooltip />} cursor={{ fill: "var(--color-neutral)" }} />

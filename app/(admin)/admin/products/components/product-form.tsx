...
import { Loader2, X, Star, AlertCircle, Info, ChevronDown, ChevronRight, ImagePlus, Trash2, Plus, Package, Palette, Layers, FileText, Sparkles, Check, BadgeDollarSign, Pencil } from "lucide-react";
...
import { ChipAttributeSelector } from "./Chipattributeselector";
import { SpecImportPanel } from "../../ai-content/SpecImportPanel";
      ...
      .map((node) => {
        ...
        return (
          <div key={node.id}>
            ...
            {!isLeaf && isOpen && (
              <div className="ml-4 pl-3 border-l border-neutral mt-0.5 mb-0.5">{selfMatch ? renderAllChildren(node.children, fullPath) : renderNodes(node.children, fullPath)}</div>
            )}
          </div>
        );
      });
  ...
  return (
    <div className="flex flex-col gap-1.5 min-w-[110px]">
      <div className="flex items-center gap-1.5 px-2 py-1 rounded-md bg-neutral-light-active border border-neutral">
        <span className="text-[10px] text-neutral-dark whitespace-nowrap">Kho:</span>
      ...
      >
        <div className="flex items-center gap-1.5">
          <span className="text-[10px] text-neutral-dark whitespace-nowrap">+Thêm:</span>
          <input
            type="number"
            value={addQuantity}
            min="0"
            ...
            className={`w-[44px] px-1 py-0.5 text-[11px] text-center border rounded-md ml-auto tabular-nums
                     focus:outline-none transition-all
                     ${hasAdd ? "border-accent bg-accent-light/30 text-accent-dark focus:border-accent" : "border-neutral bg-neutral-light focus:bg-neutral-light focus:border-primary"}`}
          />
        </div>
        {hasAdd && (
          <div className="flex items-center gap-1 pt-0.5 border-t border-accent/20">
            <span className="text-[9px] text-neutral-dark tabular-nums">{existingQuantity}</span>
            <span className="text-[9px] text-neutral-dark">→</span>
            <span className="text-[11px] font-bold text-accent tabular-nums">{total}</span>
            <span className="text-[9px] text-accent/70 ml-auto tabular-nums">+{addNum}</span>
          </div>
        )}
      </div>
    </div>
        ...
        <table className="w-full border-collapse" style={{ minWidth: 580 }}>
          <thead>
            <tr className="bg-neutral-light-active border-b border-neutral">
              ...
              <th className="px-3 py-2.5 text-left text-[10px] font-bold text-neutral-dark uppercase tracking-wider w-[110px]">Tồn kho</th>
...
function ColorImages({
  ...
  const [editingColorKeys, setEditingColorKeys] = useState<Set<string>>(new Set());

  const toggleEditColor = (key: string) => {
    setEditingColorKeys((prev) => {
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        {colorImages.map((ci) => {
          ...
          const SLOTS = 5;
          const isEditingColor = !ci.color || editingColorKeys.has(ci._key);
          const isRenamed = isEdit && ci.originalColor !== undefined && ci.originalColor !== ci.color;
            ...
            <div key={ci._key} className="border border-neutral rounded-lg overflow-hidden">
              <div className="flex items-center justify-between px-3 py-2 bg-neutral-light-active border-b border-neutral">
                <div className="flex items-center gap-2 flex-1 min-w-0">
                  ...
                  <span className="text-[12px] font-semibold text-primary truncate">{(label ?? ci.color) || "Chưa chọn màu"}</span>
                  {ci.color && label && <span className="text-[10px] text-neutral-dark font-mono shrink-0">{ci.color}</span>}
                  {isRenamed && <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-accent-light text-accent font-semibold shrink-0 border border-accent-light-active">đổi màu</span>}
                  {toDeleteCount > 0 && <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-promotion-light text-promotion font-semibold shrink-0">−{toDeleteCount}</span>}
                  {newCount > 0 && <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-primary text-neutral-light font-semibold shrink-0">+{newCount}</span>}
                </div>
                <div className="flex items-center gap-1 shrink-0">
                  {ci.color && (
                    <button
                      type="button"
                      onClick={() => toggleEditColor(ci._key)}
                      title="Sửa màu"
                      className={`w-5 h-5 flex items-center justify-center rounded transition-colors cursor-pointer
                                    ${editingColorKeys.has(ci._key) ? "text-primary bg-neutral-light-active" : "text-neutral-dark hover:text-primary hover:bg-neutral-light-active"}`}
                    >
                      <Pencil size={11} />
                    </button>
                  )}
              ...
              <div className="p-3 space-y-2.5">
                {isEditingColor && (
                  <div className="grid grid-cols-2 gap-2 pb-2 border-b border-neutral">
                    ...
                    {ci.color && (
                      <button type="button" onClick={() => toggleEditColor(ci._key)} className="col-span-2 text-[11px] text-neutral-dark hover:text-primary transition-colors cursor-pointer text-left">
                        ✓ Đóng chỉnh sửa màu
                      </button>
                    )}
                  </div>
...
export default function ProductForm({ product }: ProductFormProps) {
    ...
    setVariants((prev) => {
      ...
      const newVariants = combos.map((attrMap) => {
        ...
        return {
          ...
          computedPrice: cp,
          existingQuantity: 0,
          addQuantity: String(DEFAULT_NEW_QUANTITY),
          isDefault: false,
          isActive: true,
          attributes: attrMap,
        };
    ...
    setColorImages((prev) => {
      ...
      prev.forEach((ci) => {
        if (!keptKeys.has(ci._key)) {
          ci.previews.forEach((u) => URL.revokeObjectURL(u));
        }
      });
      ...
      const toAdd = selectedColorValues
        ...
        .map((c) => {
          ...
          const restoredImages = isEdit
            ? (originalColorImagesRef.current.get(c) ?? []).map((img) => ({
                ...img,
          ...
          return {
            ...
            files: [],
            previews: [],
            ...(restoredImages !== undefined ? { existingImages: restoredImages } : {}),
          };
        });
  ...
  const handleAddOption = (attrId: string, createdId: string, label: string, value: string) =>
    setTemplate((prev) =>
      prev
        ? {
            ...prev,
  ...
  const handleUpdateColor = (key: string, field: "color" | "altText", val: string) => {
    if (field !== "color") {
      setColorImages((p) => p.map((c) => (c._key === key ? { ...c, [field]: val } : c)));
      return;
    }

    const colorAttr = template?.attributes.find((a) => a.code === "color");
    if (!colorAttr) {
      setColorImages((p) => p.map((c) => (c._key === key ? { ...c, color: val } : c)));
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const variantsPayload = variants.map((v) => ({
        ...
        quantity: v.existingQuantity + (Number(v.addQuantity) || 0),
        isDefault: v.isDefault,
        isActive: v.isActive,
      ...
      const colorImagesPayload = colorImages
        .filter((c) => {
          ...
          const colorRenamed = c.originalColor !== undefined && c.originalColor !== c.color;
          const hasNewFiles = c.files.length > 0;
          const hasDeletedImages = (c.existingImages ?? []).some((i) => i.toDelete);
          return colorRenamed || hasNewFiles || hasDeletedImages;
        })
        .map((c) => ({
          color: c.color.trim(),
          altText: c.altText.trim() || c.color.trim(),
          ...(isEdit && c.originalColor !== undefined ? { originalColor: c.originalColor } : {}),
          ...
          <Section icon={<Package size={13} />} title="Thông số kỹ thuật" badge={`${filledSpecsCount}/${totalSpecsCount}`} defaultOpen={false}>
            <div className="space-y-5">
              ...
              {/* <AiSpecSuggestButton
                productName={name}
                categoryId={categoryId}
                specGroups={template.specifications.map((group) => ({
                ...
                specs={specs}
                onApply={(updates) => {
                  Object.entries(updates).forEach(([specId, value]) => {
                    updateSpec(specId, value);
                  });
                }}
              /> */}
              {/* ── SPEC IMPORT PANEL ── */}
              <SpecImportPanel
                ...
                specs={specs}
                onApply={(updates) => {
                  Object.entries(updates).forEach(([specId, { value, isHighlight }]) => {
                    updateSpec(specId, value);
                    if (isHighlight) toggleHighlight(specId);
                  });
                }}
              />

...
export default function AdminCategoriesPage() {
   ...
   const handleInlineUpdate = (id: string, patch: Partial<Category>) =>
      setCategories((prev) =>
      ...
      <div className="space-y-5 p-5 bg-neutral-light min-h-full">
         ...
         <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <StatsCard
               label="Danh mục gốc"
               value={
                  loading ? "—" : categories.filter((c) => !c.parentId).length
               }
         ...
         <div className="bg-neutral-light border border-neutral rounded-2xl overflow-hidden shadow-sm">
               ...
               {STATUS_TABS.map((tab) => {
                  ...
                  return (
                     ...
                     >
                        {tab.label}
                        <span
                           className={`text-[11px] px-1.5 py-0.5 rounded-md font-semibold ${activeTab === tab.value ? "bg-white/20 text-white" : "bg-neutral-light-active text-primary"}`}
                        >
                           {count}
                        </span>
                     </button>
            ...
            <div className="px-5 py-2.5 border-b border-neutral bg-neutral-light-active/40 flex items-center gap-2 flex-wrap">
               ...
               <span className="text-[11px] text-primary font-medium mr-1">
                  Lọc thêm:
               </span>
                  ...
                  <thead>
                     <tr className="bg-neutral-light-active border-b border-neutral">
                        {[
                           ...
                           <th
                              key={col}
                              className="px-4 py-3 text-left text-[11px] font-semibold text-primary uppercase tracking-wider whitespace-nowrap"
                           >
                              {col}
                           </th>
                        ))}
                  ...
                  <tbody>
                     {loading ? (
                        ...
                        categories.map((cat, idx) => {
                           ...
                           const hasChildren = (cat._count?.children ?? 0) > 0;
                           // Chỉ cho xóa khi không có sản phẩm VÀ không có danh mục con
                           const canDelete = !hasProducts && !hasChildren;

                           return (
                              <tr
                                 key={cat.id}
                                 className={`border-b border-neutral hover:bg-neutral-light-active/50 transition-colors duration-100 ${isChild ? "bg-neutral-light-active/20" : ""}`}
                              >
                                 <td className="px-4 py-3.5 text-[12px] text-primary w-12">
                                    {rowNum}
                                 </td>
                                 ...
                                 <td className="px-4 py-3.5">
                                    <div className="flex items-center gap-2.5">
                                       ...
                                       <div className="min-w-0">
                                          <div className="text-[13px] font-medium text-primary truncate max-w-[200px]">
                                             {cat.name}
                                          </div>
                                          <div className="text-[11px] text-primary font-mono truncate max-w-[200px]">
                                             /{cat.slug}
                                          </div>
                                       </div>
                                    </div>
                                 ...
                                 <td className="px-4 py-3.5">
                                    <span
                                       className={`inline-flex px-2 py-0.5 rounded-md text-[11px] font-medium ${isChild ? "bg-violet-50 text-violet-600" : "bg-blue-50 text-blue-600"}`}
                                    >
                                       {isChild
                                          ? "Danh mục con"
                                          : "Danh mục gốc"}
                                 ...
                                 <td className="px-4 py-3.5">
                                    <div className="flex items-center gap-1">
                                       ...
                                       {/* Chỉ hiện nút xóa khi không có sản phẩm VÀ không có danh mục con */}
                                       {canDelete && (
                                          <button
                                             onClick={() =>
                                                setDeleteTarget(cat)

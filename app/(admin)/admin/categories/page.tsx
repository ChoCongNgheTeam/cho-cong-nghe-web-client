...
import { getCategoriesAdmin, softDeleteCategory, updateCategory } from "./_lib/categories";
...
function buildTree(flat: Category[]): CategoryNode[] {
  ...
  const sortNodes = (nodes: CategoryNode[]) => {
    nodes.sort((a, b) => (a.position ?? 0) - (b.position ?? 0) || a.name.localeCompare(b.name, "vi"));
...
function flattenTree(nodes: CategoryNode[], expandedIds: Set<string>): CategoryNode[] {
...
function searchTree(nodes: CategoryNode[], q: string): CategoryNode[] {
  ...
  const walk = (node: CategoryNode, ancestors: CategoryNode[]) => {
    const match = node.name.toLowerCase().includes(lq) || node.slug.toLowerCase().includes(lq);
...
function CategoryImage({ category, size = 9 }: { category: Category; size?: number }) {
  const [imgError, setImgError] = useState(false);
  const src = category.imageUrl ?? category.imagePath ?? null;
  const px = size * 4; // tailwind w-9 = 36px
  if (src && !imgError) {
    return (
      <div className="relative shrink-0 rounded-lg overflow-hidden border border-neutral/40 bg-neutral-light-active" style={{ width: px, height: px }}>
        <Image src={src} alt={category.name} fill className="object-cover" onError={() => setImgError(true)} unoptimized />
      </div>
    );
  }
  return (
    <div className="rounded-lg bg-neutral-light-active flex items-center justify-center shrink-0 border border-neutral/30" style={{ width: px, height: px }}>
      ...
      {open &&
        createPortal(
          <>
            ...
            <div className="fixed z-[101] w-44 bg-white border border-neutral rounded-xl shadow-lg py-1 max-h-72 overflow-y-auto" style={{ top: dropPos.top, left: dropPos.left }}>
...
const DEPTH_BORDER_COLOR = ["border-accent/40", "border-violet-300", "border-rose-200"];
...
function TreeRow({
  ...
  return (
    <tr className={`border-b border-neutral transition-colors duration-100 hover:bg-neutral-light-active/40 ${DEPTH_BG[node.depth] ?? DEPTH_BG[2]}`}>
      {/* STT */}
      <td className="px-4 py-3 text-[12px] text-primary/50 w-10 tabular-nums">{idx + 1}</td>
        ...
        <div className="flex items-center gap-2" style={{ paddingLeft: indent }}>
          ...
          {node.depth > 0 && (
            <span className="shrink-0 flex items-center">
              ...
              <span className={`inline-block w-3 h-px ${DEPTH_BORDER_COLOR[node.depth]} border-t`} />
            </span>
          )}
          ...
          <div className="min-w-0">
            <div className={`font-medium text-primary truncate max-w-[240px] ${node.depth === 0 ? "text-[13px]" : "text-[12px]"}`}>{node.name}</div>
            <div className="text-[10px] text-primary/50 font-mono truncate max-w-[240px]">/{node.slug}</div>
          </div>
        </div>
      </td>
      ...
      <td className="px-4 py-3 whitespace-nowrap">
        {node.depth === 0 && <span className="inline-flex px-2 py-0.5 rounded-md text-[10px] font-semibold bg-blue-50 text-blue-600 border border-blue-100">Gốc</span>}
        {node.depth === 1 && <span className="inline-flex px-2 py-0.5 rounded-md text-[10px] font-semibold bg-violet-50 text-violet-600 border border-violet-100">Cấp 2</span>}
        {node.depth === 2 && <span className="inline-flex px-2 py-0.5 rounded-md text-[10px] font-semibold bg-rose-50 text-rose-500 border border-rose-100">Cấp 3</span>}
      </td>
      ...
      {/* Vị trí */}
      <td className="px-4 py-3 text-[12px] text-primary/60 tabular-nums">{node.position ?? 0}</td>
...
export default function AdminCategoriesPage() {
  ...
  const hasActiveFilters = activeTab !== "ALL" || !!search || sortBy !== "position" || featuredFilter !== "";
  ...
  const handleInlineUpdate = useCallback((id: string, patch: Partial<Category>) => {
    setAllCategories((prev) => {
      ...
      return prev.map((c) => (affectedIds.has(c.id) ? { ...c, ...patch } : c));
    });
  }, []);
  ...
  return (
    <div className="space-y-5 p-5 bg-neutral-light min-h-full">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatsCard label="Danh mục gốc" value={loading ? "—" : rootCount} sub="Không có danh mục cha" icon={<FolderTree size={18} />} valueClassName="text-violet-600" />
      ...
      <div className="bg-neutral-light border border-neutral rounded-2xl overflow-hidden shadow-sm">
        ...
        <div className="px-5 py-3 border-b border-neutral flex items-center gap-2 flex-wrap">
          ...
          <div className="ml-auto flex items-center gap-2">
            <span className="text-[12px] text-primary/60">{loading ? "..." : `${visibleRows.length} / ${meta.statusCounts.ALL} danh mục`}</span>
        ...
        <div className="px-5 py-2.5 border-b border-neutral bg-neutral-light-active/40 flex items-center gap-2 flex-wrap">
          <SlidersHorizontal size={13} className="text-primary/50 shrink-0" />
          <span className="text-[11px] text-primary/60 font-medium mr-1">Lọc:</span>
        ...
        {hasActiveFilters && (
          <div className="px-5 py-2 border-b border-neutral flex items-center gap-2 flex-wrap bg-accent/5">
            ...
            {activeTab !== "ALL" && <Chip label={`Trạng thái: ${activeTab === "active" ? "Hoạt động" : "Ẩn"}`} onRemove={() => setActiveTab("ALL")} />}
            {search && (
              <Chip
                label={`Tìm: "${search}"`}
                onRemove={() => {
            ...
            {featuredFilter && <Chip label={featuredFilter === "true" ? "Nổi bật" : "Không nổi bật"} onRemove={() => setFeaturedFilter("")} />}
            {sortBy !== "position" && <Chip label={`Sắp xếp: ${SORT_OPTIONS.find((o) => o.value === sortBy)?.label}`} onRemove={() => setSortBy("position")} />}
          </div>
        )}
        ...
        {search && !loading && (
          <div className="px-5 py-2 bg-amber-50 border-b border-amber-100 flex items-center gap-2 text-[12px] text-amber-700">
            <Search size={12} />
            Đang hiển thị <span className="font-semibold">{visibleRows.length}</span> kết quả cho &quot;{search}&quot; — cây danh mục được mở rộng tự động.
          </div>
        )}
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-neutral-light-active border-b border-neutral">
                {["#", "Danh mục", "Cấp", "Danh mục con", "Vị trí", "Trạng thái", "Hành động"].map((col) => (
                  <th key={col} className="px-4 py-3 text-left text-[11px] font-semibold text-primary/60 uppercase tracking-wider whitespace-nowrap">
                    {col}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {loading ? (
                ...
                visibleRows.map((node, idx) => (
                  <TreeRow key={node.id} node={node} idx={idx} isExpanded={expandedIds.has(node.id)} onToggle={toggleExpand} onUpdated={handleInlineUpdate} onDelete={setDeleteTarget} />
                ))
              )}
            </tbody>
          </table>
        ...
        {!loading && visibleRows.length > 0 && (
          <div className="px-5 py-3 border-t border-neutral flex items-center justify-between">
            <p className="text-[12px] text-primary/50">
              Hiển thị <span className="font-medium text-primary">{visibleRows.length}</span> / {meta.statusCounts.ALL} danh mục
              {!search && (

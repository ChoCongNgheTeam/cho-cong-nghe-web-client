...
export default function NewVoucherPage() {
  ...
  const handleSubmit = async (form: VoucherFormData) => {
    ...
    try {
      ...
      setTimeout(() => {
        router.push(href(`/vouchers/${res.data.id}`));
      }, 1200);
    } catch (e: any) {
      ...
      <div className="px-6 py-4 flex justify-center">
        <div className="w-full">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h1 className="text-[18px] font-bold text-primary">Tạo voucher mới</h1>
              <p className="text-[12px] text-neutral-dark">Điền thông tin để tạo mã giảm giá</p>
            </div>
          </div>
          <VoucherForm initialData={DEFAULT_FORM} onSubmit={handleSubmit} saving={saving} error={error} submitLabel="Tạo voucher" onCancel={() => router.push(href(`/vouchers`))} />{" "}
        </div>
      </div>
    </div>
  );

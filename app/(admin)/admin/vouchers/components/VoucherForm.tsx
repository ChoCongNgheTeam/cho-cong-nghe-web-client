...
export function voucherToForm(v: VoucherDetail): VoucherFormData {
   return {
      ...
      maxDiscountValue:
         v.maxDiscountValue !== undefined ? String(v.maxDiscountValue) : "",
      maxUses: v.maxUses !== undefined ? String(v.maxUses) : "",
      maxUsesPerUser:
         v.maxUsesPerUser !== undefined ? String(v.maxUsesPerUser) : "",
      startDate: v.startDate
         ? new Date(v.startDate).toISOString().slice(0, 16)
         : "",
   ...
   return (
      <div className="space-y-1.5">
         <label className="text-[12px] font-semibold text-neutral-dark uppercase tracking-wider">
            {label}
            {required && (
               <span className="text-promotion normal-case font-normal ml-1">
                  *
               </span>
            )}
         </label>
...
export function VoucherForm({
   ...
   const set = (key: keyof VoucherFormData, value: any) =>
   ...
   const updateTargetType = (idx: number, targetType: TargetType) =>
      setForm((prev) => ({
         ...
         targets: prev.targets.map((t, i) =>
            i === idx ? { targetType, targetId: undefined } : t,
         ),
      }));
   ...
   const updateTargetId = (idx: number, targetId: string) =>
      setForm((prev) => ({
         ...prev,
         targets: prev.targets.map((t, i) =>
            i === idx ? { ...t, targetId: targetId || undefined } : t,
         ),
      }));
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
         ...
         <div className="bg-neutral-light border border-neutral rounded-2xl p-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
               <FormRow
                  label={
                     isPercent ? "Phần trăm giảm (%)" : "Số tiền giảm (VNĐ)"
                  }
                  required
               >
                  <div className="relative">
                     <input
                        ...
                        required
                     />
                     <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[12px] text-neutral-dark font-medium">
                        {isPercent ? "%" : "₫"}
                     </span>
                  </div>
               </FormRow>
         ...
         <div className="bg-neutral-light border border-neutral rounded-2xl p-5 space-y-4">
            <div className="flex items-center justify-between">
               ...
               <button
                  type="button"
                  onClick={addTarget}
                  className="flex items-center gap-1 px-2.5 py-1 text-[11px] text-accent border border-accent/30 rounded-lg hover:bg-accent/5 cursor-pointer"
               >
                  <Plus size={11} /> Thêm
               </button>
            </div>
               ...
               {form.targets.map((target, idx) => (
                  <div key={idx} className="flex items-center gap-2 flex-col">
                     ...
                     <select
                        ...
                        onChange={(e) =>
                           updateTargetType(idx, e.target.value as TargetType)
                        }
                        className={`${inputCls} w-40 shrink-0 cursor-pointer`}
                     >
                        {Object.entries(TARGET_TYPE_LABELS).map(
                           ([value, label]) => (
            ...
            <label className="flex items-start gap-3 cursor-pointer w-fit">
               ...
               <div>
                  ...
                  <p className="text-[11px] text-neutral-dark">
                     Voucher được hiển thị và áp dụng
                  </p>
               </div>
            </label>
         </div>

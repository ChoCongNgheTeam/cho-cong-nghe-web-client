...
export default function VoucherDetailPage() {
   ...
   const handleSave = async (form: VoucherFormData) => {
      ...
      try {
         ...
         const msg = e?.message ?? "Không thể cập nhật voucher";
         setSaveError(msg);
         toastError(msg);
      } finally {
         setSaving(false);
   ...
   const handleDelete = async () => {
      ...
      try {
         ...
         success("Đã xoá voucher thành công!");
         router.push("/admin/vouchers");
      } catch (e: any) {
         const msg = e?.message ?? "Không thể xoá voucher";
         setDeleteError(msg);
         toastError(msg);
      } finally {
         setDeleting(false);
   ...
   const usagePercent = voucher.maxUses
      ? Math.min(100, (voucher.usesCount / voucher.maxUses) * 100)
      : 0;
   ...
   return (
      <div className="min-h-screen bg-neutral-light">
         ...
         <div className="flex items-center gap-3 px-6 pt-5 pb-3">
            ...
            <span className="text-neutral-dark text-[13px]">/</span>
            <span className="text-[13px] text-primary font-bold font-mono">
               {voucher.code}
            </span>
         </div>
         ...
         <div className="px-6 py-4 grid grid-cols-1 lg:grid-cols-3 gap-6 max-w-6xl">
            ...
            <div className="lg:col-span-1 space-y-4">
               <div className="bg-neutral-light border border-neutral rounded-2xl p-5 space-y-4">
                  ...
                  <div>
                     <h2 className="text-[18px] font-bold text-primary font-mono tracking-wider">
                        {voucher.code}
                     </h2>
                     {voucher.description && (
                  ...
                  <div className="flex flex-wrap gap-2">
                     ...
                     <span
                        className={`inline-flex items-center px-2.5 py-0.5 rounded-lg text-[11px] font-semibold ${DISCOUNT_TYPE_COLORS[voucher.discountType]}`}
                     >
                        {DISCOUNT_TYPE_LABELS[voucher.discountType]}{" "}
                  ...
                  <div className="space-y-3">
                     <div className="flex items-center justify-between">
                        ...
                        <span className="text-[13px] font-bold text-primary">
                           {voucher.usesCount}
                           {voucher.maxUses && (
                              <span className="text-[11px] text-neutral-dark font-normal">
                                 {" "}
                                 / {voucher.maxUses}
                              </span>
                           )}
                     ...
                     {voucher.maxDiscountValue && (
                        <div className="flex items-center justify-between">
                           ...
                           <span className="text-[12px] font-medium text-primary">
                              {formatVND(voucher.maxDiscountValue)}
                           </span>
                        </div>
                     )}
                  ...
                  <div className="bg-neutral-light border border-neutral rounded-2xl p-5 space-y-3">
                     ...
                     <div className="flex flex-wrap gap-1.5">
                        {voucher.targets.map((t) => (
                           <span
                              key={t.id}
                              className="text-[11px] px-2.5 py-1 rounded-lg bg-neutral-light-active text-neutral-dark font-medium"
                           >
                              {TARGET_TYPE_LABELS[t.targetType] ?? t.targetType}
                              {t.targetId && (
                                 <span className="ml-1 font-mono opacity-60 text-[10px]">
               ...
               <div className="bg-neutral-light border border-neutral rounded-2xl p-6">
                  <div className="flex items-center justify-between mb-5">
                     <h3 className="text-[15px] font-bold text-primary">
                        {isEditMode ? "Chỉnh sửa voucher" : "Thông tin voucher"}
                     </h3>
                     {!isEditMode && (
                        <Link
                           href={`/admin/vouchers/${voucher.id}?edit=true`}
                  ...
                  {isEditMode ? (
                     ...
                     <div className="space-y-4 text-[13px]">
                        <div className="grid grid-cols-2 gap-4">
                           <div>
                              <p className="text-[11px] font-semibold text-neutral-dark uppercase tracking-wider mb-1">
                                 Mã voucher
                              </p>
         ...
         <Popzy
            ...
            content={
               <div className="py-2">
                  ...
                  <p className="text-[15px] font-bold text-primary text-center font-mono mb-5">
                     "{voucher.code}"
                  </p>
                  <p className="text-[12px] text-promotion text-center mb-6">
                     Voucher sẽ được chuyển vào thùng rác.
                  </p>
                  {deleteError && (

...
export default function ReviewsAdminPage() {
   ...
   const pending =
      data?.data.filter((r) => r.isApproved === "PENDING").length ?? 0;
   const approved =
      data?.data.filter((r) => r.isApproved === "APPROVED").length ?? 0;
   const rejected =
      data?.data.filter((r) => r.isApproved === "REJECTED").length ?? 0;
   const avgRating = data?.data.length
      ? (
               ...
               {data && data.data.length > 0 && (
                  <div className="px-4 py-2.5 border-b border-neutral flex items-center gap-2">
                     ...
                     <span className="text-[12px] text-primary">
                        {allSelected
                           ? "Bỏ chọn tất cả"
                           : `Chọn tất cả ${data.data.length} đánh giá trên trang`}
                     </span>
                  </div>
               )}
               <AdminTable
            ...
            <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
               <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 space-y-4">
                  ...
                  <div className="bg-neutral-light rounded-xl px-3 py-2.5 space-y-1">
                     <p className="text-[12px] text-primary">
                        {deleteTarget.user?.fullName ?? "Ẩn danh"} —{" "}
                        {deleteTarget.orderItem?.productVariant?.product
                           ?.name ?? "—"}
                     </p>
                     {deleteTarget.comment && (
                        <p className="text-[12px] text-primary line-clamp-2">
                           "{deleteTarget.comment}"
                        </p>
                     )}
                  </div>

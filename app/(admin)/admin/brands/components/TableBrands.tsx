...
export function getBrandColumns({ page, pageSize, selected, openStatusId, toggleOne, setOpenStatusId, onToggleActive, onDeleteClick }: GetBrandColumnsParams): AdminColumn<Brand>[] {
  return [
    ...
    },
    {
      ...
      label: "Trạng thái",
      render: (brand) => {
        const statusOption = brand.isActive ? { label: "Hoạt động", color: "text-emerald-600 bg-emerald-50" } : { label: "Ẩn", color: "text-orange-500 bg-orange-50" };
                ...
                {STATUS_OPTIONS.map((opt) => (
                  <button
                    ...
                    onClick={(e) => {
                      ...
                      if (opt.value !== (brand.isActive ? "active" : "hidden")) {

...
function validateCampaignForm(form: CampaignFormData): CampaignFormErrors {
  ...
  if (startDate && endDate) {
    if (endDate <= startDate) {
      ...
      if (diffMinutes < 5) {
        errors.dateRange = "Khoảng thời gian chiến dịch phải ít nhất 5 phút";
      }
    }
  }
  ...
  return (
    <div className="space-y-1.5">
      <label className="text-[12px] font-semibold text-neutral-dark uppercase tracking-wider">
        {label}
        {required && <span className="text-promotion normal-case font-normal ml-0.5">*</span>}
      </label>
      {children}
...
export function CampaignForm({ initialData, onSubmit, saving, error, submitLabel = "Lưu", onCancel }: CampaignFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      {/* Thông tin cơ bản */}
      <div className={`bg-neutral-light border rounded-2xl p-5 space-y-4 transition-colors ${fieldErrors.name || fieldErrors.type ? "border-promotion/40" : "border-neutral"}`}>
        <p className="text-[11px] font-bold text-neutral-dark uppercase tracking-widest flex items-center gap-2">
          Thông tin cơ bản
          {(fieldErrors.name || fieldErrors.type) && <AlertCircle size={12} className="text-promotion" />}
        </p>

        <FormRow label="Tên chiến dịch" required>
          <input value={form.name} onChange={(e) => set("name", e.target.value)} placeholder="Flash Sale 12.12" className={inputCls(!!fieldErrors.name)} />
          <FieldError message={fieldErrors.name} />
        </FormRow>

        <FormRow label="Loại chiến dịch" required>
          <select value={form.type} onChange={(e) => set("type", e.target.value)} className={`${inputCls(!!fieldErrors.type)} cursor-pointer`}>
      ...
      {/* Thời gian */}
      <div className={`bg-neutral-light border rounded-2xl p-5 space-y-4 transition-colors ${hasTimeErrors ? "border-promotion/40" : "border-neutral"}`}>
        <p className="text-[11px] font-bold text-neutral-dark uppercase tracking-widest flex items-center gap-2">
          Thời gian
          {hasTimeErrors && <AlertCircle size={12} className="text-promotion" />}
        </p>

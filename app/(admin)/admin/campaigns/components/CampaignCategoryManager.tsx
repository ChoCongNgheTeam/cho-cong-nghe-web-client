...
function CategoryRow({
   ...
   const isDirty =
      edit.title !== (cc.title ?? "") ||
      edit.description !== (cc.description ?? "") ||
      edit.position !== cc.position;
   ...
   const handleImageChange = async (file: File) => {
      ...
      try {
         ...
         // re-fetch handled by parent via onUpdated with imageUrl placeholder
         success("Đã cập nhật ảnh");
         onUpdated({ id: cc.id, imageUrl: URL.createObjectURL(file) });
      } catch (e: any) {
   ...
   const imageUrl = cc.imageUrl ?? cc.category?.imageUrl;
      ...
      <div
         className={`border rounded-xl transition-all duration-200 overflow-hidden ${
            expanded
               ...
               : "border-neutral hover:border-neutral-dark/30 bg-neutral-light"
         }`}
      >
         {/* ── Header row ── */}
         <div className="flex items-center gap-2 px-3 py-2.5">
            ...
            {/* position badge */}
            <span className="w-5 h-5 rounded-md bg-neutral flex items-center justify-center text-[10px] font-bold text-neutral-dark shrink-0">
               {cc.position + 1}
            </span>
            ...
            <div className="w-8 h-8 rounded-lg border border-neutral bg-neutral overflow-hidden shrink-0 flex items-center justify-center">
               {imageUrl ? (
                  ...
                  <ImageIcon size={14} className="text-neutral-dark/40" />
               )}
            </div>
            ...
            <div className="flex-1 min-w-0">
               <p className="text-[13px] font-medium text-primary truncate">
                  {cc.category?.name ?? cc.categoryId}
               </p>
               {cc.title && (
                  <p className="text-[11px] text-neutral-dark truncate">
                     {cc.title}
                  </p>
               )}
            </div>
            ...
            <div className="flex items-center gap-1 shrink-0">
               ...
               <button
                  ...
                  className="w-6 h-6 flex items-center justify-center rounded text-neutral-dark hover:bg-neutral transition-colors"
               >
                  {expanded ? (
                     <ChevronUp size={13} />
                  ) : (
               ...
               <button
                  ...
                  className={`w-6 h-6 flex items-center justify-center rounded transition-colors ${
                     confirmRemove
                        ? "bg-promotion text-white"
                        : "text-neutral-dark/50 hover:bg-promotion-light hover:text-promotion"
                  }`}
               ...
               <div className="flex items-start gap-3">
                  ...
                  >
                     {uploadingImg ? (
                        ...
                        <div className="flex flex-col items-center gap-1">
                           <ImageIcon
                              size={18}
                              className="text-neutral-dark/40"
                           />
                  ...
                  <div className="flex-1 space-y-1.5">
                     ...
                     <p className="text-[11px] text-neutral-dark/60">
                        Ảnh hiển thị cho danh mục này trong chiến dịch
                     </p>
...
function AddCategoriesPanel({
   ...
   nextPosition: number;
   onAdded: (
      added: CampaignCategory[],
      lookup: Map<string, CategoryOption>,
   ) => void;
   onClose: () => void;
}) {
   const { success, error: toastError } = useToasty();
      ...
      <div className="border border-accent/30 rounded-xl bg-accent/[0.02] overflow-hidden">
         <div className="flex items-center justify-between px-4 py-3 border-b border-neutral/60">
            <p className="text-[13px] font-semibold text-primary">
               Thêm danh mục vào chiến dịch
            </p>
            <button
               onClick={onClose}
               className="w-6 h-6 flex items-center justify-center rounded text-neutral-dark hover:bg-neutral"
            >
               <X size={13} />
            </button>
         </div>
         ...
         <div className="p-3 space-y-2.5">
            ...
            <div className="relative">
               <Search
                  size={13}
                  className="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-dark/50"
               />
               <input
                  type="text"
                  value={search}
            ...
            <div className="max-h-52 overflow-y-auto space-y-1 pr-0.5">
               {filtered.length === 0 ? (
                  <p className="text-[12px] text-neutral-dark/50 text-center py-4">
                     {available.length === 0
                        ? "Tất cả danh mục đã có trong chiến dịch"
                        ...
                        <button
                           ...
                           className={`w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-left transition-colors ${
                              checked
                                 ...
                                 : "hover:bg-neutral border border-transparent"
                           }`}
                        >
                           <div
            ...
            {selected.size > 0 && (
               <div className="flex items-center justify-between pt-1 border-t border-neutral/60">
                  <span className="text-[12px] text-neutral-dark">
                     Đã chọn{" "}
                     <strong className="text-primary">{selected.size}</strong>{" "}
                     danh mục
                  </span>
                  <button
                     onClick={handleAdd}
...
export function CampaignCategoryManager({
   campaignId,
   categories: initialCategories,
   availableCategories,
   onChanged,
}: CampaignCategoryManagerProps) {
   const [categories, setCategories] =
      useState<CampaignCategory[]>(initialCategories);
   ...
   const handleUpdated = (
      partial: Partial<CampaignCategory> & { id: string },
   ) => {
      update(
         categories.map((c) =>
   ...
   const handleAdded = (
      ...
      // BE trả về không có `category` object — enrich từ lookup
      ...
      <div className="space-y-3">
         ...
         <div className="flex items-center justify-between">
            <p className="text-[11px] font-bold text-neutral-dark uppercase tracking-widest">
               Danh mục trong chiến dịch ({categories.length})
            </p>
            {available.length > 0 && (
               <button
         ...
         {sorted.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-8 border border-dashed border-neutral rounded-xl text-center gap-2">
               ...
               <p className="text-[12px] text-neutral-dark/50">
                  Chưa có danh mục nào trong chiến dịch
               </p>
               {available.length > 0 && (
                  <button

...
export default function NotificationSettingsView() {
  ...
  const systemUnreadCount = systemNotifications.reduce((total, item) => total + (item.isRead ? 0 : 1), 0);
    ...
    <div className="space-y-6">
      <section className="rounded-2xl border border-neutral bg-neutral-light shadow-sm">
        <div className="border-b border-neutral px-5 py-4">
          <div className="flex items-center gap-2 text-accent">
            ...
            <h2 className="text-base font-semibold text-primary">Thông báo hệ thống</h2>
          </div>
          <p className="text-sm text-neutral-dark mt-1">Quản lý kênh nhận thông báo và báo cáo</p>
        </div>
        <div className="px-6 py-5 space-y-3 text-sm">
          {[
            ...
            const Icon = row.icon;
            return (
              <label key={row.label} className="flex items-center justify-between rounded-xl border border-neutral bg-neutral-light-active px-4 py-3">
      ...
      {/* <section className="rounded-2xl border border-neutral bg-neutral-light shadow-sm">
        <div className="border-b border-neutral px-5 py-4 flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2 text-accent">
            <Bell className="h-5 w-5" />
            <h2 className="text-base font-semibold text-primary">Bảng thông báo</h2>
            {systemUnreadCount > 0 && <span className="text-[11px] font-semibold text-accent bg-accent-light px-2 py-0.5 rounded-full leading-none">{systemUnreadCount} mới</span>}
          </div>
          {systemUnreadCount > 0 && (
            <button onClick={markAllAsRead} className="flex items-center gap-1.5 text-[12px] font-medium text-neutral-dark hover:text-accent transition-colors duration-150 cursor-pointer">
              <CheckCheck size={13} />
              Đọc tất cả
            </button>
          )}
          ...
          <div className="rounded-2xl border border-neutral overflow-hidden">
            {systemNotifications.length === 0 && !isLoading ? (
              <div className="py-12 text-center px-6 bg-neutral-light">
                ...
                <p className="text-[12px] text-neutral-dark">Hệ thống sẽ hiển thị khi có phát sinh mới từ người dùng hoặc hệ thống.</p>
              </div>
            ) : (
                ...
                {systemNotifications.map((item) => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-3.5 flex flex-wrap sm:flex-nowrap gap-3 items-start transition-colors ${
                      item.isRead ? "bg-neutral-light hover:bg-neutral-light-hover" : "bg-accent-light hover:bg-accent-light"
                    }`}
                  >
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-accent/10 text-accent">{item.type}</span>
                    ...
                    <div className="text-[11px] text-neutral-dark whitespace-nowrap">{formatRelativeDate(item.createdAt)}</div>
                  </button>
                ))}
              </div>
            )}

...
function ProductsSection() {
  ...
  return (
    <SectionCard
      ...
      desc="Cài đặt hiển thị sản phẩm và chức năng review"
      onSave={save}
      saving={saving}
      loading={loading}
    >
      <div className="space-y-3">
        <ToggleRow
          ...
          desc="Khách hàng có thể gửi review sau khi mua"
          value={data.enable_product_review}
          onChange={(v) => set("enable_product_review", v)}
        />
        <ToggleRow
          label="Hiển thị sao đánh giá"
          desc="Hiển thị rating sao (1-5) trên trang sản phẩm"
          value={data.enable_star_rating}
          onChange={(v) => set("enable_star_rating", v)}
        />
        <ToggleRow
          label="Bắt buộc chọn số sao"
          desc="Người dùng phải chọn sao khi viết review"
          value={data.require_star_rating}
          onChange={(v) => set("require_star_rating", v)}
        />
        <ToggleRow
          label="Hiển thị nhãn 'Đã mua hàng'"
          desc="Gắn nhãn xác nhận cho người đã mua sản phẩm"
          value={data.show_verified_label}
          onChange={(v) => set("show_verified_label", v)}
        />
        <ToggleRow
        ...
        <ToggleRow
          ...
          desc="Hiển thị nút so sánh trên listing"
          value={data.enable_product_compare}
          onChange={(v) => set("enable_product_compare", v)}
        />
        <ToggleRow
          label="Hiển thị giá giảm"
          desc="Hiển thị giá gốc và % giảm giá nếu có promotion"
          value={data.enable_product_discount}
          onChange={(v) => set("enable_product_discount", v)}
        />
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 pt-2 border-t border-neutral">
        <div>
          ...
          <input
            ...
            onChange={(e) => set("products_per_page", Number(e.target.value))}
          />
        </div>
        <div>
...
function CheckoutSection() {
  ...
  return (
    <SectionCard
      ...
      desc="Tuỳ chỉnh luồng thanh toán cho khách hàng"
      onSave={save}
      saving={saving}
      loading={loading}
    >
      ...
      <div className="space-y-3">
        ...
        />
        <ToggleRow
          label="Mặc định địa chỉ thanh toán giống giao hàng"
          value={data.billing_same_as_shipping}
          onChange={(v) => set("billing_same_as_shipping", v)}
        />
        <ToggleRow label="Bắt buộc nhập mã bưu điện" value={data.mandatory_postcode} onChange={(v) => set("mandatory_postcode", v)} />
      </div>

      <p className="text-[11px] font-semibold uppercase tracking-widest text-neutral-dark pt-1 border-t border-neutral">
        Khách không đăng nhập
      </p>
      <div className="space-y-3">
        <ToggleRow
          label="Cho phép đặt hàng không cần tài khoản"
          desc="Guest checkout — không yêu cầu đăng ký"
          value={data.enable_guest_checkout}
          onChange={(v) => set("enable_guest_checkout", v)}
        />
        <ToggleRow
      ...
      <div className="space-y-3">
        <ToggleRow
          ...
          desc="Email xác nhận đơn hàng kèm hóa đơn PDF"
          value={data.send_invoice_email}
        ...
        <ToggleRow
          label="Cho phép nhiều coupon / đơn"
          desc="Khách có thể áp dụng nhiều mã giảm giá cùng lúc"
          value={data.enable_multi_coupon}
          onChange={(v) => set("enable_multi_coupon", v)}
        />
        ...
        <ToggleRow
          label="Cho phép chọn điểm lấy hàng"
          desc="Khách chọn điểm tự lấy thay vì giao tận nơi"
          value={data.enable_pickup_point}
          onChange={(v) => set("enable_pickup_point", v)}
        />
      </div>
      ...
      <div className="space-y-3">
        <ToggleRow
          label="Bật giá trị đơn hàng tối thiểu"
          value={data.enable_min_order_amount}
          onChange={(v) => set("enable_min_order_amount", v)}
        />
...
function CustomerSection() {
  ...
  return (
    <SectionCard
      icon={Users}
      title="Khách hàng"
      desc="Kiểm soát đăng ký và xác minh tài khoản"
      onSave={save}
      saving={saving}
      loading={loading}
    >
      <div className="space-y-3">
        <ToggleRow
          ...
          desc="Tài khoản được kích hoạt ngay sau khi đăng ký"
          value={data.auto_approval}
          onChange={(v) => set("auto_approval", v)}
        />
        <ToggleRow
...
function OrderSection() {
  ...
  return (
    <SectionCard
      icon={Package}
      title="Đơn hàng"
      desc="Định dạng mã đơn, thời hạn hủy và hoàn trả"
      onSave={save}
      saving={saving}
      loading={loading}
    >
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          ...
          <input
            className={inputCls}
            value={data.order_code_prefix}
            onChange={(e) => set("order_code_prefix", e.target.value.toUpperCase())}
            maxLength={6}
            placeholder="ORD"
          />
        </div>
        <div>
          ...
          <input
            className={inputCls}
            value={data.order_code_separator}
            onChange={(e) => set("order_code_separator", e.target.value)}
            maxLength={2}
            placeholder="-"
          />
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 pt-2 border-t border-neutral">
        <div>
          ...
          <input
            ...
            min={0}
            className={inputCls}
            value={data.cancel_within_minutes}
            onChange={(e) => set("cancel_within_minutes", Number(e.target.value))}
          />
        ...
        <div>
          ...
          <input
            ...
            min={0}
            className={inputCls}
            value={data.return_within_days}
            onChange={(e) => set("return_within_days", Number(e.target.value))}
          />
        </div>
      </div>
    </SectionCard>
...
function WalletSection() {
  ...
  return (
    <SectionCard
      icon={Wallet}
      title="Ví điện tử"
      desc="Cho phép khách nạp tiền vào ví để thanh toán"
      onSave={save}
      saving={saving}
      loading={loading}
    >
      <div className="space-y-3">
        <ToggleRow
          label="Nạp ví qua cổng thanh toán online"
          desc="VNPay, MoMo, ZaloPay..."
          value={data.enable_online_recharge}
          onChange={(v) => set("enable_online_recharge", v)}
        />
        <ToggleRow
          label="Nạp ví chuyển khoản thủ công"
          desc="Admin xác nhận thủ công sau khi kiểm tra"
          value={data.enable_offline_recharge}
          onChange={(v) => set("enable_offline_recharge", v)}
        />
      </div>
      ...
      {(data.enable_online_recharge || data.enable_offline_recharge) && (
        <div className="pt-2 border-t border-neutral">
          ...
          <input
            ...
            onChange={(e) => set("min_recharge_amount", Number(e.target.value))}
          />
        </div>
      )}
    </SectionCard>
...
function InvoiceSection() {
  ...
  return (
    <SectionCard
      icon={FileText}
      title="Hóa đơn"
      desc="Thông tin in trên hóa đơn gửi cho khách"
      onSave={save}
      saving={saving}
      loading={loading}
    >
...
function TaxSection() {
  ...
  return (
    <SectionCard icon={Receipt} title="Thuế" desc="Áp dụng thuế VAT vào đơn hàng" onSave={save} saving={saving} loading={loading}>
      <ToggleRow
        label="Bật tính thuế"
        desc="Thuế sẽ được cộng vào tổng đơn hàng"
        value={data.enable_tax}

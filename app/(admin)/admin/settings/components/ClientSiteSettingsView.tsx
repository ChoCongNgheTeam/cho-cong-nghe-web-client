...
import { Settings2, Layout, Megaphone, Search, Cookie, FileText, Save, Loader2, Globe, Tag, Image as ImageIcon, ToggleLeft } from "lucide-react";
...
export default function ClientSiteSettingsView() {
  ...
  return (
    <div className="space-y-6">
      {/* ── Display / Banner ── */}
      <SectionCard icon={Layout} title="Hiển thị site" desc="Quản lý banner, popup và thông báo trên trang client" onSave={() => mockSave(setSavingDisplay, "hiển thị")} saving={savingDisplay}>
        <div className="space-y-3">
          <ToggleRow label="Hiển thị banner thông báo" desc="Thanh thông báo nằm trên cùng trang" checked={display.showBanner} onChange={(v) => setDisplay((p) => ({ ...p, showBanner: v }))} />
          {display.showBanner && (
            <div>
          ...
          <ToggleRow
            ...
            onChange={(v) => setDisplay((p) => ({ ...p, showAnnouncement: v }))}
          />
          {display.showAnnouncement && (
            <div>
              <FieldLabel hint="(để trống nếu không có)">Nội dung thông báo</FieldLabel>
              <input
      ...
      {/* ── Search ── */}
      <SectionCard icon={Search} title="Tìm kiếm" desc="Cấu hình bộ lọc, gợi ý và sắp xếp cho trang tìm kiếm client" onSave={() => mockSave(setSavingSearch, "tìm kiếm")} saving={savingSearch}>
        <div className="space-y-3">
          <ToggleRow
            label="Gợi ý tìm kiếm (Autocomplete)"
            desc="Hiện danh sách gợi ý khi người dùng gõ"
            checked={search.enableSuggest}
            onChange={(v) => setSearch((p) => ({ ...p, enableSuggest: v }))}
          />
          <ToggleRow label="Bộ lọc sản phẩm" desc="Cho phép lọc theo danh mục, giá, đánh giá" checked={search.enableFilter} onChange={(v) => setSearch((p) => ({ ...p, enableFilter: v }))} />
          <ToggleRow label="Sắp xếp kết quả" desc="Cho phép người dùng chọn thứ tự hiển thị" checked={search.enableSort} onChange={(v) => setSearch((p) => ({ ...p, enableSort: v }))} />
          <div className="grid gap-4 sm:grid-cols-2 pt-1">
            <div>
              ...
              <div className="relative">
                ...
                <svg className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-neutral-dark" viewBox="0 0 16 16" fill="none">
      ...
      {/* ── Cookie / GDPR ── */}
      <SectionCard icon={Cookie} title="Cookie & Quyền riêng tư" desc="Cấu hình banner đồng ý cookie và các tuỳ chọn theo dõi" onSave={() => mockSave(setSavingCookie, "cookie")} saving={savingCookie}>
        <div className="space-y-3">
          <ToggleRow label="Hiện banner đồng ý cookie" desc="Yêu cầu người dùng xác nhận chính sách cookie" checked={cookie.showBanner} onChange={(v) => setCookie((p) => ({ ...p, showBanner: v }))} />
          <ToggleRow
            label="Cookie phân tích (Analytics)"
            desc="Thu thập dữ liệu hành vi người dùng (Google Analytics...)"
            checked={cookie.enableAnalytics}
            onChange={(v) => setCookie((p) => ({ ...p, enableAnalytics: v }))}
          />
          <ToggleRow
            label="Cookie tiếp thị (Marketing)"
            desc="Cho phép quảng cáo cá nhân hoá (Facebook Pixel...)"
            checked={cookie.enableMarketing}
            onChange={(v) => setCookie((p) => ({ ...p, enableMarketing: v }))}
          />
          <div>
            <FieldLabel>URL trang chính sách bảo mật</FieldLabel>
            <input value={cookie.policyUrl} onChange={(e) => setCookie((p) => ({ ...p, policyUrl: e.target.value }))} placeholder="/chinh-sach-bao-mat" className={inputCls} />
          </div>
        </div>
      </SectionCard>

      {/* ── SEO / Meta ── */}
      <SectionCard icon={FileText} title="SEO & Meta" desc="Cấu hình thẻ meta, OG image và robots cho trang client" onSave={() => mockSave(setSavingSeo, "SEO")} saving={savingSeo}>
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <FieldLabel>Tên site</FieldLabel>
            <input value={seo.siteName} onChange={(e) => setSeo((p) => ({ ...p, siteName: e.target.value }))} placeholder="Tên trang web" className={inputCls} />
          </div>
          <div>
            <FieldLabel>URL gốc (Canonical)</FieldLabel>
            <input value={seo.siteUrl} onChange={(e) => setSeo((p) => ({ ...p, siteUrl: e.target.value }))} placeholder="https://example.com" className={inputCls} />
          </div>
          <div className="sm:col-span-2">
            <FieldLabel>Mô tả site (meta description)</FieldLabel>
            <textarea
              value={seo.siteDesc}
              onChange={(e) => setSeo((p) => ({ ...p, siteDesc: e.target.value }))}
          ...
          <div className="sm:col-span-2">
            ...
            <div className="flex gap-2">
              <input value={seo.ogImage} onChange={(e) => setSeo((p) => ({ ...p, ogImage: e.target.value }))} placeholder="https://example.com/og-image.jpg (1200×630px)" className={inputCls} />
              <button
                type="button"
          ...
          <div>
            ...
            <div className="relative">
              <select
                value={seo.robotsTxt}
                onChange={(e) => setSeo((p) => ({ ...p, robotsTxt: e.target.value }))}
                className="w-full rounded-xl border border-neutral bg-neutral-light px-3.5 py-2.5 text-sm text-primary appearance-none focus:outline-none focus:ring-2 focus:ring-accent/30 focus:border-accent/50 transition-colors cursor-pointer pr-9"
              >
                ...
                <option value="noindex, nofollow">noindex, nofollow (Chặn hoàn toàn)</option>
              </select>
              <svg className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-neutral-dark" viewBox="0 0 16 16" fill="none">
                <path d="M4 6l4 4 4-4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
              </svg>
          ...
          <div className="flex items-end">
            <ToggleRow label="Canonical URL tự động" desc="Tự động thêm thẻ canonical cho mỗi trang" checked={seo.canonicalEnabled} onChange={(v) => setSeo((p) => ({ ...p, canonicalEnabled: v }))} />
          </div>
        </div>
      </SectionCard>
    </div>

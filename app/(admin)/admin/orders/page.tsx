...
import { Search, Filter, CalendarDays, Download, Eye, Package, RefreshCw, Plus } from "lucide-react";
...
import { OrderStatusCell, PaymentBadge, StatsCard, TableSkeleton } from "./components";
...
export default function OrdersPage() {
  ...
  // Modal hủy đơn — được trigger từ OrderStatusCell khi user chọn "CANCELLED"
  ...
  const todayRevenue = allOrders.reduce((s, o) => s + Number(o.totalAmount), 0);

  const handleStatusChange = useCallback((orderId: string, newStatus: OrderStatus) => {
    setAllOrders((prev) => prev.map((o) => (o.id === orderId ? { ...o, orderStatus: newStatus } : o)));
  }, []);

  // OrderStatusCell gọi callback này khi user chọn CANCELLED → mở modal xác nhận
  ...
  return (
    <div className="space-y-5 p-5 bg-neutral-light h-full">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatsCard label="Doanh thu hôm nay" value={new Intl.NumberFormat("vi-VN").format(todayRevenue)} sub="Tổng giá trị các đơn đã thanh toán" />
        <StatsCard label="Đơn hàng mới" value={String(statusCounts.PENDING)} sub="Đơn đang chờ bạn xác nhận" />
        <StatsCard label="Đơn hàng thành công" value={String(statusCounts.DELIVERED)} sub="Đã hoàn tất giao đến khách hàng" />
        <StatsCard label="Đang giao hàng" value={String(statusCounts.SHIPPED)} sub="Sản phẩm đang trên đường vận chuyển" />
      </div>
            ...
            {STATUS_TABS.map((tab) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-[12px] font-medium transition-all duration-150 whitespace-nowrap cursor-pointer ${
                  activeTab === tab.value ? "bg-accent text-primary shadow-sm" : "text-primary hover:bg-neutral-light-active hover:text-primary"
                }`}
              >
                {tab.label}
            ...
            <tbody>
              {loading ? (
                ...
                paginatedOrders.map((order, idx) => {
                  ...
                  return (
                    <tr key={order.id} className="border-b border-neutral hover:bg-neutral-light-active/60 transition-colors duration-100 group">
                      ...
                      <td className="px-4 py-3.5">
                        <div className="flex flex-col gap-0.5">
                          ...
                          <span className="text-[12px] text-primary">{order.user.phone}</span>
                        </div>
                      </td>
                      <td className="px-4 py-3.5">
                      ...
                      <td className="px-4 py-3.5">
                        {/* OrderStatusCell nhận thêm onCancelRequest để mở modal confirm */}

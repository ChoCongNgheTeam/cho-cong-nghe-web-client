...
import { ArrowLeft, Package, Loader2, Pencil, Ban, MapPin, User, CreditCard, Receipt, RefreshCw, ChevronRight } from "lucide-react";
...
export default function OrderDetailPage() {
  ...
  // Chỉ cho edit khi đơn chưa giao / chưa hủy
  ...
  return (
    <div className="min-h-screen bg-neutral-light">
      ...
      <div className="px-6 py-4 max-w-5xl mx-auto space-y-5">
        {/* Header */}
        <div className="bg-neutral-light border border-neutral rounded-2xl px-5 py-4 flex items-center justify-between gap-4 shadow-sm flex-wrap">
          <div>
            ...
            <SectionCard title={`Sản phẩm (${order.orderItems?.length ?? 0})`} icon={<Package size={15} />}>
                  ...
                  return (
                    <div key={item.id} className="flex items-center gap-3 py-3 border-b border-neutral/50 last:border-0">
                      ...
                      <div className="text-right shrink-0">
                        ...
                        <p className="text-[12px] text-neutral-dark">× {item.quantity}</p>
                      </div>
                      <div className="text-right shrink-0 w-24">
                        <p className="text-[14px] font-bold text-accent">{formatVND(Number(item.unitPrice) * item.quantity)}</p>
                      </div>
                    </div>
                  );
                })}
              ...
              <div className="mt-3 pt-3 border-t border-neutral space-y-2">
                ...
                <div className="flex justify-between pt-2 border-t border-neutral">
                  ...
                  <span className="text-[17px] font-bold text-accent">{formatVND(Number(order.totalAmount))}</span>
                </div>
              </div>
            </SectionCard>

   ...
   if (error || !order)
      return (
         <div className="flex flex-col items-center justify-center h-64 gap-3">
            ...
            <button
               onClick={loadOrder}
               className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-accent text-white text-[13px] cursor-pointer"
            >
         ...
         <div className="space-y-5 px-6 py-4">
            ...
            <div className="bg-neutral-light border border-neutral rounded-2xl px-5 py-4 flex items-center justify-between gap-4 flex-wrap">
               <div>
                  ...
                  <p className="text-[12px] text-neutral-dark mt-1">
                     Đặt ngày {formatDate(order.orderDate)} · Cập nhật{" "}
            ...
            <div className="grid grid-cols-3 gap-5">
               ...
               <div className="col-span-2 space-y-5">
                  {/* Products */}
                  <SectionCard
                     title={`Sản phẩm (${order.orderItems?.length ?? 0})`}
                  ...
                  >
                     <div className="space-y-1">
                        {(order.orderItems ?? []).map((item) => {
                           const imgUrl =
                              (item as any).image ??
                              item.productVariant?.product?.img?.[0]
                                 ?.imageUrl ??
                              null;
                           const attrs =
                              item.productVariant?.variantAttributes
                                 ?.map((va) => va.attributeOption.value)
                                 .join(" / ") || "";
                           return (
                              <div
                                 key={item.id}
                              ...
                              >
                                 <div className="w-12 h-12 rounded-xl overflow-hidden border border-neutral bg-neutral-light-active shrink-0">
                                    {imgUrl ? (
                                       ...
                                       <Package
                                          size={16}
                                          className="text-neutral-dark m-4"
                                       />
                                    )}
                                 </div>
                                 <div className="min-w-0 flex-1">
                                    <Link
                                       ...
                                       className="text-[13px] font-semibold text-primary hover:text-accent line-clamp-1"
                                    >
                                 ...
                                 <div className="text-right shrink-0 w-24">
                                    <p className="font-bold text-promotion">
                                       {formatVND(
                     ...
                     <div className="mt-3 pt-3 border-t border-neutral space-y-2">
                        ...
                        {Number(order.voucherDiscount) > 0 && (
                           <div className="flex justify-between text-[13px]">
                              ...
                              <span className="text-emerald-600 font-medium">
                                 - {formatVND(Number(order.voucherDiscount))}
                              </span>
                           </div>
                        )}
                        <div className="flex justify-between pt-2 border-t border-neutral">
                           ...
                           <span className="text-[17px] font-bold text-promotion">
               ...
               <div className="flex flex-col gap-3 pt-1">
                  ...
                  <div>
                     ...
                     <p className="text-[13px] text-neutral-dark leading-relaxed">
                        Đơn hàng sẽ bị hủy và tồn kho sẽ được hoàn lại. Hành
                        động này không thể hoàn tác.
                     </p>
                  </div>
                  {cancelError && (
                     <p className="text-[12px] text-promotion px-3 py-2 rounded-lg bg-promotion-light">
                        {cancelError}
                     </p>
                  )}
               </div>
            ...
            footerButtons={[
               {
                  title: "Không, giữ lại",
                  className:
                     "px-4 py-2 rounded-lg border border-neutral text-[13px] text-primary hover:bg-neutral-light-active cursor-pointer",
                  onClick: () => setShowCancelModal(false),
               },
               {
                  title: cancelling ? "Đang hủy..." : "Xác nhận hủy",
                  className:
                     "px-4 py-2 rounded-lg bg-promotion text-white text-[13px] font-medium hover:opacity-90 cursor-pointer disabled:opacity-50",

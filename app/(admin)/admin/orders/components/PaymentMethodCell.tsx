...
export function PaymentMethodCell({
   ...
   const isCOD =
      currentMethod.name?.toUpperCase().includes("COD") ||
      methods.find((m) => m.id === currentMethod.id)?.code === "COD";
   ...
   if (!canChange) {
      return (
         <span className="text-[12px] text-primary">{currentMethod.name}</span>
      );
   }
         ...
         {open &&
            createPortal(
               <div
                  ...
                  className="fixed w-52 bg-neutral-light border border-neutral rounded-xl shadow-lg z-[9999] overflow-hidden"
               >
                  {/* Header */}
                  <div className="px-3 py-2 border-b border-neutral bg-neutral-light-active">
                     ...
                     <p className="text-[11px] text-primary/50 mt-0.5">
                        Dùng khi thanh toán online lỗi
                     </p>
                  </div>
                  ...
                  {methods.length === 0 ? (
                     <div className="px-3 py-3 flex items-center gap-2 text-[12px] text-primary/60">
                        <Loader2 size={12} className="animate-spin" /> Đang
                        tải...
                     </div>
                  ) : codMethods.length === 0 ? (
                     <div className="px-3 py-3 text-[12px] text-primary/50 text-center">
                        COD không khả dụng
                     </div>
                  ) : (
                     codMethods.map((m) => (

...
export function getPromotionColumns({
   ...
   onToggleActive,
   onDeleteClick,
}: GetPromotionColumnsParams): AdminColumn<Promotion>[] {
   return [
      // ── Checkbox ──────────────────────────────────────────────────────────────
      {
         key: "_select",
         ...
         render: (promotion) => (
            <div className="space-y-0.5">
               ...
               {promotion.description && (
                  <span className="text-[11px] text-neutral-dark line-clamp-1 block max-w-xs">
                     {promotion.description}
                  </span>
               )}
            </div>
                  ...
                  promotion.rules.slice(0, 2).map((rule) => (
                     <span
                        ...
                        className={`inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-medium ${
                           ACTION_TYPE_COLORS[rule.actionType] ??
                     ...
                     >
                        ...
                        {rule.discountValue !== undefined &&
                           (rule.actionType === "DISCOUNT_PERCENT"
                              ? ` ${rule.discountValue}%`
                              : ` ${formatVND(rule.discountValue)}`)}
                     </span>
                  ))
               )}
               {promotion.rules.length > 2 && (
      ...
      // ── Áp dụng cho ───────────────────────────────────────────────────────────
      {
         ...
         label: "Áp dụng cho",
         render: (promotion) => {
            const hasAll = promotion.targets.some(
               (t) => t.targetType === "ALL",
            );
            if (hasAll) {
               return (
            ...
            const uniqueTypes = [
               ...new Set(promotion.targets.map((t) => t.targetType)),
            ];
            return (
               <div className="flex flex-wrap gap-1">
                  {uniqueTypes.map((type) => (
                     <span
                        key={type}
                        className="text-[11px] px-2 py-0.5 rounded-md bg-neutral-light-active text-neutral-dark"
                     >
                        {TARGET_TYPE_LABELS[type] ?? type}{" "}
                        <span className="font-semibold text-primary">
                           (
                           {
                              promotion.targets.filter(
                                 (t) => t.targetType === type,
                              ).length
                           }
                           )
            ...
            <div className="space-y-0.5">
               <div className="text-[11px] text-neutral-dark">
                  {promotion.startDate ? (
                     ...
                     <span className="italic">Không giới hạn bắt đầu</span>
                  )}
               </div>
               <div className="text-[11px] text-neutral-dark">
                  {promotion.endDate ? (
                     ...
                     <span className="italic">Không giới hạn kết thúc</span>
                  )}
               </div>
            </div>
         ),
               ...
               <div className="relative inline-block">
                  <button
                     onClick={(e) => {
                        e.stopPropagation();
                        if (canToggle)
                           setOpenStatusId(isOpen ? null : promotion.id);
                     }}
                     disabled={!canToggle}
                     className={`flex items-center gap-1.5 px-3 py-1 rounded-lg text-[12px] font-medium transition-colors ${status.color} ${
                        canToggle
                           ? "cursor-pointer"
                     ...
                     <div className="absolute z-20 left-0 top-full mt-1 w-44 bg-neutral-light border border-neutral rounded-xl shadow-lg overflow-hidden">
                        {STATUS_DROPDOWN.map((opt) => (
                           <button
                              ...
                              onClick={(e) => {
                                 e.stopPropagation();
                                 const currentValue = promotion.isActive
                                    ? "active"
                                    : "inactive";

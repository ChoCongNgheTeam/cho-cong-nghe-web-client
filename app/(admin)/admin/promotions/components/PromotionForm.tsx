...
const inputCls = (hasError?: boolean) =>
  `w-full px-3 py-2 text-[13px] bg-neutral-light border rounded-xl text-primary placeholder:text-neutral-dark/60 focus:outline-none focus:ring-2 transition-all ${
    hasError ? "border-promotion/60 focus:ring-promotion/20 focus:border-promotion" : "border-neutral focus:ring-accent/30 focus:border-accent"
  }`;
...
function validateForm(form: PromotionFormData): FormErrors {
  ...
  if (startDate && endDate) {
    if (endDate <= startDate) {
      ...
      if (diffMinutes < 5) {
        errors.dateRange = "Khoảng thời gian khuyến mãi phải ít nhất 5 phút";
      }
    }
  }
  ...
  if (form.minOrderValue !== "") {
    ...
    if (isNaN(val) || val < 0) {
      ...
      errors.minOrderValue = "Giá trị đơn tối thiểu không được vượt quá 100,000,000đ";
    }
  }
  ...
  if (form.maxDiscountValue !== "") {
    ...
    if (isNaN(val) || val < 0) {
      ...
      errors.maxDiscountValue = "Giảm tối đa không được vượt quá 100,000,000đ";
    }
  }
  ...
  // Rules
  const ruleErrors: Record<number, Partial<Record<keyof RuleForm, string>>> = {};
  form.rules.forEach((rule, i) => {
    ...
    if (rule.actionType === "DISCOUNT_PERCENT") {
      if (!rule.discountValue) {
        ...
        else if (v > 100) re.discountValue = "% giảm không được vượt quá 100%";
      }
    }
    ...
    if (rule.actionType === "DISCOUNT_FIXED") {
      if (!rule.discountValue) {
        ...
        const v = Number(rule.discountValue);
        if (isNaN(v) || v <= 0) re.discountValue = "Số tiền giảm phải lớn hơn 0";
        else if (v > 100_000_000) re.discountValue = "Số tiền giảm không được vượt quá 100,000,000đ";
      }
    }
...
function buildToastMessages(errors: FormErrors): string[] {
  ...
  if (errors.rules) {
    Object.entries(errors.rules).forEach(([i, re]) => {
      Object.values(re).forEach((msg) => msgs.push(`Rule #${Number(i) + 1}: ${msg}`));
    });
  }
  if (errors.targets) {
    Object.entries(errors.targets).forEach(([i, msg]) => msgs.push(`Target #${Number(i) + 1}: ${msg}`));
  }
  return msgs;
}
...
function RuleCard({
  ...
  return (
    <div className={`border rounded-xl p-4 space-y-3 bg-neutral-light transition-colors ${hasRuleError ? "border-promotion/40" : "border-neutral"}`}>
      ...
      {showBuyXGetY && (
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input type="number" min={1} value={rule.buyQuantity} onChange={(e) => onChange(index, "buyQuantity", e.target.value)} placeholder="VD: 2" className={inputCls(!!errors?.buyQuantity)} />
            <FieldError message={errors?.buyQuantity} />
          </div>
          <div>
            <FieldLabel required>Tặng (Y)</FieldLabel>
            <input type="number" min={1} value={rule.getQuantity} onChange={(e) => onChange(index, "getQuantity", e.target.value)} placeholder="VD: 1" className={inputCls(!!errors?.getQuantity)} />
      ...
      {showGift && (
        <div>
          ...
          <input
            ...
            onChange={(e) => onChange(index, "giftProductVariantId", e.target.value)}
...
function TargetCard({
  ...
  return (
    <div className={`border rounded-xl p-4 space-y-3 bg-neutral-light transition-colors ${error ? "border-promotion/40" : "border-neutral"}`}>
      <div className="flex items-center justify-between">
...
// ── Section ────────────────────────────────────────────────────────────────────
function Section({ title, children, collapsible = false, hasError = false }: { title: string; children: React.ReactNode; collapsible?: boolean; hasError?: boolean }) {
  const [open, setOpen] = useState(true);
  return (
    <div className={`bg-neutral-light border rounded-xl overflow-hidden transition-colors ${hasError ? "border-promotion/40" : "border-neutral"}`}>
      <button
        type="button"
      ...
      >
        <span className="text-[12px] font-semibold text-neutral-dark uppercase tracking-wider flex items-center gap-2">
          {title}
          {hasError && <AlertCircle size={12} className="text-promotion" />}
        </span>
...
export function PromotionForm({ initialData = DEFAULT_FORM, onSubmit, saving, error, submitLabel, onCancel, searchAPIs }: PromotionFormProps) {
  ...
  const hasTimeErrors = !!(fieldErrors.startDate || fieldErrors.endDate || fieldErrors.dateRange || fieldErrors.minOrderValue || fieldErrors.maxDiscountValue || fieldErrors.usageLimit);
  const hasRuleErrors = !!(fieldErrors.rules && Object.keys(fieldErrors.rules).length > 0);
  const hasTargetErrors = !!(fieldErrors.targets && Object.keys(fieldErrors.targets).length > 0);

  return (
    <div className="space-y-4">
      {/* ── Thông tin cơ bản ── */}
      <Section title="Thông tin cơ bản" hasError={!!(fieldErrors.name || fieldErrors.priority)}>
        <div className="space-y-4">
          <div>
            <FieldLabel required>Tên khuyến mãi</FieldLabel>
            <input value={form.name} onChange={(e) => set("name", e.target.value)} placeholder="VD: Giảm 20% mùa hè" className={inputCls(!!fieldErrors.name)} />
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="number" value={form.priority} onChange={(e) => set("priority", e.target.value)} placeholder="0 = cao nhất" className={inputCls(!!fieldErrors.priority)} />
      ...
      {/* ── Thời gian & Giới hạn ── */}
      <Section title="Thời gian & Giới hạn" collapsible hasError={hasTimeErrors}>
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="datetime-local" value={form.startDate} onChange={(e) => set("startDate", e.target.value)} className={inputCls(!!fieldErrors.startDate)} />
              <FieldError message={fieldErrors.startDate} />
            </div>
            <div>
              <FieldLabel>Ngày kết thúc (giờ VN)</FieldLabel>
              <input type="datetime-local" value={form.endDate} onChange={(e) => set("endDate", e.target.value)} className={inputCls(!!fieldErrors.endDate)} />
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => set("maxDiscountValue", e.target.value)}
                placeholder="Để trống = không giới hạn"
      ...
      {/* ── Rules ── */}
      <Section title={`Quy tắc ưu đãi (${form.rules.length})`} hasError={hasRuleErrors}>
        <div className="space-y-3">
          {form.rules.map((rule, i) => (
            <RuleCard key={i} rule={rule} index={i} onChange={updateRule} onRemove={removeRule} canRemove={form.rules.length > 1} errors={fieldErrors.rules?.[i]} />
          ))}
          <button
            type="button"
            onClick={addRule}
      ...
      {/* ── Targets ── */}
      <Section title={`Đối tượng áp dụng (${form.targets.length})`} hasError={hasTargetErrors}>
        <div className="space-y-3">

      ...
      <div className="px-6 py-4 flex justify-center">
        <div className="w-full flex gap-6 items-start">
          ...
          <div className="flex-1 min-w-0 space-y-4">
            ...
            {/* Edit mode */}
            {editMode && editForm ? (
              <PromotionForm initialData={editForm} onSubmit={handleSave} saving={saving} error={saveError} submitLabel="Lưu thay đổi" onCancel={handleCancelEdit} searchAPIs={searchAPIs} />
            ) : (
              <>
                {/* Rules */}
                    ...
                    {promotion.targets.length === 0 ? (
                      ...
                      promotion.targets.map((target) => (
                        <div key={target.id} className="flex items-center gap-3 p-3 rounded-xl border border-neutral">
                          ...
                          {target.targetId && <span className="text-[13px] text-neutral-dark truncate">{target.targetName ?? target.targetId}</span>}
                        </div>
                      ))
                    )}
                  </div>

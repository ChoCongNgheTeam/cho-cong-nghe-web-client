...
// "ALL" là tab đặc biệt để bỏ chọn hết filter, không phải PromotionStatus thật
...
export default function PromotionsPage() {
   ...
   // ── Filters ───────────────────────────────────────────────────────────────
   // Multi-select status: Set rỗng = không lọc (hiện tất cả)
   ...
   // ── Modals ────────────────────────────────────────────────────────────────
   // 1. Modal xóa đơn (promotion không active)
   const deleteModal = usePopzy();
   const [deleteTarget, setDeleteTarget] = useState<Promotion | null>(null);
   const [deleting, setDeleting] = useState(false);
   const [deleteError, setDeleteError] = useState<string | null>(null);

   // 2. Modal cảnh báo khi cố xóa promotion đang active
   ...
   const fetchPromotions = useCallback(async () => {
      ...
      setError(null);
      try {
         // Set rỗng → không gửi status (BE trả ALL)
         // Set có phần tử → join bằng dấu phẩy → tuỳ chỉnh theo API thực tế
         const statusParam =
   ...
   // Toggle status tab (multi-select)
   // Bấm "ALL" → xóa hết; bấm tab khác → toggle trong Set
   ...
   const handleToggleActive = useCallback(
      async (promotion: Promotion) => {
         try {
            const res = await updatePromotion(promotion.id, {
               isActive: !promotion.isActive,
            });
            setPromotions((prev) =>
               prev.map((p) => (p.id === promotion.id ? res.data : p)),
            );
            fetchPromotions();
         } catch (e: any) {
   ...
   // ── Delete (single) ───────────────────────────────────────────────────────
   // Nếu promotion đang active → hiện modal cảnh báo trước
   // Nếu không → mở modal xác nhận xóa thẳng
   const handleDeleteClick = useCallback(
      (promotion: Promotion) => {
         setDeleteError(null);
         const isCurrentlyActive = promotion.isActive && !promotion.isExpired;
   ...
   // Từ modal cảnh báo → người dùng vẫn muốn xóa → chuyển sang modal xác nhận
   const handleProceedDeleteActive = useCallback(() => {
      ...
      if (activeDeleteTarget) {
         ...
         // Delay nhỏ để tránh animation conflict giữa 2 modal
   ...
   const handleBulkDeactivate = useCallback(async () => {
      try {
         const targets = promotions.filter(
            (p) => selected.has(p.id) && p.isActive,
         );
         await Promise.all(
            targets.map((p) => updatePromotion(p.id, { isActive: false })),
         );
         setSelected(new Set());
         fetchPromotions();
      } catch (e: any) {
   ...
   // Số item đã chọn đang active (hiển thị cảnh báo trong bulk delete)
   const selectedActiveCount = [...selected].filter(
      (id) => promotions.find((p) => p.id === id)?.isActive,
   ).length;
   ...
   return (
      <div className="min-h-screen bg-neutral-light">
         ...
         <div className="px-6 pb-5 grid grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <StatsCard
               label="Sắp diễn ra"
               value={meta.statusCounts.upcoming}
               sub="Chưa đến ngày bắt đầu"
               icon={<Clock size={18} />}
         ...
         <div className="mx-6 bg-neutral-light border border-neutral rounded-2xl overflow-hidden shadow-sm mb-8">
            ...
            <div className="px-5 py-3 border-b border-neutral flex items-center gap-2 flex-wrap">
               ...
               {STATUS_TABS.map((tab) => {
                  ...
                  // Tab "ALL" sáng khi Set rỗng; tab khác sáng khi có trong Set
                  const isActive = isAllTab
                     ? selectedStatuses.size === 0
                     : selectedStatuses.has(tab.value as PromotionStatus);
                  ...
                  return (
                     ...
                     >
                        ...
                        >
                           {meta.statusCounts[
                              tab.value as keyof typeof meta.statusCounts
                           ] ?? 0}
                        </span>
                     </button>
                  );
               ...
               <span className="ml-auto text-[12px] text-primary">
                  {meta.total} khuyến mãi
               </span>
            </div>

            {/* ── Bulk action bar — chỉ hiện khi đã chọn ít nhất 1 item ── */}
            {selected.size > 0 && (
               <div className="flex items-center gap-3 px-5 py-2.5 bg-accent/5 border-b border-accent/20 flex-wrap">
                  <span className="text-[12px] text-accent font-medium">
                     Đã chọn {selected.size} khuyến mãi
                  </span>

                  {/* Vô hiệu hóa những item đang active trong selection */}
                  {selectedActiveCount > 0 && (
                     <button
                        onClick={handleBulkDeactivate}
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-orange-200 bg-orange-50 text-orange-600 text-[12px] font-medium hover:bg-orange-100 transition-colors cursor-pointer"
                     >
                        <EyeOff size={13} />
                        Vô hiệu hóa ({selectedActiveCount})
                     </button>
                  )}
                  ...
                  <button
                     ...
                     className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-red-200 bg-red-50 text-promotion text-[12px] font-medium hover:bg-red-100 transition-colors cursor-pointer"
                  >
                     <Trash2 size={13} />
                  ...
                  <button
                     ...
                     className="text-[12px] text-primary hover:text-primary cursor-pointer ml-auto"
                  >
                     Bỏ chọn
                  </button>
               </div>
      ...
      ════════════════════════════════════════════════════════════════════════ */}
         ...
         {/* Modal 1 — Cảnh báo khi xóa promotion đang ACTIVE */}
         <Popzy
            ...
            content={
               <div className="py-2">
                  ...
                  <p className="text-[14px] font-semibold text-primary text-center mb-3">
                     "{activeDeleteTarget?.name}"
                  </p>
                  <p className="text-[13px] text-primary/60 text-center mb-6">
                     đang được áp dụng cho khách hàng. Bạn có chắc chắn muốn xóa
                     không?
                  </p>
                  <div className="flex gap-2">
                     <button

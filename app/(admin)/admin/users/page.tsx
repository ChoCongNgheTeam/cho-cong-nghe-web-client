      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-neutral bg-neutral-light shrink-0">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[11px] text-primary/50 truncate">{user.email}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-5 py-3 border-b border-neutral bg-neutral-light-active/40 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[12px] font-semibold text-primary">Lịch sử đơn hàng</span>
          </div>
          {!loading && !error && (
            <div className="flex items-center gap-2">
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            <div className="flex flex-col items-center justify-center h-48 gap-3">
              ...
              <span className="text-[12px] text-primary/50">Đang tải đơn hàng...</span>
            </div>
          ) : error ? (
            ...
            <div className="flex flex-col items-center justify-center h-48 gap-2 px-6 text-center">
              ...
              <p className="text-[13px] text-primary/60">Người dùng chưa có đơn hàng nào</p>
            </div>
          ) : (
            <div className="divide-y divide-neutral">
              {orders.map((order) => {
                ...
                return (
                  <div key={order.id} className="px-5 py-4 hover:bg-neutral-light-active/50 transition-colors group">
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-1.5 mb-1.5">
                          <span className="text-[12px] font-semibold text-primary font-mono">#{order.orderCode}</span>
                          <button
                        ...
                        <div className="flex items-center gap-2 mb-2">
                          <span className="text-[11px] text-primary/50">{formatOrderDate(order.orderDate)}</span>
                          {order.itemCount !== undefined && (
                            <>
                              <span className="text-primary/30">·</span>
                              <span className="text-[11px] text-primary/50">{order.itemCount} sản phẩm</span>
                            </>
                          )}
                        </div>
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <OrderStatusBadge status={order.orderStatus} />
                          {paymentCfg && <span className={`text-[10px] font-medium ${paymentCfg.className}`}>{paymentCfg.label}</span>}
                        </div>
                      </div>
                      <div className="flex flex-col items-end gap-1 shrink-0">
                        <span className="text-[13px] font-semibold text-primary">{formatCurrency(order.totalAmount)}</span>
                        <button
                          onClick={() => router.push(`/admin/orders/${order.id}`)}
...
function UserOrderSidebar({ user, onClose }: { user: User | null; onClose: () => void }) {
  if (!user) return null;
...
export default function UserPage() {
  ...
  const hasFilter = activeTab !== "ALL" || !!search || !!dateFilter || isClientSort;
  ...
  const columns: AdminColumn<UserWithStats>[] = [
      ...
      ? [
          {
            ...
            render: (row: UserWithStats) => (
              <div className="flex flex-col items-center gap-0.5">
                <span className="text-[13px] font-semibold text-primary">{row.orderCount ?? "—"}</span>
                {row.totalSpent !== undefined && <span className="text-[10px] text-neutral-dark">{formatCurrency(row.totalSpent)}</span>}
              </div>
            ),
          },
        ]
    ...
    },
    {
      ...
      render: (row) => {
        ...
        if (rowRole === "ADMIN") return <span className="text-[11px] text-neutral-dark italic">—</span>;
        return (
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          ...
          <ExportButton
            ...
            label="Export"
            disabled={loading}
            onSuccess={(_count, fmt) => success(`Đã export người dùng dạng ${fmt.toUpperCase()}`)}
      ...
      <div className="bg-neutral-light border border-neutral rounded-xl">
        ...
        {hasFilter && (
          <div className="flex items-center justify-between px-3 sm:px-4 py-1.5 border-b border-neutral bg-neutral-light-active/40 flex-wrap gap-2">
            <div className="flex items-center gap-3">
              <button onClick={handleClearFilters} className="text-[11px] text-accent hover:underline cursor-pointer font-medium">
                Xóa bộ lọc
              </button>
              {isClientSort && (
                <span className="text-[11px] text-amber-600 flex items-center gap-1">
                  <TrendingUp size={11} />
                  {sortField === "orderCount" ? "Đang xếp theo số đơn hàng" : "Đang xếp theo tổng chi tiêu"}
                </span>
              )}
            </div>
            <div className="flex items-center gap-3 text-[10px] text-neutral-dark">
              <span className="flex items-center gap-1">
                <LogIn size={10} className="text-emerald-600" /> Đang online
              </span>
              <span className="flex items-center gap-1">
                <ShoppingCart size={10} className="text-amber-500" /> Đang đặt hàng
              </span>
            </div>
          </div>
        )}

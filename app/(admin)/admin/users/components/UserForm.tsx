...
interface Props { editingUser?: User | null; }

const defaultForm: UserFormState = {
  userName: "", email: "", fullName: "", phone: "",
  gender: "MALE", role: "CUSTOMER", isActive: true,
  avatarImage: null, password: "",
};
    ...
    <div className="flex flex-col gap-1.5">
      ...
      {error && (
        <p className="text-[11px] text-promotion flex items-center gap-1">
          <span className="w-1 h-1 rounded-full bg-promotion inline-block" />{error}
        </p>
      )}
    </div>
  );
...
export default function UserForm({ editingUser }: Props) {
  ...
  useEffect(() => {
    if (editingUser) {
      ...
      setForm({
        ...
        gender, role: editingUser.role ?? "CUSTOMER",
  ...
  const validate = (): boolean => {
    ...
    // Password: bắt buộc khi tạo mới; tuỳ chọn khi edit (để trống = không đổi)
          ...
          <section>
            ...
            <div className="flex items-center gap-6">
              <div className="relative shrink-0">
                ...
                <label htmlFor="avatarUpload" className="absolute -bottom-1.5 -right-1.5 w-7 h-7 bg-accent rounded-xl flex items-center justify-center cursor-pointer hover:bg-accent-hover transition-colors shadow-sm">
              ...
              <div className="flex flex-col gap-2">
                <label htmlFor="avatarUpload" className="inline-flex items-center gap-2 px-4 py-2 text-[12px] font-medium text-accent border border-accent rounded-xl hover:bg-accent-light transition-colors cursor-pointer w-fit">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                {errors.phone && !phoneInputWarning && (
                  <p className="text-[11px] text-promotion flex items-center gap-1">
                    <span className="w-1 h-1 rounded-full bg-promotion inline-block" />{errors.phone}
                  </p>
                )}
              </div>
            </div>
  ...
  <>
    ...
    <div className="border-t border-neutral" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-5">
              ...
              <Field label="Trạng thái tài khoản">
                <div onClick={() => setField("isActive", !form.isActive)}
                  ...
                  <div className={["flex items-center rounded-full p-0.5 transition-colors shrink-0",
                    form.isActive ? "bg-accent" : "bg-neutral-active"].join(" ")}
                    style={{ width: 40, height: 22 }}>
                    <div className={["w-4 h-4 bg-white rounded-full shadow-sm transition-transform duration-200",
                      form.isActive ? "translate-x-4" : "translate-x-0"].join(" ")} />
                  </div>
                </div>
              </Field>
            </div>
        ...
        <div className="px-7 py-4 border-t border-neutral bg-neutral-light-active/60 flex items-center justify-between gap-3">
          <p className="text-[11px] text-neutral-dark"><span className="text-promotion">*</span> Các trường bắt buộc</p>

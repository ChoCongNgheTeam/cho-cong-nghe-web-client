...
import { X, Bell, CheckCheck, Loader2, Gift, Clock, Package, Megaphone, Tag, UserX, MessageSquare, Star, Inbox, ChevronRight } from "lucide-react";
...
const TYPE_CONFIG: Record<string, { icon: React.ElementType; color: string; bg: string }> = {
  ...
  VOUCHER_EXPIRING: { icon: Clock, color: "text-star", bg: "bg-neutral-light-active" },
  VOUCHER_ASSIGNED: { icon: Tag, color: "text-accent-dark", bg: "bg-accent/10" },
  CAMPAIGN_PROMOTION: { icon: Megaphone, color: "text-promotion", bg: "bg-promotion/10" },
  ORDER_STATUS: { icon: Package, color: "text-accent", bg: "bg-accent/10" },
  USER_INACTIVE: { icon: UserX, color: "text-neutral-dark", bg: "bg-neutral-light-active" },
  COMMENT_NEW: { icon: MessageSquare, color: "text-accent", bg: "bg-accent/10" },
  REVIEW_NEW: { icon: Star, color: "text-star", bg: "bg-neutral-light-active" },
};
const DEFAULT_CFG = { icon: Bell, color: "text-neutral-dark", bg: "bg-neutral-light-active" };
...
export default function MobileNotificationSheet({ isOpen, onClose }: MobileNotificationSheetProps) {
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-neutral shrink-0">
          <div className="flex items-center gap-2">
            <span className="text-base font-semibold text-primary">Thông báo</span>
            {unreadCount > 0 && <span className="text-[11px] font-semibold text-accent bg-accent/10 px-2 py-0.5 rounded-full">{unreadCount} mới</span>}
          </div>
          <div className="flex items-center gap-2">
            {unreadCount > 0 && (
              <button
                onClick={markAllAsRead}
                className="flex items-center gap-1 text-xs font-medium text-neutral-dark
        ...
        <div className="flex-1 overflow-y-auto overscroll-contain">
          {notifications.length === 0 && !isLoading ? (
            <div className="py-16 text-center px-6">
              ...
              <p className="text-sm font-medium text-primary mb-1">Chưa có thông báo</p>
              <p className="text-xs text-neutral-dark">Chúng tôi sẽ thông báo khi có gì mới</p>
            </div>
          ) : (
            <>
              {notifications.map((n) => {
                ...
                return (
                  <div
                    ...
                    className={[
                      "relative flex gap-3 px-4 py-3.5 border-b border-neutral/60 last:border-0",
                  ...
                  >
                    ...
                    <div className={`shrink-0 w-9 h-9 rounded-xl ${cfg.bg} flex items-center justify-center`}>
                      <IconComp className={`w-4 h-4 ${cfg.color}`} strokeWidth={1.8} />
                    </div>
                    <div className="flex-1 min-w-0 pr-1">
                      <p className={`text-[13px] leading-snug line-clamp-2 ${!n.isRead ? "font-semibold" : "font-medium"} text-primary`}>{n.title}</p>
                      <p className="text-xs text-neutral-dark mt-0.5 line-clamp-2 leading-relaxed">{n.body}</p>
                      <p className="text-[11px] text-neutral-darker mt-1">{formatRelativeDate(n.createdAt)}</p>
                    </div>
              ...
              <div ref={sentinelRef} className="py-3 flex justify-center">
                {isLoading && <Loader2 className="w-4 h-4 animate-spin text-neutral-dark" />}
              </div>
            </>
          )}
        </div>
        ...
        <div className="border-t border-neutral bg-neutral-light-active shrink-0">
          <Link
            ...
            onClick={onClose}
            className="flex items-center justify-center gap-1.5 py-3.5 text-[13px] font-medium
              text-neutral-dark hover:text-accent transition-colors"
          >

...
import { DEFAULT_NOTIFICATION_ICON, NOTIFICATION_ICONS } from "../_libs/constants";
...
export default function MobileNotificationSheet({ isOpen, onClose }: MobileNotificationSheetProps) {
  ...
  const handleItemClick = useCallback(
    ...
    [markAsRead, navigateToOrders, navigateToComment, navigateToReview, onClose],
  );
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-neutral shrink-0">
          ...
          <div className="flex items-center gap-2">
            {unreadCount > 0 && (
              <button onClick={markAllAsRead} className="flex items-center gap-1 text-xs font-medium text-neutral-dark hover:text-accent transition-colors cursor-pointer">
        ...
        <div className="flex-1 overflow-y-auto overscroll-contain">
          {notifications.length === 0 && !isLoading ? (
            ...
            <>
              {notifications.map((n) => {
                const cfg = NOTIFICATION_ICONS[n.type] ?? DEFAULT_NOTIFICATION_ICON;

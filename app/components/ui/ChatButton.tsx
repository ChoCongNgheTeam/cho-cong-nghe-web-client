...
export default function ChatButton() {
   ...
   return (
      <div className="fixed bottom-20 right-6 z-[9998] flex flex-col items-end gap-3">
         {/* Chatbox placeholder */}
         <div
            className={`w-72 rounded-2xl border border-neutral bg-neutral-light shadow-xl overflow-hidden transition-all duration-300 origin-bottom-right ${
               open
         ...
         >
            ...
            <div
               className={`transition-all duration-200 ${open ? "rotate-90 opacity-0 absolute" : "rotate-0 opacity-100"}`}
            >
               <MessageCircle size={20} strokeWidth={2} />
            </div>
            <div
               className={`transition-all duration-200 ${open ? "rotate-0 opacity-100" : "-rotate-90 opacity-0 absolute"}`}
            >

    ...
    <div className={`flex items-end gap-1.5 ${isUser ? "flex-row-reverse" : "flex-row"} ${isNew ? "chat-msg-enter" : ""}`}>
      ...
      <div
        className={`max-w-[82%] px-3 py-2 rounded-2xl text-[12.5px] leading-relaxed chat-bubble ${
          isUser ? "bg-accent text-white rounded-br-sm" : "bg-white text-neutral-dark border border-neutral rounded-bl-sm shadow-sm prose-chat"
        }`}
        {...(!isUser && {
...
=======
      ...
      setLoading(true);
>>>>>>> c80227baacc98af1b5829eac6df0a34a067bbd16
      ...
      try {
        ...
        const assistantMsg: Message = {
          role: "assistant",
          content: (res as any)?.data?.content ?? "Xin lỗi, có lỗi xảy ra.",
        };
  ...
  return (
    <>
      ...
      <div ref={ref} className="flex fixed bottom-32 lg:bottom-20 right-4 sm:right-6 z-50 flex-col items-end gap-3">
        {/* Chat panel */}
        {open && (
          <div className="chat-panel-enter rounded-2xl border border-neutral bg-neutral-light shadow-2xl overflow-hidden w-80 flex flex-col">
            ...
            <div className="bg-accent px-4 py-3 flex items-center justify-between shrink-0">
              ...
              <div className="flex items-center gap-1.5">
                {messages.length > 0 && (
                  <button onClick={clearHistory} title="Xóa lịch sử" className="text-white/60 hover:text-white transition-colors cursor-pointer p-1.5 rounded-md hover:bg-white/10">
                    <RotateCcw size={13} />
                  </button>
                )}
                <button onClick={() => setOpen(false)} className="text-white/70 hover:text-white transition-colors cursor-pointer p-1.5 rounded-md hover:bg-white/10">
                  <X size={16} />
                </button>
              </div>
            </div>
        ...
        >
          <span className={`absolute -top-1 -right-1 w-3 h-3 rounded-full bg-emerald-400 border-2 border-neutral-light ${hasUnread ? "unread-pulse" : ""}`} />

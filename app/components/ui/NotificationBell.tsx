...
export default function NotificationBell({
   ...
   const resolvedFooterHref =
      footerHref ??
      (variant === "admin" ? "/admin/notifications" : "/profile/notifications");
   const resolvedFooterLabel = footerLabel ?? "Xem tất cả thông báo";
   const resolvedHeaderLabel =
      headerLabel ?? (variant === "admin" ? "Thông báo hệ thống" : "Thông báo");
   ...
   const {
      ...
      markAsRead,
      markAllAsRead,
   } = variant === "admin" ? adminStore : userStore;
   ...
   const handleItemClick = useCallback(
      async (n: (typeof notifications)[number]) => {
         ...
         if (n.type === "REVIEW_NEW") {
            ...
            setOpen(false);
            navigateToReview(data?.reviewId, data?.productSlug); // ← dùng đúng fn
            return;
         }
      },
            ...
            <div className="flex items-center justify-between px-4 py-3.5 border-b border-neutral">
               ...
               {unreadCount > 0 && (
                  <button
                     ...
                     className="flex items-center gap-1.5 text-[12px] font-medium text-neutral-dark hover:text-accent transition-colors duration-150 cursor-pointer group"
                  >
                     <CheckCheck
                        size={13}

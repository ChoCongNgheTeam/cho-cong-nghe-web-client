...
const COLORS = ["#ef4444", "#f97316", "#fbbf24", "#fb7185", "#e879f9", "#a78bfa", "#60a5fa"];
const SHAPES = ["dot", "heart", "star", "dot", "heart", "star", "dot", "heart", "star", "dot"] as const;
...
export default function WishlistHeart({ productId, className = "", absolute = false }: WishlistHeartProps) {
    ...
    (e: React.MouseEvent<HTMLButtonElement>) => {
      ...
      if (svg) {
        ...
        if (nextLiked) {
          ...
          const anim = svg.animate([{ transform: "scale(1)" }, { transform: "scale(0.6)", offset: 0.3 }, { transform: "scale(1.05)", offset: 0.7 }, { transform: "scale(1)" }], {
            duration: 350,
            easing: "ease-out",
            fill: "forwards",
          });
  ...
  const buttonClass = clsx("w-9 h-9 flex items-center justify-center cursor-pointer disabled:opacity-60 disabled:cursor-wait transition-all", absolute && "absolute top-2 right-2 z-10", className);
  ...
  return (
    <>
      <style>{`
         ...
         `}</style>
      <button type="button" aria-label={liked ? "Bỏ yêu thích" : "Thêm vào yêu thích"} onClick={handleToggle} disabled={isPending} className={buttonClass}>
        <canvas
          ref={canvasRef}
          width={100}
          height={100}
        ...
        />
        <svg ref={svgRef} width="26" height="26" viewBox="0 0 24 24" fill="none" style={{ display: "block", transformOrigin: "center" }}>
          <path

...
export default function AdminSidebar() {
   ...
   const isGroupActive = (group: NavGroup) =>
      group.items.some((i) => isItemActive(pathname, i.href));

   return (
      <>
         <div
            className={`${collapsed ? "w-14" : "w-64"} bg-neutral-light border-r border-neutral h-full flex flex-col transition-all duration-300 relative z-40`}
         >
            {/* ── Logo ── */}
            <div
               className={`border-b border-neutral flex items-center justify-between ${collapsed ? "px-2 py-3" : "px-3 py-3"}`}
            >
               {!collapsed && (
            ...
            {/* Nút expand khi collapsed — nằm riêng dưới logo */}
            {collapsed && (
               <button
                  onClick={() => {
                     setCollapsed(false);

...
function getRuleLabel(rules: SaleScheduleRule[]): string {
   ...
   const r = rules[0];
   if (r.actionType === "DISCOUNT_PERCENT" && r.discountValue)
      return `Giảm ${r.discountValue}%`;
   if (r.actionType === "DISCOUNT_FIXED" && r.discountValue) {
      const val =
         r.discountValue >= 1_000_000
            ? `${(r.discountValue / 1_000_000).toFixed(0)}tr`
            : `${(r.discountValue / 1000).toFixed(0)}k`;
      return `Giảm ${val}`;
   }
...
const Countdown = memo(function Countdown({
   ...
   return (
      <span
         className="font-bold text-promotion tabular-nums"
...
function TabItem({
   ...
   const endTime = mainPromotion?.endDate
      ? formatTime(mainPromotion.endDate)
      : null;
   const startTime = mainPromotion?.startDate
      ? formatTime(mainPromotion.startDate)
      : null;
         ...
         className={`shrink-0 min-w-[120px] px-4 py-3 text-center transition-all cursor-pointer relative ${
            isActive
               ...
               : day.hasActiveSale
                 ...
                 : "hover:bg-neutral-light-active opacity-60"
         }`}
      >
         {/* Ngày */}
         <div className="font-semibold text-primary text-sm leading-tight flex items-center justify-center gap-1">
            {day.isToday ? (
               <span className="text-promotion font-bold">Hôm nay</span>
            ) : (
               <span>{formatDateTab(day.date)}</span>
            )}
            {isLoading && (
               <span className="w-3 h-3 border border-promotion border-t-transparent rounded-full animate-spin" />
            )}
         </div>
...
export function HotSaleOnline({ saleSchedule }: HotSaleOnlineProps) {
   ...
   const safeTodayProducts = todayProducts ?? {
      products: [],
      total: 0,
      promotions: [],
      endDate: null,
                     ...
                     {hasSaleDays ? (
                        schedule.map((day) => (
                           <TabItem
                              ...
                              isLoading={loadingDate === day.date}
                     ...
                     {loadingDate === activeDate ? (
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 py-4 items-stretch">
                           {Array.from({ length: 4 }).map((_, i) => (
                              <div
                                 key={i}
                                 className="rounded-xl border border-neutral bg-neutral-light-active animate-pulse h-52"
                              />
                           ))}
                        </div>
                     ) : products.length > 0 ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-10 gap-2">
                           ...
                           {activeDay &&
                              !activeDay.isToday &&
                              activeDay.hasActiveSale && (
                                 <p className="text-[12px] text-neutral-dark/60">
                                    Ngày {formatDateTab(activeDay.date)}
                                 </p>
                              )}
                        </div>
                     )}

        ...
        <div className="absolute bottom-0 inset-x-0 p-3">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-full bg-promotion flex items-center justify-center shrink-0">
              <span className="text-white text-xs font-bold">{blog.author.fullName.charAt(0)}</span>
            </div>
            <span className="text-xs font-medium text-white truncate">{blog.author.fullName}</span>
            <span className="text-white/60">•</span>
            <span className="text-xs text-white/80 shrink-0">{formatDate(blog.publishedAt)}</span>
          </div>
        </div>
      </div>

  ...
  return (
    <section className="py-6 md:py-8">
      <div className="container px-4">
        <Slidezy items={{ mobile: 1, tablet: 2, desktop: 2 }} gap={24} speed={400} loop={false} nav={false} controls={bannersTop.length > 2} slideBy={1} draggable={true}>
          {bannersTop.map((banner) => (
            <Link key={banner.id} href={banner.linkUrl ?? "#"} className="group relative overflow-hidden rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-500 block">
              <div className="relative h-[200px] md:h-[240px] overflow-hidden">
                {banner.imageUrl && <Image src={banner.imageUrl} alt={banner.title} fill className="object-cover object-center transition-transform duration-700 group-hover:scale-110" />}
              </div>

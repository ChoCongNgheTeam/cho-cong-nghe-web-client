  ...
  return (
    <Link
      ...
      className="group relative flex flex-col rounded-xl overflow-hidden select-none h-full
                 ...
                 hover:shadow-lg hover:-translate-y-0.5 hover:border-accent/40"
    ...
    >
      <div className="relative w-full aspect-[5/3] overflow-hidden">
        {product.thumbnail ? (
          <Image
            ...
            alt={product.name}
            fill
            className="object-contain p-4 scale-90 transition-transform duration-500 ease-out group-hover:scale-100"
            sizes="(max-width: 640px) 50vw, 300px"
          />
        ) : (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-1 rounded-lg bg-neutral text-neutral-dark">
        ...
        <div className="flex gap-1.5 px-2.5 pt-2 sm:flex-row flex-col">
          {visibleHighlights.map((h) => (
            <span
              key={h.key}
              title={h.value}
              className="inline-flex items-center gap-1 min-w-0 bg-neutral text-primary-light border border-surface-border text-[10px] px-2 py-[2px] rounded-md font-medium sm:max-w-[48%] sm:shrink"
            >
              <HighlightIcon icon={h.icon} className="w-3 h-3 flex-shrink-0 text-neutral-dark" />
      ...
      <div className="px-2.5 mb-2.5 mt-auto">
        {isUpcoming ? (
          <button
            ...
            style={{ color: flashSale.promotion, borderColor: flashSale.promotion }}
          >
            Sắp diễn ra
          </button>
        ) : (
          <button
            ...
            className="w-full rounded-full h-8 xs:h-9 sm:h-10 text-center text-xs xs:text-sm font-semibold border cursor-pointer transition-all duration-200 ease-out"
            style={{ color: flashSale.promotion, borderColor: flashSale.promotion }}
            onMouseEnter={(e) => {
              const btn = e.currentTarget;
              btn.style.background = flashSale.promotion;
              btn.style.color = "#fff";
              btn.style.boxShadow = flashSale.ctaBorderGlow;
            }}
            onMouseLeave={(e) => {
              const btn = e.currentTarget;
              btn.style.background = "transparent";
              btn.style.color = flashSale.promotion;
      ...
      <div className="px-2.5 pb-2.5 flex items-center justify-between">
        {product.rating.count > 0 ? (
          ...
          <span className="text-[9px] sm:text-[11px] text-neutral-dark italic">Chưa có đánh giá</span>
        )}

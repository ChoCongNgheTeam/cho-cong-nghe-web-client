  ...
  return (
    <div className="min-h-screen bg-neutral-light py-12">
      <div className="container max-w-2xl mx-auto px-4">
        <div className="mb-8">
          <Breadcrumb items={[{ label: "Trang chủ", href: "/" }, { label: "Tài khoản", href: "/account" }, { label: "Đổi mật khẩu" }]} />
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-primary mt-1">Cập nhật mật khẩu để bảo mật tài khoản của bạn</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-neutral-light border border-neutral rounded-lg shadow-lg p-8">
          <form onSubmit={handleSubmit} className="space-y-6">
            {/* Current Password */}
            <div>
              <label htmlFor="currentPassword" className="block mb-2 text-sm font-medium text-primary">
                Mật khẩu hiện tại
              </label>
              <div className="relative">
                <input
                  ...
                  className={`w-full px-4 py-3 pr-12 border rounded-lg bg-neutral-light text-primary placeholder:text-neutral-dark focus:outline-none focus:ring-2 focus:ring-accent focus:border-transparent transition-all duration-200 ${
                    errors.currentPassword ? "border-promotion" : "border-neutral"
                  }`}
                  placeholder="Nhập mật khẩu hiện tại"
                />
                <button type="button" onClick={() => toggleShowPassword("current")} className="absolute right-3 top-1/2 -translate-y-1/2 text-neutral-dark hover:text-primary transition-colors">
                  {showPassword.current ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
                </button>
              </div>
              {errors.currentPassword && <p className="mt-1 text-sm text-promotion">{errors.currentPassword}</p>}
            </div>
            ...
            {/* New Password */}
            <div>
              <label htmlFor="newPassword" className="block mb-2 text-sm font-medium text-primary">
                Mật khẩu mới
              </label>
              <div className="relative">
                <input
                  ...
                  onChange={handleChange}
                  className={`w-full px-4 py-3 pr-12 border rounded-lg bg-neutral-light text-primary placeholder:text-neutral-dark focus:outline-none focus:ring-2 focus:ring-accent focus:border-transparent transition-all duration-200 ${
                    errors.newPassword ? "border-promotion" : "border-neutral"
                  }`}
                  placeholder="Nhập mật khẩu mới"
                />
                <button type="button" onClick={() => toggleShowPassword("new")} className="absolute right-3 top-1/2 -translate-y-1/2 text-neutral-dark hover:text-primary transition-colors">
                  {showPassword.new ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
                </button>
              </div>
              {errors.newPassword && <p className="mt-1 text-sm text-promotion">{errors.newPassword}</p>}

              {/* Password Strength Indicator */}
              {formData.newPassword && (
                <div className="mt-3">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs text-primary">Độ mạnh mật khẩu</span>
                    <span className={`text-xs font-medium ${strengthScore <= 2 ? "text-promotion" : strengthScore <= 3 ? "text-accent" : strengthScore <= 4 ? "text-accent" : "text-green-500"}`}>
                      {getStrengthText()}
                    </span>
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <PasswordRequirement met={passwordStrength.special} text="Chứa ký tự đặc biệt (!@#$...)" />
                  </div>
                </div>
              )}
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  onChange={handleChange}
                  className={`w-full px-4 py-3 pr-12 border rounded-lg bg-neutral-light text-primary placeholder:text-neutral-dark focus:outline-none focus:ring-2 focus:ring-accent focus:border-transparent transition-all duration-200 ${
                    errors.confirmPassword ? "border-promotion" : "border-neutral"
                  }`}
                  placeholder="Nhập lại mật khẩu mới"
                />
                <button type="button" onClick={() => toggleShowPassword("confirm")} className="absolute right-3 top-1/2 -translate-y-1/2 text-neutral-dark hover:text-primary transition-colors">
                  {showPassword.confirm ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
                </button>
              </div>
              {errors.confirmPassword && <p className="mt-1 text-sm text-promotion">{errors.confirmPassword}</p>}
            </div>
            ...
            <div className="flex gap-3 p-4 bg-accent-light border border-accent-light-active rounded-lg">
              ...
              <div className="text-sm text-primary">
                <p className="font-medium text-primary mb-1">Lưu ý bảo mật</p>
                <ul className="space-y-1 text-xs">
                  <li className="text-primary">• Không chia sẻ mật khẩu với bất kỳ ai</li>
                  <li className="text-primary">• Sử dụng mật khẩu khác nhau cho mỗi tài khoản</li>
                  <li className="text-primary">• Thay đổi mật khẩu định kỳ để bảo mật tốt hơn</li>
                </ul>
              </div>
            </div>

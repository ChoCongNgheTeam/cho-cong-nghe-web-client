export const VALIDATION_MESSAGES = {
   ...
   password: {
      ...
      invalid:
         "Mật khẩu không được chứa ký tự có dấu hoặc ký tự đặc biệt không hợp lệ",
   },
   confirmPassword: {
...
export function validatePassword(value: string): string | undefined {
   ...
   if (!PASSWORD_UPPERCASE_REGEX.test(value))
      return VALIDATION_MESSAGES.password.uppercase;
   if (!PASSWORD_DIGIT_REGEX.test(value))
      return VALIDATION_MESSAGES.password.digit;
}
...
export function validatePhone(value: string): string | undefined {
   ...
   if (!trimmed) return;
   const normalized = trimmed.startsWith("+84")
      ? "0" + trimmed.slice(3)
      : trimmed;

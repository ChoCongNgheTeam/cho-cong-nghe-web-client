...
export default function ResetPasswordContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await apiRequest.post("/auth/reset-password", { token, password: newPassword, confirmPassword }, { noAuth: true });
      ...
      const errorMessage = (error as any)?.data?.message || (error as any)?.message || "Không thể đặt lại mật khẩu. Vui lòng thử lại.";
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-neutral-light">
        <div className="max-w-md w-full p-8 bg-neutral-light border border-neutral rounded-lg shadow-lg">
          <Breadcrumb items={[{ label: "Trang chủ", href: "/" }, { label: "Quên mật khẩu", href: "/forgot-password" }, { label: "Đặt lại mật khẩu" }]} />
          <h1 className="text-2xl font-bold text-promotion mb-4">Link không hợp lệ</h1>
          <p className="text-primary-light mb-6">Link đặt lại mật khẩu không hợp lệ hoặc đã hết hạn.</p>
          <button onClick={() => router.push("/forgot-password")} className="w-full bg-primary text-neutral-light py-3 rounded-lg hover:bg-primary-hover transition-colors duration-200">
            Yêu cầu link mới
          </button>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-neutral-light">
      <div className="max-w-md w-full p-8 bg-neutral-light border border-neutral rounded-lg shadow-lg">
        <Breadcrumb items={[{ label: "Trang chủ", href: "/" }, { label: "Quên mật khẩu", href: "/forgot-password" }, { label: "Đặt lại mật khẩu" }]} />
        <h1 className="text-2xl font-bold text-primary mb-6">Đặt lại mật khẩu</h1>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block mb-2 text-sm font-medium text-primary">Mật khẩu mới</label>
            <input
              type="password"
              value={newPassword}
          ...
          <div>
            <label className="block mb-2 text-sm font-medium text-primary">Xác nhận mật khẩu</label>
            <input
              type="password"
        ...
        <div className="mt-4 p-3 bg-accent-light border border-accent-light-active rounded-lg">
          <p className="text-xs text-primary-light">
            <span className="font-semibold text-primary">Yêu cầu mật khẩu:</span>
            <br />
            <span className="text-primary"> • Tối thiểu 6 ký tự</span>
            <br />
            <span className="text-primary"> • Nên bao gồm chữ hoa, chữ thường và số</span>
          </p>
        </div>
      </div>
    </div>

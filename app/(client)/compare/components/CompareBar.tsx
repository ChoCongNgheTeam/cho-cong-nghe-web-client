...
function AddSlot() {
  ...
  const handleSelect = (product: ProductDetail) => {
    const result = add(product, isMobile);
...
export default function CompareBar() {
  const { items, remove, clear, maxCompare } = useCompareStore();
  const isMobile = useIsMobile();
  const MAX = maxCompare(isMobile); // 2 trên mobile, 3 trên desktop
    ...
    <div className="fixed bottom-0 left-0 right-0 z-50 bg-neutral-light border-t border-neutral shadow-lg px-6 py-3">
      <div className="max-w-6xl mx-auto flex items-center gap-4">
        ...
        <div className="flex gap-2.5 flex-1 items-center overflow-x-auto">
          ...
          {/* Ô thêm sản phẩm — ẩn khi đã đủ MAX */}
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          ...
          <button
            ...
            className="text-xs font-medium bg-primary text-neutral-light px-4 py-2 rounded-lg disabled:opacity-30 hover:opacity-80 transition-opacity"
          >
            So sánh ngay
          </button>
        </div>

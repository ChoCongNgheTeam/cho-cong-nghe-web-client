               ...
               <Section title="4. Thời gian diễn ra chương trình">
                  <div className="rounded-lg px-4 py-3 bg-neutral-light-active border border-neutral">
                     <p className="text-primary">
                        ...
                        <span className="text-primary-light ml-2" style={{ fontSize: "12px" }}>
               ...
               <Section title="5. Chi tiết cách thức và thể lệ tham gia chương trình tại ChoCongNghe">
                  ...
                  <ul className="space-y-3 mb-5">
                     {[
                        ...
                        <>Cứ mỗi <strong>4.000 đồng</strong> trên hóa đơn thanh toán, khách hàng sẽ được tích <strong>01 điểm thưởng</strong>. Số điểm thưởng được tích sẽ dựa vào giá trị cuối cùng của hóa đơn khách hàng thanh toán. <span className="text-primary-light">Ví dụ: Giá trị đơn hàng là 500.000đ, khách hàng áp dụng mã khuyến mãi 100.000đ → giá trị thanh toán 400.000đ → được tích 100 điểm.</span></>,
                     ].map((item, i) => (
                        <li key={i} className="flex gap-2 leading-relaxed text-primary">
                           <span className="mt-1 shrink-0">•</span>
                           <span>{item}</span>
                  ...
                  <div className="rounded-lg p-4 bg-neutral-light-active border border-neutral mb-6">
                     ...
                     <p className="mt-2 text-primary-light" style={{ fontSize: "12px" }}>
                  ...
                  <ol className="space-y-3 mb-5">
                     {[
                        ...
                        <li key={i} className="flex items-start gap-3 text-primary">
                           <span className="w-6 h-6 rounded-full flex items-center justify-center shrink-0 font-bold bg-promotion text-white mt-0.5" style={{ fontSize: "12px" }}>
                              {i + 1}
                           </span>
               ...
               <Section title="6. Các quy định khác">
                  ...
                  <ul className="space-y-2 mb-5">
                     {[
                        ...
                        <>Điểm thưởng có hạn sử dụng trong vòng <strong>12 tháng</strong> kể từ lúc tích điểm và hết hạn vào ngày cuối cùng của tháng. <span className="text-primary-light">Ví dụ: Điểm tích vào ngày 24/09/2023 sẽ hết hạn vào ngày 30/09/2024.</span></>,
                  ...
                  <ul className="space-y-2">
                     {[
                        ...
                        <>Tất cả các thắc mắc và khiếu nại về chương trình, vui lòng liên hệ với chúng tôi qua hotline: <strong>1800 6616</strong> (miễn phí).</>,
                     ].map((item, i) => (
                        <li key={i} className="flex gap-2 leading-relaxed text-primary">
                           <span className="mt-1 shrink-0">•</span>
                           <span>{item}</span>
...
function SubTitle({ children }: { children: React.ReactNode }) {
   return (
      <p className="font-semibold mb-3 text-primary" style={{ fontSize: "13px" }}>{children}</p>
   );
}
...
function SubSection({ label, children }: { label: string; children: React.ReactNode }) {
   return (
      <div className="pl-4 border-l-[3px] border-promotion">
         <p className="font-semibold mb-1 text-primary-light" style={{ fontSize: "12px" }}>{label}</p>

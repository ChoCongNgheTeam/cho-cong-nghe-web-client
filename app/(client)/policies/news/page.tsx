...
function mapBlog(item: BlogApiItem): NewsBlog {
  ...
  const category = typeof rawCategory === "string" ? { name: rawCategory, slug: rawCategory } : rawCategory ? { name: rawCategory.name ?? "Tin tức", slug: rawCategory.slug ?? "" } : null;
...
export default async function NewsPage() {
    ...
    return (
      <div className="min-h-screen bg-neutral-light">
        <div className="container py-10">
          <h1 className="text-xl font-bold text-primary mb-2">Tin tức khuyến mãi</h1>
          <p className="text-primary-light">Hiện chưa có bài viết nào. Vui lòng quay lại sau.</p>
        </div>
      </div>
    );
  }
  ...
  const tabs = ["T?t c?", ...Array.from(new Set(blogs.map((item) => item.category?.name).filter(Boolean)))].slice(0, 4);
  ...
  return (
    <div className="min-h-screen bg-neutral-light">
      ...
      <div className="border-b border-neutral bg-neutral-light">
        <div className="container py-6 sm:py-8">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="uppercase tracking-[0.15em] text-primary font-medium mb-1">Cập nhật mới nhất</p>
              <h1 className="text-[28px] font-bold text-primary leading-tight">Tin tức khuyến mãi</h1>
            </div>
              ...
              {tabs.map((tab, i) => (
                <button
                  ...
                  className={[
                    ...
                    i === 0 ? "bg-neutral-light text-primary shadow-sm" : "text-primary hover:text-primary",
                  ].join(" ")}
                >
                  {tab}
                </button>
      ...
      <div className="container py-6 sm:py-8">
        <Link href={`/blog/${featuredNews.slug}`} className="group block mb-8">
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-0 rounded-xl overflow-hidden border border-neutral bg-neutral-light hover:shadow-md transition-shadow duration-200">
            ...
            <div className="lg:col-span-3 relative overflow-hidden bg-accent-light aspect-video">
              <Image src={featuredNews.thumbnail} alt={featuredNews.title} fill className="object-cover group-hover:scale-[1.02] transition-transform duration-300" />
            </div>
            ...
            <div className="lg:col-span-2 p-5 sm:p-6 lg:p-7 flex flex-col justify-between">
              <div>
                <div className="flex items-center gap-2 mb-3 text-sm">
                  <span className={`px-2 py-0.5 rounded font-semibold ${getCategoryColor(featuredNews)}`}>{getCategoryLabel(featuredNews)}</span>
                  <span className="text-primary">{formatDate(featuredNews.publishedAt || featuredNews.createdAt)}</span>
                </div>
                <h2 className="font-bold text-primary leading-snug mb-3 group-hover:text-accent transition-colors">{featuredNews.title}</h2>
                <p className="text-primary leading-relaxed line-clamp-3">{featuredNews.excerpt}</p>
              </div>
          ...
          {restNews.map((item) => (
            ...
            >
              ...
              <div className="relative overflow-hidden aspect-video bg-accent-light">
                <Image src={item.thumbnail} alt={item.title} fill className="object-cover group-hover:scale-[1.03] transition-transform duration-300" />
              </div>

              {/* Content */}
              <div className="p-4 flex flex-col flex-1">
                <div className="flex items-center gap-2 mb-2 text-sm">
                  <span className={`px-2 py-0.5 rounded font-semibold ${getCategoryColor(item)}`}>{getCategoryLabel(item)}</span>
                  <span className="text-primary">{formatDate(item.publishedAt || item.createdAt)}</span>
                </div>
                <h3 className="text-[14px] font-bold text-primary leading-snug mb-2 line-clamp-2 group-hover:text-accent transition-colors flex-1">{item.title}</h3>
                <p className="text-primary leading-relaxed line-clamp-2 mb-3">{item.excerpt}</p>

...
export default function VoucherPromotionModal({
  ...
  const calculateDiscount = useCallback((voucher: Voucher, orderTotal: number): number => {
  ...
  const canUseVoucher = useCallback((voucher: Voucher, orderTotal: number): boolean => {
  ...
  const handleApplyVoucher = useCallback(() => {
    ...
    if (!canUseVoucher(voucher, cartTotal)) {
      if (cartTotal < voucher.min_order_value) {
        setVoucherError(
          `Đơn hàng tối thiểu ${formatPrice(voucher.min_order_value)} để sử dụng mã này`
        );
        ...
        if (now < startDate) {
          setVoucherError(`Mã voucher có hiệu lực từ ${formatDate(voucher.start_date)}`);
        } else if (now > endDate) {
          setVoucherError(`Mã voucher đã hết hạn ngày ${formatDate(voucher.end_date)}`);
        } else {
  ...
  }, [voucherCode, cartTotal, canUseVoucher, calculateDiscount, formatPrice, formatDate, onApplyVoucher]);
  ...
  const handleConfirm = useCallback(() => {
    const selectedIds = promotionsList.filter((p) => p.selected).map((p) => p.id);
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-[60] flex items-end lg:items-center lg:justify-end p-0 sm:p-4 lg:p-0 pointer-events-none">
        <div
          className="bg-white w-full sm:max-w-lg lg:h-full lg:max-w-md shadow-2xl flex flex-col sm:rounded-t-2xl lg:rounded-l-2xl lg:rounded-tr-none animate-slide-up lg:animate-slide-left max-h-[90vh] lg:max-h-full pointer-events-auto"
              ...
              {voucherSuccess && appliedVoucherValue > 0 && (
                <div className="mt-2 p-2 bg-green-50 border border-green-200 rounded-lg flex items-center gap-2">
                  ...
                  <span className="text-xs text-green-700">
                    Mã <strong>{appliedVoucherCode}</strong> đã áp dụng - Giảm{" "}
                    <strong>{formatPrice(appliedVoucherValue)}</strong>
                  </span>
                </div>
              )}
              ...
              <div className="mt-4">
                ...
                <div className="space-y-2 max-h-[300px] overflow-y-auto">
                  {availableVouchers.map((voucher) => {
                    ...
                    const discount = calculateDiscount(voucher, cartTotal);
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-3 mt-2 text-xs text-neutral-dark">
                            ...
                            {voucher.usage_limit && (
                              <div className="flex items-center gap-1">
                                ...
                                <span>
                                  {voucher.usage_limit - voucher.usage_count} lượt
                                </span>
                              </div>
                            )}
                          </div>
                          ...
                          {!canUse && (
                            <p className="text-xs text-red-600 mt-1">
                              {cartTotal < voucher.min_order_value
            ...
            <div className="p-4">
              ...
              <div className="mt-4 p-3 bg-accent-light rounded-lg border border-accent">
                <p className="text-xs text-primary-darker">
                  <span className="font-semibold">Lưu ý:</span> Một số khuyến mãi
                  không áp dụng đồng thời. Chọn khuyến mãi phù hợp nhất với đơn hàng.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="border-t border-neutral p-4 bg-white">
            <div className="flex items-center justify-between mb-3 text-sm">
              <span className="text-neutral-darker">
                ...
                {selectedCount > 0 && voucherSuccess && appliedVoucherValue > 0 && " + "}
                {voucherSuccess && appliedVoucherValue > 0 && "1 voucher"}
                {selectedCount === 0 && (!voucherSuccess || appliedVoucherValue === 0) && "Chưa chọn"}
              </span>

...
export default function AddToCartButton({
  ...
  const handleClick = async () => {
    ...
    setLoading(true);
    const qty = getQuantity ? getQuantity() : quantity;
    try {
      await addToCart(productVariantId, qty, meta);
      // Chỉ chạy nếu addToCart không throw
      toasty.success(meta?.productName ? `Đã thêm "${meta.productName}" vào giỏ hàng` : "Đã thêm vào giỏ hàng");
    } catch (err) {
      const message = err instanceof Error ? err.message : null;
      toasty.error(message ?? "Không thể thêm vào giỏ hàng, vui lòng thử lại");
    } finally {
      setLoading(false);
    }
  };

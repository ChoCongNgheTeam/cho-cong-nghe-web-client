          ...
          {options.map((variant) => {
            ...
            return (
              ...
              >
                {/* Product image */}
                {variant.imageUrl ? (
                  <Image src={variant.imageUrl} alt={variant.colorLabel} width={36} height={44} className="w-9 h-11 object-contain shrink-0" />
                ) : (
                  <div
                ...
                {isOutOfStock && (
                  <div className="absolute inset-0 bg-neutral-light/80 flex items-center justify-center">
                    <span className="text-[13px] text-promotion font-semibold">Hết hàng</span>
                  </div>
                )}
              </button>
            );
    ...
    <>
      ...
      <div
        ...
        className={`fixed bottom-0 left-0 right-0 z-50
               bg-neutral-light rounded-t-2xl shadow-2xl
               flex flex-col
      ...
      >
        ...
        >
          <div className="w-10 h-1 rounded-full bg-neutral-dark" />
        </div>

        {/* Header */}
        <div className="flex-shrink-0 flex items-center justify-between gap-2 px-4 py-3 border-b border-neutral">
          <div className="flex items-baseline gap-1.5 min-w-0 flex-1">
            <span className="text-base font-semibold text-primary shrink-0">{title}</span>
            {productName && <span className="text-base text-neutral-darker line-clamp-1 min-w-0">{productName}</span>}
          </div>
          <button type="button" onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-full bg-neutral hover:bg-neutral-active transition shrink-0" aria-label="Đóng">
...
// ─── Main component ──────────────────────────────────────────────────────────
export default function VariantDropdown({ triggerLabel, options, selectedId, isOpen, isFetching, isChanging, errorMessage, productName, onToggle, onSelect, onRetry }: VariantDropdownProps) {
  ...
  const sharedContent = <OptionGrid options={options} selectedId={selectedId} isFetching={isFetching} errorMessage={errorMessage} onSelect={handleSelect} onRetry={onRetry} />;
  ...
  return (
    <>
        ...
        >
          {isChanging ? (
            <>
              ...
              <span className="text-neutral-darker truncate">Đang cập nhật...</span>
            </>
          ) : isFetching && options.length === 0 ? (
            <>
              <Loader2 className="h-3 w-3 animate-spin text-neutral-dark shrink-0" />
              <span className="text-neutral-darker truncate">{triggerLabel}</span>
            </>
          ) : (
            <span className="font-medium text-neutral-darker truncate">{triggerLabel}</span>
          )}
      ...
      {/* ── Mobile bottom sheet ── */}
      {isMobile && (
        <BottomSheet isOpen={isOpen} onClose={onToggle} title="Màu" productName={productName}>
          {sharedContent}
        </BottomSheet>
      )}
    </>

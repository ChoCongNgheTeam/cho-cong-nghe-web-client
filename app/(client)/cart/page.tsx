...
import { Trash2, Plus, Minus, ShoppingCart, ChevronUp, ChevronDown, X, ChevronRight } from "lucide-react";
                    ...
                    <div className="flex gap-2 sm:gap-3 min-w-0">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="xl:hidden flex items-center  gap-1 sm:gap-3 flex-wrap">
                          ...
                          <div className="flex items-center border border-neutral rounded w-fit shrink-0">
                            ...
                            <span className="flex h-8 w-8 items-center justify-center border-x border-neutral text-sm font-medium text-primary">{item.quantity}</span>
                            <button onClick={() => handleIncrease(item.id)} className="flex h-8 w-8 items-center justify-center rounded-r text-neutral-darker transition hover:bg-accent-light">
                      ...
                      <div className="hidden xl:flex items-center gap-4 shrink-0">
                        ...
                        <div className="flex items-center border border-neutral rounded w-fit shrink-0">
                          ...
                          <span className="flex h-7 w-7 items-center justify-center border-x border-neutral text-sm font-medium text-primary">{item.quantity}</span>
                          <button
        ...
        {showSidebar && (
          <div className="bg-neutral-light border-t border-neutral rounded-t-2xl">
            ...
            <div className="px-4 py-3 space-y-2 text-sm max-h-[40vh] overflow-y-auto">
              <h3 className="font-semibold text-primary">Chi tiết thanh toán</h3>
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium text-primary">-{formatVND(totalDiscount + voucherValue)}</span>
              </div>
              <div className="flex justify-between pl-4">
                <span className="text-neutral-dark text-xs">Giảm giá sản phẩm</span>
                <span className="text-xs">-{formatVND(totalDiscount)}</span>
              </div>
              <div className="flex justify-between pl-4">
                <span className="text-neutral-dark text-xs">Voucher</span>
                <span className="text-xs font-medium">{voucherValue > 0 ? `-${formatVND(voucherValue)}` : "0₫"}</span>
              </div>

              <div className="border-t border-neutral pt-2">
                <div className="flex justify-between items-center">
                  <span className="font-semibold text-primary">Cần thanh toán</span>
                  <span className="text-lg font-bold text-promotion">{formatVND(finalTotalWithVoucher)}</span>
                </div>
              </div>
          ...
          <div className="bg-neutral-light border-t border-neutral flex items-center gap-2 px-3 py-2.5">
            <button onClick={() => setShowSidebar((prev) => !prev)} className="flex-1 flex items-center justify-end gap-2 min-w-0 py-1 rounded-lg hover:bg-neutral transition">
              <div className="flex flex-col items-end min-w-0">
                <span className="text-base font-bold text-promotion whitespace-nowrap">{formatVND(finalTotalWithVoucher)}</span>
                {totalDiscount + voucherValue > 0 && <span className="text-xs text-neutral-darker whitespace-nowrap">Tiết kiệm {formatVND(totalDiscount + voucherValue)}</span>}
              </div>
            ...
            <button
              ...
              className={`shrink-0 rounded-xl px-5 py-3 text-sm font-bold transition shadow-lg ${
                selectedItems.length === 0 ? "cursor-not-allowed bg-neutral text-neutral-dark opacity-50" : "bg-primary-dark text-neutral-light hover:bg-accent-hover active:scale-[0.98]"
              }`}
            >
              Xác nhận đơn
            </button>

...
export function buildCategoryMetadata(slug: string, page: number): Metadata {
  ...
  const title = page > 1 ? `${categoryTitle} - Trang ${page} | ${SITE_NAME}` : `${categoryTitle} | ${SITE_NAME}`;
  ...
  const canonicalUrl = page > 1 ? `${BASE_URL}/category/${slug}?page=${page}` : `${BASE_URL}/category/${slug}`;

...
export default async function CategoryPage({
   ...
   const brands: BrandApiItem[] =
      brandsResult.status === "fulfilled"
         ? (brandsResult.value as BrandApiItem[])
         : [];
   const banners: MediaApiItem[] =
      bannersResult.status === "fulfilled"
         ? (bannersResult.value as MediaApiItem[])
         : [];
              ...
              {subCategories.map((sub) => (
               ...
               {isRoot && banners.length > 0 && (
                  <div className="mb-5 overflow-hidden w-full">
                     <Slidezy
                        items={{ mobile: 1, tablet: 1, desktop: 1 }}
                        speed={500}
                        loop
                        autoplay
                        nav
                     ...
                     >
                        {banners
                           ...
                           .map((banner) => (
                              <div
                                 key={banner.id}
                                 className="relative w-full aspect-[1450/340] overflow-hidden"
                              >
                                 {banner.linkUrl ? (
                                    <Link
                                       href={banner.linkUrl}
                                    ...
                                    >
                                       <Image
                                          src={banner.imageUrl ?? ""}
                                          alt={banner.title ?? categoryTitle}
                                          fill

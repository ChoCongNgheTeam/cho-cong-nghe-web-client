        ...
        <div className="container py-4">
          ...
          {isRoot && banners.length > 0 && (
            <div className="mb-5 overflow-hidden w-full">
              <Slidezy items={{ mobile: 1, tablet: 1, desktop: 1 }} speed={500} loop autoplay nav controls gap={0} className="overflow-hidden">
                {banners
                  .filter((banner) => banner.imageUrl)
                  .map((banner) => (
                    <div key={banner.id} className="relative w-full aspect-[1450/340] overflow-hidden">
                      {banner.linkUrl ? (
                        <Link href={banner.linkUrl} className="absolute inset-0">
                          <Image src={banner.imageUrl ?? ""} alt={banner.title ?? categoryTitle} fill className="object-cover rounded-2xl" priority />
                        </Link>
                      ) : (
                        <Image src={banner.imageUrl ?? ""} alt={banner.title ?? categoryTitle} fill className="object-cover" priority />
                      )}
                    </div>
                  ))}
              </Slidezy>

...
function getRangeMinMax(group: FilterGroup): { min: number; max: number } | null {
...
export default function ProductFilter({ filters }: ProductFilterProps) {
  ...
  useEffect(() => {
    ...
    for (const [key, val] of Object.entries(debouncedSliders)) {
      ...
      changed = true;
      updates[`${key}_min`] = val.min === bounds.min ? null : String(val.min);
      updates[`${key}_max`] = val.max === bounds.max ? null : String(val.max);
    }
    if (!changed) return;
  ...
  // ─── Desktop URL helpers ─────────────────────────────────────────────────────
  const getValues = useCallback((key: string) => searchParams.getAll(key), [searchParams]);
  const getValue = useCallback((key: string) => searchParams.get(key) ?? "", [searchParams]);
  ...
  const openSheet = useCallback(() => {
    ...
    filters.forEach((g) => {
      if (g.type === "ENUM") {
        ...
        initBool[g.key] = raw === "true" ? true : raw === "false" ? false : null;
      }
    });
  ...
  const applyDraft = useCallback(() => {
    ...
    closeSheet();
  }, [draftEnum, draftRange, draftBool, filters, pathname, router, searchParams, closeSheet, startTransition]);
  ...
  const draftCount = useMemo(() => {
    ...
    for (const [key, val] of Object.entries(draftRange)) {
      ...
      if (bounds && (val.min !== bounds.min || val.max !== bounds.max)) count++;
    }
    return count;
  ...
  const SectionHeader = ({ label, groupKey }: { label: string; groupKey: string }) => (
    <button type="button" onClick={() => toggleCollapse(groupKey)} className="w-full flex items-center justify-between py-2.5 border-b border-neutral mb-3 cursor-pointer">
      <span className="text-sm font-semibold text-primary">{label}</span>
      {collapsed[groupKey] ? <ChevronDown className="w-4 h-4 text-primary" /> : <ChevronUp className="w-4 h-4 text-primary" />}
    </button>
  );
  ...
  const Chip = ({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) => (
    <button
      ...
      className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all cursor-pointer duration-150 ${
        active ? "border-accent bg-accent-light text-accent" : "border-neutral bg-neutral-light text-primary hover:border-accent-light hover:bg-accent-light/50"
      }`}
    >
      {label}
    </button>
        ...
        {!collapsed[group.key] && (
          <div className="flex flex-wrap gap-2">
            {group.options.map((opt) => (
              <Chip key={opt.value} label={opt.label} active={selected.includes(opt.value)} onClick={() => toggleEnum(group.key, opt.value)} />
            ))}
          </div>
        )}
      </div>
  ...
  const renderRangeDesktop = (group: FilterGroup) => {
    ...
    const fmt = (v: number) => (isPrice ? formatNumber(v) + "đ" : `${v}${unit ? " " + unit : ""}`);
    const setLocal = (min: number, max: number) => setSliderValues((prev) => ({ ...prev, [group.key]: { min, max } }));
        ...
        {!collapsed[group.key] && (
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              <div className="flex-1 px-2 py-1.5 text-xs border border-neutral rounded-lg bg-neutral-light-active text-primary text-center">{fmt(local.min)}</div>
              <span className="text-neutral-dark text-xs">–</span>
              <div className="flex-1 px-2 py-1.5 text-xs border border-neutral rounded-lg bg-neutral-light-active text-primary text-center">{fmt(local.max)}</div>
            </div>
            {isSameMinMax ? (
              <p className="text-xs text-primary text-center">{fmt(bounds.min)}</p>
            ) : (
              <>
                <input
                  type="range"
                  ...
                  value={local.min}
                  onChange={(e) => setLocal(Math.min(Number(e.target.value), local.max - (isPrice ? 500000 : 0.1)), local.max)}
                ...
                <input
                  ...
                  step={isPrice ? 500000 : 0.1}
                  value={local.max}
                  onChange={(e) => setLocal(local.min, Math.max(Number(e.target.value), local.min + (isPrice ? 500000 : 0.1)))}
                  className="w-full accent-[rgb(var(--accent))] h-1.5 rounded-full cursor-pointer"
                />
              </>
            )}
      ...
      <div key={group.key}>
        ...
        {!collapsed[group.key] && (
          <div className="flex gap-2">
            <Chip label="Tất cả" active={current === null} onClick={() => setBoolean(group.key, null)} />
            <Chip label="Có" active={current === true} onClick={() => setBoolean(group.key, true)} />
            <Chip label="Không" active={current === false} onClick={() => setBoolean(group.key, false)} />
          </div>
        )}
      </div>
    );
  ...
  const SheetChip = ({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) => (
    <button
      type="button"
      onClick={onClick}
    ...
    return (
      <div key={group.key} className="mb-5">
        ...
        <div className="grid grid-cols-2 gap-2">
          {group.options.map((opt) => (
            <SheetChip key={opt.value} label={opt.label} active={selected.includes(opt.value)} onClick={() => draftToggleEnum(group.key, opt.value)} />
          ))}
        </div>
      </div>
    );
  ...
  const renderRangeSheet = (group: FilterGroup) => {
    ...
    return (
      <div key={group.key} className="mb-5">
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            <div className="flex-1 px-3 py-2 text-sm border border-neutral rounded-xl bg-neutral-light-active text-primary text-center font-medium">{fmt(local.min)}</div>
            <span className="text-neutral-dark text-sm">–</span>
            <div className="flex-1 px-3 py-2 text-sm border border-neutral rounded-xl bg-neutral-light-active text-primary text-center font-medium">{fmt(local.max)}</div>
          </div>
          {isSameMinMax ? (
            <p className="text-sm text-primary text-center">{fmt(bounds.min)}</p>
          ) : (
            <>
              <input
                type="range"
                ...
                step={isPrice ? 500000 : 0.1}
                value={local.min}
                onChange={(e) => draftSetRange(group.key, Math.min(Number(e.target.value), local.max - (isPrice ? 500000 : 0.1)), local.max)}
              ...
              <input
                ...
                step={isPrice ? 500000 : 0.1}
                value={local.max}
                onChange={(e) => draftSetRange(group.key, local.min, Math.max(Number(e.target.value), local.min + (isPrice ? 500000 : 0.1)))}
                className="w-full accent-[rgb(var(--accent))] h-1.5 rounded-full cursor-pointer"
              />
            </>
          )}
    ...
    return (
      <div key={group.key} className="mb-5">
        ...
        <div className="grid grid-cols-3 gap-2">
          <SheetChip label="Tất cả" active={current === null} onClick={() => draftSetBool(group.key, null)} />
          <SheetChip label="Có" active={current === true} onClick={() => draftSetBool(group.key, true)} />
          <SheetChip label="Không" active={current === false} onClick={() => draftSetBool(group.key, false)} />
        </div>
      </div>
    );
  };
  ...
  if (!isMobile) {
    return (
      <div className={`bg-neutral-light border border-neutral rounded-2xl overflow-hidden transition-opacity duration-200 ${isPending ? "opacity-50 pointer-events-none" : "opacity-100"}`}>
        <div className="flex items-center justify-between px-4 py-3 border-b border-neutral bg-neutral-light-active">
          <div className="flex items-center gap-2">
            ...
            {isPending && <span className="text-xs text-primary animate-pulse">Đang lọc...</span>}
          </div>
          {hasFilters && (
            <button type="button" onClick={resetAll} className="flex items-center gap-1 text-xs text-accent hover:text-accent-hover transition-colors cursor-pointer">
              <X className="w-3.5 h-3.5" />
              Xoá tất cả
            </button>
          )}
        </div>
        <div className="px-4 py-4 space-y-5">{filters.map(renderGroupDesktop)}</div>
      </div>
    );
  }
  ...
  return (
    <>
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-1.5 whitespace-nowrap px-3.5 py-2 rounded-full text-sm font-medium border transition-all duration-150 cursor-pointer ${
                    isActive ? "bg-accent text-neutral-light border-accent shadow-sm" : "bg-neutral-light text-primary border-neutral hover:border-accent-light"
                  }`}
                >
        ...
        >
          ...
          {hasFilters && (
            <span className="absolute -top-1.5 right-0 w-4 h-4 bg-accent text-neutral-light text-[10px] font-bold rounded-full flex items-center justify-center leading-none">
              {Array.from(searchParams.entries()).filter(([k]) => k !== "page" && k !== "sortBy" && k !== "sortOrder").length}
            </span>
          )}
        </button>
      </div>
      ...
      <div
        aria-hidden="true"
        onClick={closeSheet}
        className={`fixed inset-0 z-40 bg-primary-darker/50 transition-opacity duration-300 ${isSheetOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
      />
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3.5 border-b border-neutral shrink-0">
          <div className="flex items-center gap-2">
            <SlidersHorizontal className="w-4 h-4 text-accent" />
            <span className="text-base font-bold text-primary">Bộ lọc tìm kiếm</span>
          </div>
          <button type="button" onClick={closeSheet} aria-label="Đóng" className="p-1.5 rounded-lg text-neutral-dark hover:text-primary hover:bg-neutral-light-active transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>

...
function OrderInfoCard({
  ...
  const voucher = billData?.voucherValue ?? Number(order.voucherDiscount);
  const tax = billData?.taxAmount ?? 0;

  // ✅ grandTotal = finalAfterVoucher + shipping + tax
  const grand = billData?.grandTotal ?? Number(order.totalAmount);
      ...
      <div className="px-4 py-3 border-t border-neutral bg-neutral-light-active space-y-1.5">
        ...
        {voucher > 0 && (
          <div className="flex justify-between text-sm">
            <span className="text-neutral-darker">Giảm giá</span>
            <span className="text-promotion font-medium">-{fmtVND(voucher)}</span>
          </div>
        )}
        ...
        <div className="flex justify-between text-base font-bold text-primary pt-1.5 border-t border-neutral">
          <span>Tổng cộng</span>
          <span className="text-accent">{fmtVND(grand)}</span>
        </div>
      </div>

      {/* Shipping info */}
      <div className="px-4 py-3 border-t border-neutral text-xs text-neutral-darker space-y-1">
        <p className="font-medium text-primary text-sm mb-1">Địa chỉ giao hàng</p>
        <p>
...
function StripeSection({
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const stripe = (window as WindowWithStripe).Stripe!(info.publishableKey);
...
export default function PaymentPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // ✅ Lưu billData nếu checkout đã tính đúng
      if (grandTotal !== undefined) {
        setBillData({
          ...
          finalAfterVoucher: finalAfterVoucher ?? 0,
  ...
  const renderPaymentSection = () => {
    switch (paymentInfo.type) {
      ...
      case "MOMO":
        return <RedirectSection type="MOMO" paymentUrl={paymentInfo.paymentUrl} />;
      case "VNPAY":
        return <RedirectSection type="VNPAY" paymentUrl={paymentInfo.paymentUrl} />;
      case "ZALOPAY":
        return <RedirectSection type="ZALOPAY" paymentUrl={paymentInfo.paymentUrl} />;
      case "STRIPE":
      ...
      <div className="container max-w-xl py-8 px-4 space-y-5">
        ...
        <div className="bg-neutral-light border border-neutral rounded-2xl overflow-hidden">
          ...
          {loadingOrder ? (
            ...
            <div className="p-4">
              {/* ✅ Truyền billData để hiển thị đúng số tiền từ checkout */}
              <OrderInfoCard order={order} billData={billData} />
            </div>
          ) : orderError ? (

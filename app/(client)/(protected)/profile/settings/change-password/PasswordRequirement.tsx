export function PasswordRequirement({ met, text }: { met: boolean; text: string }) {
  return (
    <div className="flex items-center gap-1.5 sm:gap-2">
      <div className={`w-3.5 h-3.5 sm:w-4 sm:h-4 rounded-full flex items-center justify-center shrink-0 transition-colors ${
      ...
      <span className={`text-xs transition-colors ${met ? "text-primary" : "text-primary-light"}`}>
        {text}
      </span>
    </div>
  );

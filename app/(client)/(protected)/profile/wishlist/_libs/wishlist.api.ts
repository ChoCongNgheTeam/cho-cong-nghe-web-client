...
export async function getWishlist(params: GetWishlistParams = {}): Promise<WishlistResponse> {
...
export async function checkWishlist(productId: string): Promise<boolean> {
  try {
    const res = await apiRequest.get<{ data: { isWishlisted: boolean } }>(`/wishlist/check/${productId}`);

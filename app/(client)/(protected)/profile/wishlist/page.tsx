...
export default function WishlistPage() {
  ...
  const products = items.filter((item) => likedIds.has(item.productId)).map(mapToProduct);
    ...
    <section className="flex-1">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-semibold text-primary">
          Sản phẩm yêu thích
          {total > 0 && <span className="ml-2 text-sm font-normal text-neutral-darker">({total} sản phẩm)</span>}
        </h1>
      </div>
      ...
      {isLoading ? (
        ...
        <div className={`grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4 transition-opacity duration-200 ${isFetching ? "opacity-50 pointer-events-none" : "opacity-100"}`}>
          {products.map((product, index) => (
            <ProductCard key={product.id} product={product} index={index} showWishlist={true} />
          ))}
        </div>
      )}
  ...
  return (
    <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="rounded-xl border border-neutral bg-neutral-light p-3 animate-pulse">
...
function WishlistEmpty() {
  return (
    <div className="flex flex-col items-center justify-center py-20 gap-4 text-neutral-darker">
      ...
      <p className="text-sm">Hãy thêm sản phẩm bạn yêu thích để xem lại sau.</p>
    </div>
  );
}

...
import { MapPin, Home, Building2, Plus, Star, Pencil, Trash2, CheckCircle, AlertTriangle, Loader2 } from "lucide-react";
...
// ─── react-select styles (tương tự ShippingSection) ──────────────────────────
const selectStyles = {
  control: (base: any, state: any) => ({
    ...base,
    borderColor: state.isFocused ? "var(--color-accent, #000)" : "var(--color-neutral, #e5e7eb)",
    boxShadow: "none",
    "&:hover": { borderColor: "var(--color-accent, #000)" },
    borderRadius: "0.5rem",
    minHeight: "42px",
    fontSize: "0.875rem",
    cursor: "pointer",
    backgroundColor: "var(--color-neutral-light, #f9fafb)",
  }),
  option: (base: any, state: any) => ({
    ...base,
    backgroundColor: state.isSelected ? "var(--color-accent, #000)" : state.isFocused ? "var(--color-accent-light, #f3f4f6)" : "white",
    color: state.isSelected ? "#fff" : "var(--color-primary, #111)",
    fontSize: "0.875rem",
    cursor: "pointer",
  }),
...
export default function AddressesPage() {
  ...
  const wardOptions = wards.map((w) => ({ value: w.code, label: w.fullName }));
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        ...
        provinceName: selectedProvinceOption?.label ?? "",
        wardCode: Number(wardCode),
        wardName: selectedWardOption?.label ?? "",
        detailAddress,
        type,
        isDefault,
      };
  ...
  const formModalContent = (
    <div className="space-y-4">
      <h2 className="text-base font-semibold text-primary pr-8">{editingAddress ? "Sửa địa chỉ" : "Thêm địa chỉ mới"}</h2>

      <div>
        <label className="block text-sm font-medium text-primary mb-1.5">Thông tin người nhận</label>
        <input className={inputCls} value={contactName} onChange={(e) => setContactName(e.target.value)} placeholder="Nhập họ và tên người nhận" />
      </div>

      <div>
        <label className="block text-sm font-medium text-primary mb-1.5">Số điện thoại</label>
        <input className={inputCls} value={phone} onChange={handlePhoneChange} placeholder="Nhập số điện thoại" inputMode="numeric" maxLength={10} />
      </div>

      {/* Tỉnh/Thành phố — dùng react-select */}
      <div>
        <label className="block text-sm font-medium text-primary mb-1.5">Tỉnh/Thành phố</label>
        {isLoadingProvinces ? (
      ...
      {/* Phường/Xã — dùng react-select */}
      <div>
        <label className="block text-sm font-medium text-primary mb-1.5">Phường/Xã</label>
        {isLoadingWards ? (
          <div className="flex items-center gap-2 text-sm text-neutral-dark border border-neutral rounded-lg px-3 py-2.5">
            <Loader2 size={14} className="animate-spin" />
            Đang tải...
      ...
      <div>
        <label className="block text-sm font-medium text-primary mb-1.5">Địa chỉ cụ thể</label>
        <input className={inputCls} value={detailAddress} onChange={(e) => setDetailAddress(e.target.value)} placeholder="Số nhà, tên đường..." />
      </div>

      <div>
        <label className="block text-sm font-medium text-primary mb-2">Loại địa chỉ</label>
          ...
          {ADDRESS_TYPES.map(({ value, label, icon: Icon }) => (
            <button
              ...
              className={`cursor-pointer flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-sm font-medium transition-colors ${
                type === value ? "border-accent bg-accent text-white" : "border-neutral bg-neutral-light text-primary hover:border-accent"
              }`}
            >
              <Icon size={14} />
              {label}
      ...
      <label className="flex items-center gap-2.5 text-sm text-primary cursor-pointer select-none">
        <input type="checkbox" checked={isDefault} onChange={(e) => setIsDefault(e.target.checked)} className="w-4 h-4 accent-accent rounded cursor-pointer" />
        Đặt làm địa chỉ mặc định
      </label>

      <div className="flex gap-3 pt-1">
        <button onClick={() => setShowModal(false)} className="cursor-pointer flex-1 border border-neutral rounded-lg py-2.5 text-sm font-medium text-primary hover:bg-neutral transition-colors">
          Hủy
        </button>
        <button
      ...
      <div className="p-4 sm:p-5">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 sm:py-20 text-center">
            ...
            <button onClick={openAddModal} className="cursor-pointer bg-accent hover:bg-accent-hover text-white px-6 py-2.5 rounded-lg text-sm font-medium transition-colors">
              Cập nhật ngay
            </button>
          </div>
        ) : (
          <div className="space-y-3">
            {addresses.map((addr) => {
              ...
              return (
                <div key={addr.id} className={`rounded-xl border p-4 sm:p-5 transition-colors ${addr.isDefault ? "border-accent bg-accent-light" : "border-neutral bg-neutral-light"}`}>
                  <div className="flex items-start justify-between gap-2 mb-3">
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      <button onClick={() => openEditModal(addr)} className="cursor-pointer p-1.5 rounded-lg hover:bg-neutral text-primary opacity-60 hover:opacity-100 transition-all" title="Sửa">
                        <Pencil size={15} />
                      </button>
                      {!addr.isDefault && (
                        <button onClick={() => openDeleteModal(addr)} className="cursor-pointer p-1.5 rounded-lg hover:bg-red-50 text-red-400 hover:text-red-600 transition-all" title="Xóa">
                          <Trash2 size={15} />
                        </button>
                      )}
                    </div>
                  ...
                  <p className="text-sm text-primary opacity-70 leading-relaxed mb-3">{addr.fullAddress}</p>

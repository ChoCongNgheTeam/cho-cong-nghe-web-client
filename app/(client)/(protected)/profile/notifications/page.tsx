...
function EmptyState() {
   return (
      <div className="flex flex-col items-center justify-center py-10 sm:py-16 px-4 sm:px-6">
         ...
         />
         <h3 className="text-sm sm:text-base font-semibold text-primary mt-2 mb-1">
         ...
         <Link
            href="/"
            className="bg-promotion hover:bg-promotion/90 active:scale-[0.98] text-white text-sm font-semibold px-7 py-2.5 rounded-full transition-all duration-150 shadow-sm hover:shadow-md"
         >
            Khám phá ngay
         </Link>
      </div>
      ...
      <div
        ...
        ${
           !item.isRead
              ...
              : "bg-neutral-light hover:bg-neutral/20 cursor-default"
        }
      `}
      >
         {/* Icon bubble */}
        ...
        `}
         ...
         <div className="flex-1 min-w-0">
            ...
            <div className="flex items-start justify-between gap-2 sm:gap-3">
               ...
               {!item.isRead && (
                  <span className="mt-1.5 w-2 h-2 rounded-full bg-blue-500 shrink-0 ring-2 ring-blue-200" />
               )}
            </div>

            {/* Body */}
            <p className="text-[12px] sm:text-[13px] text-neutral-darker/65 mt-0.5 line-clamp-2 leading-relaxed">
               {item.body}
            </p>
      ...
      <div className="flex flex-col">
         ...
         <div className="flex items-center justify-between mb-4 sm:mb-5">
            <div className="flex items-center gap-2 sm:gap-2.5">
               <h1 className="text-lg sm:text-[22px] font-semibold tracking-tight text-primary">
                  Thông báo
               </h1>
               {unreadCount > 0 && (
                  <span className="inline-flex items-center px-1.5 sm:px-2 py-0.5 text-[10px] sm:text-[11px] font-bold bg-blue-500 text-white rounded-full leading-none">
                     {unreadCount}
                  </span>
               )}
            </div>
            ...
            <div className="flex items-center gap-1 sm:gap-1.5">
               ...
               >
                  <RefreshCw
                     className={`w-4 h-4 sm:w-3.5 sm:h-3.5 ${isLoading ? "animate-spin" : ""}`}
                  />
         ...
         <div className="rounded-xl sm:rounded-2xl border border-neutral/70 overflow-hidden shadow-sm bg-neutral-light">
            {isLoading && notifications.length === 0 ? (
               ...
               <div
                  ...
                  className="overflow-y-auto max-h-[calc(100vh-200px)] sm:max-h-[calc(100vh-260px)] min-h-[200px] divide-y divide-neutral/40"
               >
                  ...
                  {!hasMore && notifications.length > 0 && (
                     <div className="flex items-center justify-center gap-3 py-4 sm:py-5">
                        ...
                        <span className="text-[10px] sm:text-[11px] text-neutral-darker/35 tracking-wide">
                           Đã hiển thị tất cả
                        </span>
                        <div className="h-px flex-1 max-w-[40px] sm:max-w-[48px] bg-neutral/60" />
                     </div>

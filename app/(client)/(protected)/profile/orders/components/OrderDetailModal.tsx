...
export default function OrderDetailModal({ order }: { order: Order }) {
  ...
  const methodUpper = (order.paymentMethod?.name ?? "")
    .toUpperCase()
    .replace(/\s+/g, "_");
  const isBankTransfer = BANK_TRANSFER_METHODS.some((m) =>
    methodUpper.includes(m.replace(/\s+/g, "_")),
  );
  const isRedirect = REDIRECT_PAYMENT_METHODS.some((m) =>
    methodUpper.includes(m),
  );
  ...
  return (
    <div className="max-h-[78vh] overflow-y-auto custom-scroll">
      {/* ── Header ─────────────────────────────────────────────────────────── */}
      <div className="sticky top-0 bg-neutral-light px-6 pt-5 pb-3 border-b border-neutral z-10">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex items-center gap-2 flex-wrap justify-end">
            <span
              className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${orderStatus.color} ${orderStatus.bgColor}`}
            >
              <span className={`w-1.5 h-1.5 rounded-full ${orderStatus.dot}`} />
              {orderStatus.label}
            </span>
            <span
              className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold ${paymentStatus.color} ${paymentStatus.bgColor}`}
            >
              {paymentStatus.label}
            </span>
          </div>
      ...
      <div className="px-6 pb-6 space-y-5 mt-4">
        ...
        {needsPayment &&
          (isBankTransfer || (isRedirect && order.paymentRedirectUrl)) && (
            <div
            ...
            >
              ...
              {isBankTransfer ? (
                <button
                  ...
                  className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-primary text-neutral-light text-xs font-semibold hover:bg-primary-hover transition-colors shrink-0 cursor-pointer"
                >
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-accent text-neutral-light text-xs font-semibold hover:bg-accent-hover transition-colors shrink-0 cursor-pointer"
                >
                ...
                >
                  <div className="relative w-12 h-12 shrink-0 rounded-lg bg-neutral overflow-hidden">
                    {imageUrl ? (
                      ...
                      <div className="w-full h-full bg-neutral-light-active flex items-center justify-center text-neutral-darker text-xs">
                        N/A
                      </div>
                    )}
                  </div>
        ...
        <div>
          ...
          <div className="bg-neutral-light rounded-xl px-4 py-3 space-y-2 text-sm">
            ...
            <div className="flex justify-between font-bold text-primary border-t border-neutral pt-2 mt-1">
              <span>Tổng cộng</span>
              <span className="text-base text-promotion">

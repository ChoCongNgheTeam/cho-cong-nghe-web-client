...
export default function OrderCard({ order, onViewDetail, onCancelSuccess, onReorderSuccess, onBeforeNavigate }: OrderCardProps) {
  ...
  const [localReviewedIds, setLocalReviewedIds] = useState<Set<string>>(new Set(order.orderItems.filter((item) => !item.canReview).map((item) => item.id)));
  ...
  const handlePaymentAction = () => {
    if (paymentAction.type === "bank") router.push(`/order/${order.orderCode}/payment`);
    else if (paymentAction.type === "redirect" && paymentAction.url) window.open(paymentAction.url, "_blank", "noopener,noreferrer");
  };
  ...
  return (
    <>
      <div className="bg-neutral-light rounded-xl border border-neutral overflow-hidden hover:shadow-md transition-shadow duration-200">
        {/* ── Header ── */}
        <div className="flex flex-col gap-1.5 sm:gap-2 px-3 sm:px-5 py-2.5 sm:py-3 bg-neutral-light border-b border-neutral">
          ...
          <div className="flex items-center justify-between gap-2">
            <span className="font-semibold text-xs sm:text-sm text-primary">#{order.orderCode}</span>
            <span className={`inline-flex items-center gap-1 sm:gap-1.5 px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-xs font-semibold ${orderStatus.color} ${orderStatus.bgColor}`}>
          ...
          <div className="flex flex-wrap items-center gap-x-2 sm:gap-x-3 gap-y-0.5 text-[10px] sm:text-xs text-neutral-darker">
            ...
            <span className="hidden xs:inline truncate max-w-[110px] sm:max-w-none">{order.paymentMethod.description}</span>
            <span className="hidden xs:inline text-neutral">•</span>
            {/* <span>{order.orderItems.length} SP</span>
            <span className="text-neutral">•</span> */}
            <span className={`inline-flex items-center px-1.5 sm:px-2 py-0.5 rounded-full text-[10px] sm:text-xs font-medium ${paymentStatus.color} ${paymentStatus.bgColor}`}>
              {paymentStatus.label}
            </span>
          </div>
          ...
          {order.paymentStatus === "UNPAID" && order.paymentExpiredAt && paymentAction.type !== null && (
            ...
            >
              ...
              <span className="truncate">
                Hết hạn TT: <span className="font-semibold">{formatDate(order.paymentExpiredAt, { withTime: true })}</span>
              </span>
            </div>
          )}
        </div>
          ...
          {order.orderItems.map((item) => {
            ...
            return (
              <div key={item.id} className="flex flex-col">
                {/* Product row */}
                <Link href={productSlug ? `/products/${productSlug}` : "#"} className="flex gap-2.5 sm:gap-3 px-3 sm:px-5 py-3 sm:py-3.5 hover:bg-neutral-light-active transition-colors">
                  {/* Image */}
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs sm:text-sm font-medium text-primary line-clamp-2 sm:line-clamp-1">{item.productVariant?.product?.name}</p>
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-xs sm:text-sm font-semibold text-primary">{formatVND(Number(item.unitPrice))}</p>
                    {Number(item.productVariant?.price) > Number(item.unitPrice) && (
                      <p className="text-[10px] sm:text-xs line-through text-neutral-darker">{formatVND(Number(item.productVariant?.price))}</p>
                    )}
                  </div>
                </Link>
                ...
                {showReview && (
                  <div className="px-3 sm:px-5 py-2 sm:py-2.5 flex items-center justify-between bg-neutral-light border-t border-neutral gap-2">
                    ...
                    <div className="hidden sm:flex items-center gap-1.5 min-w-0">
                      ...
                      <span className="text-xs text-neutral-darker truncate">Hãy đánh giá sản phẩm bạn vừa nhận được!</span>
                    </div>
                    {/* Mobile: just star icon */}
                    <div className="flex sm:hidden items-center gap-1">
                      <Star className="w-3.5 h-3.5 text-amber-500 fill-amber-400" />
                      <span className="text-[10px] text-neutral-darker">Đánh giá</span>
                    </div>
                    ...
                    >
                      ...
                      <span className="hidden sm:inline">{alreadyReviewed ? "Đã đánh giá ✓" : "Đánh giá ngay"}</span>
                      <span className="sm:hidden">{alreadyReviewed ? "✓" : "Ngay"}</span>
                    </button>
                  </div>
                )}
              </div>
        ...
        <div className="px-3 sm:px-5 py-3 sm:py-4 bg-neutral-light border-t border-neutral">
          ...
          <div className="flex items-center justify-between mb-2.5 sm:hidden">
            ...
            <span className="text-sm font-bold text-primary">{formatVND(Number(order.totalAmount))}</span>
          </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-4">
            ...
            <div className="flex flex-wrap items-center gap-1.5 sm:gap-2.5">
              ...
              {showReorder && <ReorderButton orderId={order.id} onReorderSuccess={onReorderSuccess} onBeforeNavigate={onBeforeNavigate} />}

              {showCancel && <CancelOrderButton orderId={order.id} onCancelSuccess={onCancelSuccess} />}
              ...
              {showPayment && paymentAction.type === "bank" && (
                ...
                >
                  ...
                  <span className="hidden sm:inline">Xem thông tin CK</span>
            ...
            <div className="hidden sm:block text-right shrink-0">
              ...
              <span className="text-base font-bold text-primary">{formatVND(Number(order.totalAmount))}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {reviewTarget && (
        <ReviewModal
          ...
          onSuccess={(stars: number) => {
            setLocalReviewedIds((prev) => new Set(prev).add(reviewTarget.orderItemId));

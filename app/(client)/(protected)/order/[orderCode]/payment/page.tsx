   ...
   return (
      ...
      >
         {copied ? (
            ...
            <Copy className="w-3.5 h-3.5 text-neutral-darker" />
         )}
      </button>
   );
}
   ...
   return (
      <div className="flex items-center justify-between py-2.5 border-b border-neutral last:border-0">
         ...
         <div className="flex items-center min-w-0 text-right">
            <span
               className={`text-sm break-all ${highlight ? "font-bold text-accent-dark text-base" : "font-medium text-primary"}`}
            >
               {value}
            </span>
...
export default function OrderPaymentPage() {
   ...
   const isBankTransfer = data.paymentMethodCode
      .toUpperCase()
               ...
               <div className="bg-neutral-light-active rounded-2xl shadow-sm overflow-hidden border border-neutral">
                  ...
                  <div className="p-5 space-y-4">
                     ...
                     <div className="bg-neutral rounded-xl px-4 py-1">
                        ...
                        <InfoRow
                           label="Số tiền"
                           value={`${new Intl.NumberFormat("vi-VN").format(total)} VND`}
                           highlight
                           copy
                        />
                        {data.bankTransferContent && (
                           <InfoRow
                              label="Nội dung CK"
                              value={data.bankTransferContent}
                              copy
                           />
                        )}
                     </div>
                     ...
                     <div className="bg-promotion-light border border-promotion-light-active rounded-xl p-3">
                        <p className="text-xs text-promotion-dark leading-relaxed">
                           ⚠️ Vui lòng ghi{" "}
                           <strong>đúng nội dung chuyển khoản</strong> để đơn
                        ...
                        <div key={item.id} className="flex gap-3 px-5 py-3.5">
                           {item.image ? (
                              <img
                                 src={item.image}
                                 alt=""
                                 className="w-14 h-14 rounded-lg object-cover border border-neutral shrink-0"
                              />
                           ) : (
            ...
            <div className="bg-neutral-light-active rounded-2xl shadow-sm overflow-hidden border border-neutral">
               ...
               <div className="px-5 py-4">
                  <p className="text-sm font-semibold text-primary">
                     ...
                     <span className="font-normal text-neutral-darker mx-1.5">
                        •
                     </span>
                     {data.shippingPhone}
                  </p>

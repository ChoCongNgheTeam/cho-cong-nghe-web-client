...
function CopyBtn({ text }: { text: string }) {
  ...
  return (
    <button
      ...
      className="ml-1.5 p-1 rounded hover:bg-neutral transition-colors shrink-0"
    >
      {copied ? <CheckCheck className="w-3.5 h-3.5 text-green-500" /> : <Copy className="w-3.5 h-3.5 text-neutral-darker" />}
    </button>
  );
}
...
export default function OrderPaymentPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await apiRequest.get<{ success: boolean; data: OrderData }>(`/orders/by-code/${orderCode}/payment-info`);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-neutral-light px-4">
        <div className="text-center space-y-3">
          <p className="text-sm text-red-500">{error ?? "Không tìm thấy đơn hàng."}</p>
    ...
    <div className="min-h-screen bg-neutral-light py-8 px-4">
      <div className="w-full max-w-lg mx-auto space-y-4">
        ...
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
          ...
          {/* Toggle items */}
          <button onClick={() => setShowItems((v) => !v)} className="w-full flex items-center justify-between px-5 py-3.5 hover:bg-neutral transition-colors border-b border-neutral">
          ...
          <div className="px-5 py-4 space-y-2.5 border-t border-neutral">
            ...
            {voucher > 0 && (
              <div className="flex justify-between text-sm">
                ...
                <span className="text-green-600 font-medium">-{formatVND(voucher)}</span>
              </div>
            )}
            {tax > 0 && (
              <div className="flex justify-between text-sm">
        ...
        <div className="space-y-3 pb-4">
          {isBankTransfer && (
            <button onClick={() => router.push("/profile/orders")} className="w-full bg-accent text-primary-darker font-semibold py-3.5 rounded-xl hover:bg-accent-dark transition-colors text-sm">
              Tôi đã chuyển khoản
            </button>
          )}
          <button
            ...
            className={`w-full font-semibold py-3.5 rounded-xl transition-colors text-sm ${
              isBankTransfer ? "border border-neutral text-neutral-darker hover:bg-neutral" : "bg-accent text-primary-darker hover:bg-accent-dark"
            }`}
          >
            Xem đơn hàng
          </button>

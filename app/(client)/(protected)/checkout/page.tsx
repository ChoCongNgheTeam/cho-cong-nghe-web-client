...
function parseAddressError(err: any): string {
   ...
   if (code === "DUPLICATE") {
      ...
      if (hasPhone && hasAddress)
         return "Số điện thoại và địa chỉ này đã được lưu trong tài khoản. Vui lòng chọn từ địa chỉ đã lưu thay vì nhập lại.";
      if (hasPhone)
         return "Số điện thoại này đã được dùng cho một địa chỉ khác. Vui lòng dùng số khác hoặc chọn từ địa chỉ đã lưu.";
      if (hasAddress)
         return "Địa chỉ này đã tồn tại trong tài khoản. Vui lòng chọn từ địa chỉ đã lưu.";
         ...
         if (!addressId) {
            try {
               ...
               if (isDuplicate) {
                  ...
                  if (hasPhone && hasAddress) {
                     toast.error(
                        "Số điện thoại và địa chỉ này đã được lưu trong tài khoản khác.",
                     );
                  } else if (hasPhone) {
                     toast.error(
                     ...
                     toast.error(
                        "Địa chỉ này đã tồn tại trong tài khoản. Vui lòng chọn từ địa chỉ đã lưu.",
                     );
                  } else {
                     toast.error(
                  ...
                  toast.error(
                     addrMsg || "Không thể lưu địa chỉ. Vui lòng thử lại.",
                  );
               }
               return;
            }
         ...
         <CartBottomBar
            ...
            summaryRows={[
               ...
               },
               {
                  ...
                  value:
                     shippingFee != null ? formatVND(shippingFee) : "Miễn phí",
               },
               {

...
export default function OrderSummary({
  ...
  const handleCheckoutClick = () => {
    if (!user) {
      sessionStorage.setItem("loginRedirectIntent", "/cart");
    ...
    <div className="rounded-xl bg-neutral-light sticky top-4 overflow-hidden border border-neutral">
      ...
      >
        <div className="flex items-center justify-between px-4 py-4 border-b border-neutral">
          ...
          <button onClick={() => setShowLoginHint(false)} className="text-neutral-dark hover:text-primary transition-colors">
            <X size={16} />
          </button>
        </div>
        ...
        >
          <div className="flex items-center gap-2.5 flex-1 min-w-0">
            ...
            <div className="flex flex-col items-start min-w-0">
              <span className="text-sm font-medium text-primary leading-tight">Chọn hoặc nhập ưu đãi</span>
              {appliedVoucherCode && (
          ...
          <ChevronRight size={16} className="text-neutral-dark group-hover:text-primary transition-colors shrink-0" />
        </button>
      )}
        ...
        <div className="space-y-2.5 text-sm">
          ...
          {totalPromotionDiscount > 0 && (
            <div className="flex justify-between items-center">
              ...
              <span className="font-medium text-neutral-darker">-{formatPrice(totalPromotionDiscount)}</span>
            </div>
          )}
          ...
          {appliedVoucherValue > 0 && (
            <div className="flex justify-between items-center">
              <span className="text-neutral-darker flex items-center gap-1.5">
                ...
                Voucher
                {appliedVoucherCode && <span className="text-xs bg-accent-light text-accent-dark font-semibold px-1.5 py-0.5 rounded-full">{appliedVoucherCode}</span>}
              </span>
              <span className="font-medium text-promotion">-{formatPrice(appliedVoucherValue)}</span>
            </div>
          )}
          ...
          <div className="flex justify-between items-center">
            ...
            {shippingFee == null ? (
              <span className="text-xs italic text-neutral-dark">Chọn địa chỉ để tính</span>
            ) : shippingFee === 0 ? (
              <span className="font-medium text-neutral-darker">Miễn phí</span>
            ) : (
              <span className="font-medium text-neutral-darker">{formatPrice(shippingFee)}</span>
            )}
          </div>
          ...
          {totalSaved > 0 && (
            <div className="flex justify-between items-center bg-neutral-light-active rounded-lg px-3 py-2 mt-1">
              <span className="text-xs font-medium text-neutral-darker">🎉 Bạn đã tiết kiệm được</span>
              <span className="text-xs font-bold text-neutral-darker">-{formatPrice(totalSaved)}</span>
            </div>
          )}
        </div>
      ...
      {showTerms && (
        <div className="px-4 py-3 bg-accent-light border-t border-neutral">
          <label className="flex gap-2 text-xs cursor-pointer items-start">
            <input type="checkbox" checked={agreedToTerms} onChange={(e) => onTermsChange?.(e.target.checked)} className="mt-0.5 cursor-pointer shrink-0 w-4 h-4 accent-accent" />
            <p className="text-neutral-darker leading-relaxed">
              Bằng việc tiến hành đặt mua hàng, bạn đồng ý với{" "}
              <Link href="/policies/TermsOfService" target="_blank" rel="noopener noreferrer" className="underline font-medium hover:text-promotion text-primary">
                Điều khoản dịch vụ
              </Link>{" "}
              và{" "}
              <Link href="/policies/DataPrivacy" target="_blank" rel="noopener noreferrer" className="underline font-medium hover:text-promotion text-primary">

...
export function TotalRow({ label, val, accent = false }: TotalRowProps) {
  return (
    <div className="flex justify-between items-center w-52">
      ...
      <span
        className={`font-mono ${
          accent ? "text-cyan-600 font-bold text-base" : "text-sm text-slate-700"
        }`}
      >
        {val}
      </span>
...
export function StatusBanner({ invoice, onReset }: StatusBannerProps) {
  ...
  return (
    <div
      className={`${sc.bg} ${sc.border} border rounded-2xl px-5 py-4 flex items-center gap-4`}
    >
      <div
        className={`w-9 h-9 rounded-xl ${sc.badge} flex items-center justify-center font-bold shrink-0`}
      >
        {sc.icon}
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span
            className={`text-xs px-2 py-0.5 rounded-full font-mono ${sc.badge}`}
          >
            {invoice.id}
          </span>
        </div>
        <p className={`text-xs mt-0.5 opacity-70 ${sc.text}`}>
          ...
          {invoice.status === "cancelled" && `Đã hủy ngày ${invoice.date}`}
        </p>
      </div>
      <button
        onClick={onReset}
    ...
    <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
      <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-start gap-4 bg-slate-50/60">
        <div>
          ...
          <p className="font-bold text-xl font-mono text-slate-900">
            {invoice.id}
          </p>
          <p className="text-xs text-slate-400 mt-1.5">
            Mẫu số <span className="text-slate-600">{invoice.template}</span>
            <span className="mx-2 opacity-30">|</span>
            Ký hiệu <span className="text-slate-600">{invoice.serial}</span>
          </p>
        </div>
        <div className="text-right shrink-0">
          <p className="text-[11px] text-slate-400 mb-1">Ngày lập</p>
      ...
      <div className="px-6 py-5 border-b border-slate-100 grid grid-cols-1 sm:grid-cols-2 gap-5">
        <div>
          <p className="text-[11px] text-slate-400 tracking-widest uppercase mb-3">
            Thông tin khách hàng
          </p>
          <div className="space-y-2">
            <Row label="Tên" val={invoice.customer} />
            <Row label="Email" val={invoice.email} />
            <Row label="SĐT" val={invoice.phone} />
            ...
            <thead>
              <tr className="border-b border-slate-100">
                {[
                  ...
                  <th
                    key={i}
                    className={`text-[11px] text-slate-400 font-semibold pb-3 ${
                      i === 0
                        ? "text-left w-6"
                        : i === 1
                        ? "text-left"
            ...
            <tbody className="divide-y divide-slate-50">
              {invoice.items.map((item, i) => (
                <tr key={i} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="py-3 text-right text-slate-800 font-mono text-xs font-semibold">
      ...
      <div className="px-6 py-5 border-b border-slate-100 flex justify-end bg-slate-50/40">
        <div className="space-y-2">
          ...
          <TotalRow label={`VAT (${invoice.tax}%)`} val={fmt(tax)} />
      ...
      <div className="px-6 py-4 flex flex-wrap items-center justify-between gap-3 bg-white">
        <p className="text-xs text-slate-400">Ngày lập: {invoice.date}</p>
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl p-6 mb-5 shadow-sm">
      ...
      <div className="flex gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="VD: INV-2024-001234"
      ...
      <div className="mt-4 pt-4 border-t border-slate-100">
        <p className="text-[11px] text-slate-400 mb-2">Thử mã mẫu:</p>
        <div className="flex flex-wrap gap-2">
          {sampleKeys.map((k) => (
            <button
              key={k}
              onClick={() => onPickSample(k)}

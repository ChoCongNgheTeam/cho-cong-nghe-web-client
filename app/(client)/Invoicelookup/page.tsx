...
export default function InvoiceLookupPage() {
  ...
  const search = async () => {
    ...
    if (!q) {
      setErr("Vui lòng nhập mã hóa đơn");
      return;
    }
    setErr("");
    setPhase("loading");
      ...
      <div className="relative max-w-2xl mx-auto px-5 py-14">
        <div className="text-center mb-10">
          ...
          <p className="text-slate-500 text-sm max-w-xs mx-auto leading-relaxed">
            Nhập mã hóa đơn để xem chi tiết, trạng thái và tải PDF.
          </p>
        </div>
        ...
        {phase === "notfound" && (
          <div className="bg-white border border-slate-200 rounded-2xl p-10 text-center shadow-sm">
            ...
            <p className="text-slate-400 text-sm mb-5">
              ...
              không tồn tại. Vui lòng kiểm tra lại hoặc liên hệ hỗ trợ.
            </p>
            <button
              onClick={reset}
        ...
        <div className="mt-12 text-center space-y-2">
          <div className="flex flex-wrap justify-center gap-6 text-xs text-slate-400">
            {[
              "🔒 SSL 256-bit",
              "📋 Mã hóa end-to-end",
              "✓ Tuân thủ TCVN 11-2021",
            ].map((t, i) => (
              <span key={i}>{t}</span>
            ))}
          </div>
          <p className="text-xs text-slate-400">
            © 2024 Hệ thống hóa đơn điện tử · Hotline: 1800 0000
          </p>
        </div>
      </div>
    </div>

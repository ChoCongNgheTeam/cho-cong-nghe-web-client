...
export default function ProductDetailSection({ slug, product }: ProductDetailSectionProps) {
  ...
  const openDialog = async () => {
    ...
    try {
      ...
      // ← Nhận thêm name và image từ API mới
  ...
  const formatSpecValue = (item: SpecificationItem) => (item.unit ? `${item.value} ${item.unit}` : item.value);
          ...
          <div className="space-y-6 w-full lg:w-[95%] xl:w-[90%] mx-auto">
            {specifications.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-10 text-neutral-dark gap-2">
                ...
                <p className="text-sm opacity-50">Chưa có thông số kỹ thuật</p>
              </div>
            ) : (
              specifications.slice(0, 3).map((group) => (
                <div key={group.groupName}>
                  <h3 className="text-sm sm:text-base font-semibold text-primary mb-3">{group.groupName}</h3>
                        ...
                        {group.items.map((item, index) => (
                          <tr
                            ...
                            className={`
                              ...
                              `}
                          >
                            <td className="px-3 sm:px-4 py-2 text-neutral-darker w-1/2">{item.name}</td>
                            <td className="px-3 sm:px-4 py-2 text-primary font-medium">{formatSpecValue(item)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>

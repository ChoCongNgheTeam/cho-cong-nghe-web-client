...
export default function ProductDetailRight({
  ...
  const storages: StorageOption[] =
    availableOptions
      ...
      ?.values?.filter((val: any) => val.enabled)
      ?.map((val: any) => ({
        label: val.label,
        value: val.value,
      })) ?? [];
  ...
  const handleQuantityChange = (newQuantity: number) => {
    const valid = Math.min(Math.max(1, newQuantity), maxStock);
    setQuantity(valid);
  };
  ...
  const handleBuyNow = async () => {
    ...
    const meta = {
      ...
      variantName: selectedVariant.sku ?? selectedVariant.name ?? "",
      price: finalPrice,
      originalPrice: basePrice,
      discountPercentage: activePrice?.discountPercentage ?? 0,
      imageUrl: selectedVariant.image ?? selectedVariant.images?.[0]?.imageUrl ?? "",
      availableQuantity: selectedVariant.quantity ?? selectedVariant.availableQuantity ?? 0,
    ...
    try {
      await addToCart(selectedVariant.id, 1, meta);
      router.push("/cart");
    } catch {
  ...
  const handleInstallment = () => {
    ...
    // TODO: implement navigation to installment page
  };
  ...
  return (
    <div className="w-full">
      ...
      {/* Title */}
      <h2 className="my-3 sm:my-4 text-lg sm:text-xl lg:text-2xl font-bold text-primary transition-colors duration-300">{product.name}</h2>

      {/* Rating & Links */}
      <div className="flex flex-wrap items-center gap-2 text-xs sm:text-sm">
        <span className="transition-colors duration-300">{product.currentVariant?.code ?? ""}</span>
        <div className="flex items-center gap-1">
          <FaStar className="text-accent text-xs sm:text-sm" />
          <span className="text-primary">{product.rating.average.toFixed(1)}</span>
        </div>
        <button onClick={onReviewClick} className="text-accent hover:underline hover:text-accent-hover transition-colors cursor-pointer">
          {product.rating.total} đánh giá
        </button>
        <span>|</span>
        <button type="button" onClick={onSpecificationClick} className="text-accent hover:underline hover:text-accent-hover transition-colors cursor-pointer">
          Thông số kỹ thuật
        </button>
      </div>
      ...
      <div className="grid grid-cols-[90px_1fr] sm:grid-cols-[100px_1fr] gap-y-4 sm:gap-y-5 gap-x-4 mt-6">
        {/* Storage */}
        <span className="font-medium text-primary text-xs sm:text-sm flex items-center">Dung lượng:</span>
          ...
          {storages.map((storage) => {
            ...
            return (
              <span
                ...
                className={`border rounded-sm px-3 py-2 sm:px-4 sm:py-3 text-xs sm:text-sm font-bold cursor-pointer relative overflow-hidden transition-colors
        ...
        {/* Color */}
        <span className="font-medium text-primary text-xs sm:text-sm flex mt-4">Màu sắc:</span>
        <div className="flex flex-wrap gap-2">
          {colors
            ...
            .map((color) => {
              ...
              return (
                <span
                  ...
                  className={`px-3 py-2 sm:px-4 sm:py-3 rounded-sm text-xs sm:text-sm font-bold cursor-pointer border relative overflow-hidden flex items-center gap-2 transition-colors
                    ${isActive ? "border-promotion text-promotion bg-promotion-light" : "bg-neutral-light"}
                    hover:bg-promotion-light`}
                >
                  {color.image && <Image src={color.image} alt={color.value} width={28} height={28} className="object-contain" />}
                  {color.label}
                  {isActive && (
                    <div className="absolute -top-1 -right-2 w-0 h-0 border-l-[30px] border-l-transparent border-t-[30px] border-t-promotion">
                      <span className="absolute -top-[28px] -right-[-7px] text-white text-xs font-bold">✓</span>
      ...
      {isInStock ? (
        <>
          ...
          <div className="bg-accent-light p-3 sm:p-4 rounded-lg mb-4 border border-accent transition-colors duration-300">
            ...
            {/* Promotions */}
            {product.availablePromotions?.length ? (
            ...
            <div className="px-3 sm:px-4 pb-3 sm:pb-4 text-xs sm:text-sm">
              <div className="flex items-start gap-3 my-3">
                ...
                <div className="flex flex-col min-w-0">
                  ...
                  <Link href="#" className="text-promotion hover:text-promotion-hover hover:underline transition-colors">
                    Xem chi tiết
                  </Link>
                </div>
              </div>
        ...
        <div className="mt-4 rounded-2xl overflow-hidden border border-neutral shadow-sm">
          <div className="bg-neutral px-5 py-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-primary font-bold text-sm sm:text-base leading-tight">Sản phẩm tạm thời hết hàng</p>
          ...
          <div className="border-t border-neutral bg-neutral-light-active px-5 py-3 flex flex-col items-center gap-3 w-full">
            ...
            <a href="tel:18006601" className="flex items-center gap-1.5 text-xs text-neutral-dark hover:text-promotion transition-colors whitespace-nowrap">

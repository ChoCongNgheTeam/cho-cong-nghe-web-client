...
import OptionValues from "@/(client)/products/components/OptionValues";
...
const HighlightsBlock = memo(function HighlightsBlock({ highlights }: { highlights: Highlight[] }) {
...
export const PoliciesBlock = memo(function PoliciesBlock() {
  return (
    <div className="mt-5">
...
const ProductDetailRight = memo(function ProductDetailRight({
  product,
  selectedOptions = {},
  onOptionChange,
  selectedVariant,
      ...
      <div className="flex flex-col gap-y-4 sm:gap-y-5 mt-6">
        {availableOptions.map((option) => (
          <div key={option.type} className="flex flex-col gap-2">
            <span className="font-medium text-primary text-xs sm:text-sm">{TYPE_LABELS[option.type] ?? option.type}:</span>
            <OptionValues option={option} selectedOptions={selectedOptions} onOptionChange={onOptionChange} />
          </div>
        ))}

                  ...
                  <Star
                    ...
                    className={`w-4 h-4 sm:w-5 sm:h-5 ${
                      star <= 8
                        ...
                        : "fill-neutral-dark text-neutral-dark"
                    }`}
                  />
                ))}
              </div>
        ...
        <div className="border-t border-neutral-dark pt-4 sm:pt-6">
          ...
          <div className="mb-6">
            <div className="relative">
              <input
                ...
                className="w-full px-3 sm:px-4 py-3 border border-neutral-dark rounded-lg pr-3 sm:pr-32 focus:outline-none focus:border-neutral-darker text-sm sm:text-base bg-neutral-light text-primary"
                maxLength={3000}
              />
              <div className="flex items-center gap-2 mt-2 sm:mt-0 sm:absolute sm:right-3 sm:top-1/2 sm:-translate-y-1/2">
                ...
                <button className="px-4 sm:px-6 py-2 bg-primary text-white rounded-full text-xs sm:text-sm font-medium hover:bg-primary-hover ml-auto cursor-pointer">
                  Gửi bình luận
                </button>
              </div>
            </div>
          ...
          <div className="space-y-4">
            {reviews.map((review) => (
              <div
                key={review.id}
                className="border-b border-neutral-dark pb-4"
              >
                <div className="flex gap-2 sm:gap-3">
                  <div className="w-8 h-8 sm:w-10 sm:h-10 bg-neutral rounded-full flex items-center justify-center text-white font-semibold text-sm sm:text-base flex-shrink-0">
                    {review.avatar}
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-neutral-darker mb-2 text-sm sm:text-base break-words">
                      {review.content}
                    </p>
                    <div className="flex items-center gap-3 sm:gap-4 text-sm">
                      <button className="flex items-center gap-1 text-neutral-darker hover:text-primary cursor-pointer">
                        <AiOutlineLike width={32} /> {review.likes}
                      </button>
                      <button className="text-neutral-darker hover:text-primary cursor-pointer">
                        ↩️ Trả lời
                      </button>
                    </div>
                    ...
                    {review.replies && review.replies.length > 0 && (
                      <div className="mt-4 ml-4 sm:ml-8 bg-neutral rounded-lg p-3 sm:p-4">
                        {review.replies.map((reply) => (
                          <div key={reply.id} className="flex gap-2 sm:gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 mb-1 flex-wrap">
                                <span className="font-semibold text-sm sm:text-sm text-primary">
                                  {reply.author}
                                </span>
                                <span className="px-2 py-0.5 bg-neutral-darker text-primary-darker text-sm rounded whitespace-nowrap">
                                  {reply.role}
                                </span>
                              ...
                              <p className="text-neutral-darker whitespace-pre-line text-sm sm:text-sm break-words">
                                {reply.content}
                              </p>
                            </div>
                          </div>

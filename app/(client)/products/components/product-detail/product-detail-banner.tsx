...
export default function ProductDetailBanner({ product, images, selectedVariant }: ProductDetailLeftProps) {
                  ...
                  return (
                    <div className="flex-1" key={index}>
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        {IconComponent && <IconComponent size={28} className="" />}
                        <span className="text-base font-semibold text-primary">{highlight?.value || "N/A"}</span>
                      </div>
                    </div>
                  );
                })}
          ...
          {isInStock && (
            <div>
              <div className="flex flex-col sm:flex-row justify-between mt-6 items-start sm:items-center gap-2">
                ...
                <button className="text-sm text-promotion hover:text-accent-hover hover:underline cursor-pointer transition-colors duration-200">Tìm hiểu thêm</button>
              </div>

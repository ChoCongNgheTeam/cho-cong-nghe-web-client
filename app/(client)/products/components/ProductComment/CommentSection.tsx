...
export default memo(function CommentSection({ productId, comments: initialComments, loading, onCommentSubmit, onReplySubmit, onFetchReplies }: CommentSectionProps) {
  ...
  const [commentIdFromUrl] = useState<string | null>(() => {
    if (typeof window === "undefined") return null;
    return new URLSearchParams(window.location.search).get("commentId");
  });

  const handleReplyContentChange = useCallback((id: string, value: string) => {
  ...
  const handleSubmitReply = useCallback(
    async (parentId: string) => {
      if (!isAuthenticated) {
        toastRef.current.warning("Vui lòng đăng nhập để trả lời");
        return;
      }
    ...
    [replyContents, replySubmitting, initialComments, onReplySubmit, isAuthenticated],
  );

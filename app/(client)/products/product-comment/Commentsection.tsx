...
const CommentNode = memo(function CommentNode({
   ...
   return (
      <div id={`comment-${node.id}`} className="flex gap-2 sm:gap-3">
         ...
         <div className="flex-1 min-w-0">
            <div className="mb-1 flex flex-wrap items-center gap-1.5 sm:gap-2">
               ...
               <span className="text-[11px] text-neutral-darker sm:text-xs">
                  • {formatRelativeDate(node.createdAt)}
               </span>
            </div>

            <p className="mb-2 whitespace-pre-wrap text-xs text-neutral-darker wrap-break-word sm:text-sm">
               {node.content}
            </p>
            ...
            <div className="flex items-center gap-3 text-xs text-neutral-darker sm:gap-4">
               ...
               {canReply && (
                  <button
                     onClick={() =>
                        onSetReplyTarget(
                           replyTargetId === node.id ? null : node.id,
                        )
                     }
            ...
            {canReply && replyTargetId === node.id && (
               <div className="mt-3 flex gap-2">
                  ...
                  <button
                     ...
                     disabled={
                        !replyContents[node.id]?.trim() || isSubmittingReply
                     }
...
export default function CommentSection({
   ...
   const [pendingScrollParentId, setPendingScrollParentId] = useState<
      string | null
   >(null);
   ...
   const commentIdFromUrl = useMemo(() => {
      ...
      return new URLSearchParams(window.location.search).get("commentId");
   }, []);
   ...
   useEffect(() => {
      ...
      const parentComment = initialComments.find((c) =>
         (c.replies ?? []).some((r) => r.id === commentIdFromUrl),
      );
   ...
   useEffect(() => {
      ...
      const parent = initialComments.find(
         (c) => c.id === pendingScrollParentId,
      );
      const replyExists = parent?.replies?.some(
         (r) => r.id === commentIdFromUrl,
      );
   ...
   useEffect(() => {
      ...
      const idx = initialComments.findIndex(
         (c) =>
            c.id === commentIdFromUrl ||
            (c.replies ?? []).some((r) => r.id === commentIdFromUrl),
      );
   ...
   const handleSubmitReply = useCallback(
      async (parentId: string) => {
         ...
         try {
            ...
            if (result?.isApproved === false || result?.status === "pending") {
               toast.info(
                  "Phản hồi của bạn đã được gửi và đang chờ quản trị viên phê duyệt.",
                  {
                     title: "Đang chờ duyệt",
                     duration: 5000,
                  },
   ...
   return (
      <div ref={commentListRef} data-product-id={productId}>
         <h4 className="mb-6 text-lg font-semibold text-primary sm:mb-8 sm:text-2xl">
            Bình luận
         </h4>
         ...
         <div className="mb-6 flex flex-col gap-4 border-b border-neutral pb-6 sm:flex-row sm:gap-6">
            ...
            <div className="min-w-0 flex-1">
               <h4 className="mb-1 text-base font-semibold text-primary opacity-80 sm:text-lg">
                  Hãy đặt câu hỏi cho chúng tôi
               </h4>
               <p className="text-xs leading-relaxed text-primary opacity-60 sm:text-sm">
                  ChoCongNghe sẽ phản hồi trong vòng 1 giờ. Nếu Quý khách gửi
               ...
               <div className="mt-3 flex flex-col gap-2 sm:flex-row">
                  <div className="relative min-w-0 flex-1">
                     <input
                        ...
                        onKeyDown={(e) =>
                           e.key === "Enter" && void handleSubmitComment()
                        }
                     ...
                     />
                     <span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-[11px] text-neutral-darker">
                        {comment.length}/3000
                     </span>
                  </div>
                  <button
                     ...
                     disabled={
                        !comment.trim() || submitting || !isAuthenticated
                     }
            ...
            {loading && initialComments.length === 0 ? (
               ...
               <>
                  {visibleComments.map((c) => (
                     <div
                        key={c.id}
                        className="border-b border-neutral pb-4 last:border-0 sm:pb-5"
                     >
                        <CommentNode
                           node={c}
                           depth={0}

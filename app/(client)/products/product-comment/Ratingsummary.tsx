...
export default function RatingSummary({
  ...
  const fetchProductData = async () => {
    ...
    try {
      ...
      if (p.rating) setLocalRating(p.rating);
    } catch {
  ...
  // Auto-trigger review sau khi đăng nhập redirect về
  useEffect(() => {
  ...
  const reviewButton = (
    <button
      ...
      ${
        permission.orderItemId && !permission.canReview
          ? "bg-neutral text-primary cursor-not-allowed"
          : "bg-promotion hover:bg-promotion-hover text-neutral-light cursor-pointer"
      }`}
    >
      {permission.orderItemId && !permission.canReview
    ...
    <>
      ...
      <div id="rating-summary">
        <h2 className="text-2xl font-semibold mb-6 text-primary">
          Đánh giá {currentVariant?.name ?? product.name}
        </h2>

        <div
          className={`flex ${hasReviews ? "flex-col sm:flex-row gap-10" : "flex-col items-center"}`}
        >
          {/* LEFT */}
            ...
            {hasReviews ? (
              <>
                ...
                <div className="flex gap-1 mb-2">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <Star
                      key={star}
                      className={`w-5 h-5 ${
                        star <= Math.round(localRating.average)
                ...
                <div className="text-sm text-neutral-darker mb-3">
                  {localRating.total} lượt đánh giá
                </div>
                {reviewButton}
              </>
            ) : (
            ...
            <div className="flex gap-2.5 w-full">
              ...
              <button
                onClick={() => {
                  ...
                  router.push(
                    `/account?redirect=${encodeURIComponent(pathname + "?review=true")}`,
                  );
                }}

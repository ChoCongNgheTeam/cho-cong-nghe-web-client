...
import { getWishlist } from "@/app/(client)/(protected)/profile/wishlist/_lib";
...
export function WishlistProvider({ children }: { children: React.ReactNode }) {
  ...
  // Reset khi logout — dùng useMemo/derive thay vì setState trong effect
  const resolvedCount = isAuthenticated ? count : 0;
  const resolvedLikedIds = isAuthenticated ? likedIds : new Set<string>();
  ...
  return <WishlistContext.Provider value={{ count: resolvedCount, likedIds: resolvedLikedIds, refresh, toggleLiked }}>{children}</WishlistContext.Provider>;
}
